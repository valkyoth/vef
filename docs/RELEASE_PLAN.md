...
- Acceptance contract: Add a separate dependency-free, no_std `vef-media-type` crate depending only on `vef-core`; implement bounded incremental parsing and validation of media type, subtype, ordered parameters, tokens, quoted strings, quoted-pair escapes, and permitted whitespace with independent raw-byte, field-count, parameter-count, decoded-byte, and work limits. Preserve raw values while comparing type/subtype and parameter names case-insensitively; reject invalid token/quote/escape syntax and duplicate parameter names without substring, prefix, suffix, or lossy normalization. Return sealed `ValidatedMediaType` evidence bound to the exact field bytes and message generation, plus non-authoritative borrowed views for inspection; this stop does not parse multipart bodies or assign partial-response authority.
- Integrate `ValidatedMediaType` into outbound content-bearing message construction before serialization. v0.158.0 must consume it for generated OPTIONS content, and later semantic validators must reuse the same evidence instead of implementing another Content-Type parser. A missing Content-Type where the selected response requires one, malformed/duplicate field input, stale evidence, or same-generation field substitution is local InvalidState and emits zero head/body bytes.
...
- Exhaust type/subtype casing, permitted whitespace, token/quoted parameter values, quoted-pair escapes, parameter order, duplicate names, empty/malformed values, field substitution, stale/cross-message evidence, all independent limits, every input split/truncation, round trips, arbitrary-input no-panic behavior, and no hidden allocation. Prove generated OPTIONS content can use only exact current `ValidatedMediaType`, emits nothing for missing/invalid/stale evidence, and no second Content-Type grammar exists in an HTTP engine or role facade.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
...
- Test sealed Content-Type absent/non-multipart/multipart/malformed/duplicate/invalid-boundary classifications and prove multipart never reaches top-level Content-Range. Exhaust all public delivery preferences and engine-selected permits under every chunk split, preference/permit mismatch, forged/stale/cross-response permit, storage-before-publication order, acknowledgement/recycle attempt, backpressure, cancellation, and capacity boundary. Prove 206 retention preflights before body, local capacity is not peer error, RetainOnly failure cancels, StreamAndRetain incomplete-200 exhaustion permanently continues StreamOnly/NoRetainedPrefix, and cancellation exposes a prefix only when all received bytes were retained. Test zero-retention ranges larger than every profile, terminal proof, optional safe leases, and DMA fencing. Exhaust head/trailer ETag finalization. For variant selection test bounded/case-insensitive Vary deduplication, `*` anywhere, exact original-request values, absent versus empty, combinable versus non-combinable repeated fields, sealed normalizers versus conservative comparison, caller principal non-forgeability, trailer Vary NoRecombine, semantic identity equality across distinct fresh response generations, evidence inequality/provenance rejection across those generations, stale/cross-correlation evidence, every split/limit, and no panic/allocation.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
...
- Exhaust safe split/separate-arena output acquisition, compile-fail alias/mutation/recycle/decode attempts, stale physical lease generations, bad copy schedules, output exhaustion, cancellation, and DMA/device adapter fencing. Combine distinct request/connection/storage generations only after fresh evidence admission and semantic-identity equality; reject forged/stale/rebound evidence, identity mismatch, caller-normalized identities, standalone-only, multipart, unknown-unit, principal/partition/navigation/Vary/coding/domain mismatch, and every `Vary: *`; prove per-response generations differ without poisoning semantic equality, while evidence freshness remains mandatory. Prove arena migration neither preserves stale leases nor blocks same-key full-200 invalidation. Test head/trailer strong-validator finalization and conflicts without ordinal change; mint ordinals at head publication, delay/reorder bodies, retry, manipulate Date, and rotate on exhaustion. Exhaust unequal overlap quarantine and prove no 304, unchanged-validator revalidation/generation refresh, caller assertion, retry, new navigation/selection context, storage rotation, or partial response clears the old context; test only complete same-key 200 replacement or total destruction followed by different-validator/new-generation empty context creation clears it. Exhaust `OverlapComparisonBudget` and sorted-interval model tests, then verify header replacement, unions, lengths, backpressure, and no panic/allocation.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
