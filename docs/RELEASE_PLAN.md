...
- Store policy disposition, current RFC wire state, local reset-output progress/reason/reservation, remote-closure cause, terminal-validation state/stage, compression-workspace ownership, immutable terminal-field-section lease, and active field-block ownership as independent generation-checked fields; policy rejection, closure, HPACK completion, or an intermediate semantic stage alone never releases another dimension. A zero-byte policy-CANCEL reservation can become dormant after remote END_STREAM without freeing its slot, so only final validation can release it or re-arm the same bounded slot without allocation. A locally reset stream's tombstone retains required state through the closed-stream classification horizon.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Acceptance contract: Emit at most one RST_STREAM, account discarded DATA credit, tolerate bounded in-flight frames, discard queued output after reset, resolve END_STREAM/GOAWAY/shutdown races, and recycle only after connection effects. Track reset output, terminal state/stage, compression workspace, and terminal-field-section lease independently. Peer RST_STREAM supersedes a zero-byte action: pending semantics releases its immutable lease exactly once and becomes `AbortedByPeerReset`; pending HPACK records deferred abort, synchronizes, then releases workspace/section reservation. Remote END_STREAM retains the dormant reset and section lease through every stage. Final `Valid` transfers the lease to the final unpublished/message-event owner; `Malformed(code)` re-arms and releases it once; begun reset bytes remain immutable. GOAWAY alone releases nothing. Connection-fatal failure transfers workspace, lease/partial storage, reset, and cleanup once to bounded shutdown. Caller-sensitive storage remains redacted and caller-scrubbed after lease termination.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Rejected promised streams use the v0.118.0 orthogonal model: policy, wire/reset/closure state, compression workspace, immutable `TerminalFieldSectionLease`, terminal stage, and active block remain distinct. HPACK success atomically moves the sealed lease into `PendingSemantics`; no stage releases/replaces it or reparses compression state. Final valid completion transfers it to the unpublished message owner; malformed or peer-reset abort releases it once under redaction; connection-fatal failure transfers cleanup. Begun CANCEL bytes remain immutable. After local reset commit, late HEADERS/CONTINUATION still synchronize HPACK, and tolerated late DATA uses connection credit only.
- Define a total auditable `RejectedPushFrameDisposition` matrix keyed by policy disposition, RFC wire state, reset commit state/reason/reservation, closure cause, terminal state/stage, compression-workspace/field-section lease state and capacity, normalized event/END flags, header phase, and active block, with no wildcard/default cell. Every cell declares protocol legality and exact error code/scope, ordered wire transitions, HPACK/workspace/lease transfer or release, semantic action, application visibility, stream/connection credit, reset dormancy/re-arm/release/serialization, peer-reset abort, redaction, and connection-fatal cleanup owner; generated/property tests enumerate the same key space.
...
- Test client-originated PUSH_PROMISE connection failure and all prior authority/admission cases. Exhaustively cross wire/reset/output state, every terminal stage, compression/section capacity, dynamic-reference source, immediate eviction/table-size reduction, caller-buffer reuse, duplicates/empty fields, sensitivity/never-index metadata, peer reset, GOAWAY, fatal failure, and capacity at each pause. Prove exact bytes/order/provenance survive every stage; only compression workspace releases after HPACK; final valid transfers the lease; malformed/abort/fatal paths release or transfer it once without diagnostic exposure; one-byte shortage never truncates or publishes; and no block/stage/reset/lease is stranded, aliased, reused early, or completed twice.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Assign stable non-secret diagnostic codes to syntax, protocol scope/code, policy, capacity, cancellation, timeout, transport, and peer/local role; include bounded offsets/counters and generation identifiers, redact field values, credentials, nonces, compression contents, and section contents, and emit each event once. Distinguish local `AssemblyInvalidationCapacity`, `PushedAssemblyProvenanceCapacity`, `StreamTrackingUnavailable`, and `PushRejectionTrackingUnavailable`. Record rejection, wire/reset/closure state, terminal stage, compression-workspace state, field-section lease presence/generation and disposition (transferred/released/shutdown-owned) without raw names/values, deferred/final peer-reset abort, and actual wire emission separately. Preserve sensitivity/never-index redaction through valid, malformed, abort, and fatal cleanup; caller-owned sensitive storage retains the caller-scrub contract.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Acceptance contract: Maintain stateful fuzz targets for HTTP/1, HPACK, HTTP/2, Structured Fields, and translation with deterministic minimized regression replay. The HTTP/2 oracle spans reset/output × every terminal stage × compression-workspace and section-lease ownership/capacity × dynamic/static/literal field source × eviction/table-size update × caller-buffer reuse × duplicate/empty/sensitive/never-index fields × END flags × peer reset/GOAWAY/failure. It asserts byte/order/provenance preservation without encoded reparse, immutable generation binding, no stale table/input alias, no publication after abort/shortage, exact single lease transfer/release/shutdown ownership, dormant-reset arbitration, redacted diagnostics, and no stranded state or authority resurrection.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
