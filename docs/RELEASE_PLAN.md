...
- Store policy disposition, current RFC wire state, and local reset-output progress as independent generation-checked fields; policy rejection alone never closes a wire stream. A locally reset stream's tombstone retains reset origin/progress and field-block ownership through the closed-stream classification horizon, providing bounded storage for the v0.145.0 in-flight-push exception without retaining or recreating application authority.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Acceptance contract: Apply the idle/reserved/open/half-closed/closed/unknown stream matrix, allow only one active fragmented header block per connection, reject illegal interleaving, and handle stream-ID exhaustion deterministically. Represent rejection policy separately from `WireStreamState::{ReservedRemote, HalfClosedLocal, Closed}` and represent local reset output as `ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByPeer}`; every inbound frame first validates and commits its RFC wire-state transition, then policy decides publication. In particular, rejection while reserved(remote) still permits HEADERS and transitions to half-closed(local), rejects DATA as connection PROTOCOL_ERROR, treats reuse of a non-idle promised ID as connection PROTOCOL_ERROR, and lets peer RST_STREAM close the stream without pretending the local reset was committed.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone. Cover the Cartesian product of policy disposition, RFC wire state, reset progress, every legal/illegal frame, END_HEADERS fragmentation, and peer/local reset order; prove rejection never substitutes for a wire transition or changes required connection-versus-stream error scope.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- For a policy-rejected promised stream, DATA in reserved(remote) takes the v0.121.0 connection-PROTOCOL_ERROR path before discard accounting; DATA after legal response HEADERS moved the stream to half-closed(local) is discarded with ordinary stream and connection credit; DATA tolerated after local RST commit or on its closed-stream race path consumes and can restore connection-level credit only and must never emit stream WINDOW_UPDATE.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Acceptance contract: Emit at most one RST_STREAM, account discarded DATA credit, tolerate bounded in-flight frames, discard queued output after reset, resolve END_STREAM/GOAWAY/shutdown races, and recycle only after connection effects. Track reset intent separately from RFC wire state with `ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByPeer}`. A queued reset is withdrawable only while zero frame octets have committed: peer RST_STREAM then closes the stream and supersedes the command, while connection failure drops it with zero emitted CANCEL frames. The first committed reset octet makes that exact frame non-withdrawable; if the connection remains writable, finish only its remaining octets once even if peer RST_STREAM arrives, never enqueue a duplicate, and never resume it after connection failure. Commit closes the wire stream and retains its tombstone through late-frame classification.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone. Race peer RST_STREAM, connection failure, GOAWAY, cancellation, and every output split before byte zero, after every partial byte count, and after commit; require exactly one complete CANCEL when local serialization reaches commit, zero CANCEL bytes when peer reset or connection failure wins before serialization, and no duplicate, restart, or partial resume after a superseded reset or connection failure. A connection failure after partial output records only the committed prefix and never reports a complete wire frame.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Rejected promised streams use the v0.118.0 orthogonal model: `PolicyDisposition::Rejecting` does not replace `WireStreamState` or `ResetCommitState`. While wire state is reserved(remote) and CANCEL has not begun serialization, complete HEADERS/CONTINUATION through synchronization-only HPACK and transition to half-closed(local); reserved(remote) DATA and a duplicate promise of the same non-idle ID are connection PROTOCOL_ERROR. A peer RST_STREAM before the first local CANCEL octet closes the stream and supersedes the queued reset; after serialization begins, finish that one frame if the connection survives. After local reset commit, late HEADERS/CONTINUATION still synchronize HPACK, and tolerated late DATA uses connection credit only with no stream WINDOW_UPDATE. No path generically treats all post-rejection frames as closed traffic.
- Define a total auditable `RejectedPushFrameDisposition` matrix keyed by policy disposition, RFC wire state, reset commit state, inbound frame kind, and active field-block ownership, with no wildcard/default cell. Every cell declares protocol legality and exact error code/scope, wire transition, HPACK action, application visibility, stream/connection flow-credit action, and reset queue/serialization action; generated/property tests enumerate the same key space so implementation and oracle cannot silently omit a combination.
...
- Test client-originated PUSH_PROMISE connection failure; every method/content/trailer/field-section case; TLS, cleartext, proxy, stale/cross-connection/cross-generation and foreign authority evidence without v0.190.0; sender/receiver associated-state perspectives; promised-ID, GOAWAY, reserved-slot/work and tombstone/cutoff exhaustion, reservation at MAX_CONCURRENT_STREAMS zero, open-time concurrency rejection, cancellation/atomic rollback/in-place rejection conversion/slot reclamation, and all earlier behavior. Exhaustively cross rejection policy with reserved(remote)/half-closed(local)/closed wire states and queued/partially-serialized/committed/superseded reset status; prove legal HEADERS transitions and HPACK synchronization, reserved(remote) DATA and duplicate promised-ID connection errors, half-closed(local) discard credit, closed late-DATA connection-only credit, and exact zero/one CANCEL race outcomes. Reset the associated stream immediately before/during/after fragmented PUSH_PROMISE, recycle its request buffers, mutate HPACK, and vary peer reset, connection failure, GOAWAY, invalid promised IDs, promised-slot exhaustion, and mandatory-control backpressure; prove rejection-only handling never republishes authority and every record releases once after its field-block/classification horizon.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Maintain stateful fuzz targets for HTTP/1 framing/transitions, HPACK tables/blocks, HTTP/2 frames/streams/flow/flood, Structured Fields, and translation; seed RFC/adversarial corpora, assert progress/capacity/no-panic/state-isolation oracles, minimize crashes, and replay every regression deterministically at the milestone gate. The HTTP/2 oracle spans rejection disposition × wire state × reset serialization progress, associated-reset/PUSH_PROMISE ordering, fragmented CONTINUATION input, every output split, HPACK mutation, flow-credit scope, identifier reuse, GOAWAY, and capacity exhaustion; it asserts exact RFC error scope, zero/one reset emission, continuous tracking, and no authority resurrection.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
