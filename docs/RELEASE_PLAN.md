...
- Acceptance contract: Enforce independent ceilings for encoded header-block bytes, decoded bytes, field count, individual name/value length, Huffman decode work, dynamic-table memory, compression ratio, and unpublished output; classify peer compression violations separately from caller-capacity exhaustion and publish no partial field section when any limit wins.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Store policy disposition, current RFC wire state, local reset-output progress/reason/reservation, remote-closure cause, terminal-validation state/stage, and active field-block ownership as independent generation-checked fields; policy rejection, remote closure, field-block completion, or an intermediate semantic stage alone never releases another dimension. A zero-byte policy-CANCEL reservation can become dormant after remote END_STREAM without freeing its slot, so only final validation can release it or re-arm the same bounded slot as a mandatory stream-error reset without allocation. A locally reset stream's tombstone retains required state through the closed-stream classification horizon, providing bounded storage for the v0.145.0 in-flight-push exception without retaining or recreating application authority.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Test Generation-checked stream table and tombstones and all previously implemented relevant behavior with positive, negative, boundary, truncation, invalid-state, cancellation, capacity, and no-panic cases. Exhaust tombstone/cutoff capacity at every live/reserved transition; interleave peer frames and local terminal-output backpressure; vary policy, wire-state, and reset-progress changes independently; prove in-place conversion or typed bounded shutdown leaves no moment when an accepted ID is idle, and prove every slot/tombstone releases exactly once only after its classification and active-field-block horizons.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Implement idle, reserved-local, reserved-remote, open, both half-closed states, and closed with exhaustive frame/role transition tables; validate before mutation, map every illegal transition through the exact error-scope milestone, preserve unrelated streams and connection windows on stream error, and recycle a slot only after terminal connection effects and borrowed events complete. Normalize END_STREAM as a state-transition event separate from its carrying HEADERS or DATA event. Thus `HeadersStart { end_stream, end_headers }` can first transition reserved(remote) to half-closed(local) and then, when `end_stream` is true, immediately to closed; CONTINUATION never changes wire state, and active field-block ownership remains live independently until END_HEADERS or connection failure. Track terminal work separately as `TerminalValidation::{NotTerminal, PendingFieldBlock(PendingBlockDisposition), PendingSemantics(SemanticStage), Valid, Malformed(StreamErrorCode), AbortedByPeerReset}` with `PendingBlockDisposition::{ValidateSemantics, AbortAfterHpackByPeerReset}`. Define sealed monotonic `SemanticStage::{PseudoFields, ConnectionFields, FieldAndContext, RequestMapping, ResponseMapping, ContentAndPhase, TrailerAndRoleRules}` ownership across v0.125.0–v0.131.0; inapplicable role stages advance as checked no-ops, never by caller construction. Wire closure can precede validation and cannot decide reset-slot release.
- Permit only sealed/private transition constructors. `NotTerminal` cannot coexist with a dormant-after-END_STREAM reset, `Valid`/`Malformed`/`AbortedByPeerReset` cannot retain semantic-stage or fragmented-block ownership, and `PendingSemantics` cannot retain HPACK field-block ownership. Each stage uses pre-reserved bounded work/storage; resumable local capacity pressure leaves the same pending stage and dormant reservation intact, while inability to retain mandatory state transfers all cleanup exactly once to connection-owned shutdown. Connection-fatal teardown is connection-owned rather than a stream enum variant.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Test Exhaustive stream-state graph and all previously implemented relevant behavior with positive, negative, boundary, truncation, invalid-state, cancellation, capacity, and no-panic cases. Cross HEADERS/DATA frame events with END_STREAM present/absent and HEADERS with END_HEADERS present/absent; prove two-step HEADERS+END_STREAM transitions, closed wire state with a still-active CONTINUATION obligation, and no wire-state transition from CONTINUATION itself. Compile-fail or model-check every forbidden terminal-validation/field-block/reset combination and every non-monotonic or caller-forged semantic-stage transition.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Apply the idle/reserved/open/half-closed/closed/unknown stream matrix, allow only one active fragmented header block per connection, reject illegal interleaving, and handle stream-ID exhaustion deterministically. Represent rejection policy separately from `WireStreamState::{ReservedRemote, HalfClosedLocal, Closed}`, local reset output as `ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByPeerReset, PolicyDormantAfterEndStream}`, reset purpose as `ResetReason::{PolicyCancel, StreamError(StreamErrorCode)}`, remote closure as `RemoteClosureCause::{Reset, EndStream}`, terminal validation, and field-block ownership. Normalize `HeadersStart { end_stream, end_headers }`, `Data { end_stream }`, `Continuation { end_headers }`, and `PeerReset` before table lookup; process the carrying frame event and then its END_STREAM event in order. Rejection while reserved(remote) still permits HEADERS, rejects DATA as connection PROTOCOL_ERROR, and treats reuse of a non-idle promised ID as connection PROTOCOL_ERROR; remote END_STREAM closes wire state but only makes an uncommitted policy action dormant until terminal validation, whereas peer reset can supersede it.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone. Cover the Cartesian product of policy disposition, RFC wire state, reset progress/reason, remote-closure cause, terminal-validation state/semantic stage, normalized frame event, END_STREAM, END_HEADERS, field-block ownership, and peer/local reset order; prove rejection never substitutes for a wire transition, closure never strands CONTINUATION or releases a dormant reset early, and flags never change required connection-versus-stream error scope.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Define one deterministic error-action precedence: connection-fatal compression/framing failure commits bounded connection shutdown and prevents any new stream reset; an uncommitted `StreamError(code)` replaces/re-arms a dormant or queued `PolicyCancel` in the same reserved slot; partially serialized or committed RST_STREAM bytes are immutable and suppress every later duplicate; and peer reset supersedes an unsent stream action. Peer reset changes pending semantics to `AbortedByPeerReset`, or records deferred abort while an active terminal field block finishes mandatory HPACK synchronization; neither path publishes semantics or application events. Wire/HPACK transitions never roll back when the chosen error action changes.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Wire closure by HEADERS+END_STREAM, DATA+END_STREAM, or peer RST_STREAM cannot release an active field-block owner. In particular, HEADERS with END_STREAM but without END_HEADERS records closed wire state immediately yet decodes every required CONTINUATION and commits valid HPACK mutations before releasing synchronization state; application publication remains independently forbidden where policy rejected the stream.
- HEADERS+END_STREAM without END_HEADERS sets `PendingFieldBlock(ValidateSemantics)`. Successful HPACK completion transfers ownership exactly once to `PendingSemantics(SemanticStage::PseudoFields)`, never directly to `Valid`; HPACK field-block storage is then releasable while the dormant reset reservation remains held through all applicable semantic stages. Malformed HPACK commits connection-fatal shutdown and transfers cleanup to that lifecycle. Peer reset during fragmentation changes only the disposition to `AbortAfterHpackByPeerReset`; completion then releases block storage, enters `AbortedByPeerReset` instead of semantics, and publishes no semantic/application event.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Own `SemanticStage::PseudoFields`: success advances a terminal field block only to `PendingSemantics(ConnectionFields)`; a forbidden/duplicate/unknown/misordered pseudo-field records terminal `Malformed(PROTOCOL_ERROR)` and consumes the retained mandatory reset reservation. It never becomes `Valid` merely because HPACK or peer END_STREAM completed first.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Own `SemanticStage::ConnectionFields`: success advances only to `PendingSemantics(FieldAndContext)`; failure records `Malformed(PROTOCOL_ERROR)` without releasing the dormant reset reservation first. Peer-reset abort and connection-owned teardown remain available at the boundary before and after this stage.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Own `SemanticStage::FieldAndContext`: success advances only to `PendingSemantics(RequestMapping)`; if the block carried END_STREAM, every malformed-message result becomes terminal `Malformed(PROTOCOL_ERROR)` and re-arms the dormant reset slot as `StreamError(PROTOCOL_ERROR)` unless immutable reset bytes, peer-reset abort, or a connection-fatal decision already won. Remote completion is not a reason to suppress the required stream error.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Own `SemanticStage::ResponseMapping`: success, including the sealed checked-no-op for request sections, advances only to `PendingSemantics(ContentAndPhase)`. Missing, duplicate, malformed, or context-invalid `:status` on terminal HEADERS records stream `Malformed(PROTOCOL_ERROR)` after HPACK synchronization and re-arms the retained mandatory reset reservation; no response or terminal-valid event is published.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Own the final HEADERS stage `SemanticStage::TrailerAndRoleRules`. Classify initial, informational, final, and trailer HEADERS independently from END_STREAM/END_HEADERS. A HEADERS END_STREAM event can close the wire stream before its fragmented block completes, but the block owner, terminal stage, dormant reset reservation, and HPACK obligation remain. Any informational 1xx HEADERS carrying END_STREAM is terminal `Malformed(PROTOCOL_ERROR)` and requires stream RST_STREAM(PROTOCOL_ERROR); only successful completion of every applicable prior stage plus trailer/role rules may mark terminal `Valid` and release the dormant slot. Forbidden trailer pseudo-fields or other malformed terminal fields re-arm it. Trailer HEADERS require END_STREAM and close exactly once.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Test ordinary, PendingConnect, AwaitingConnectOutcome, capacity-reset, padding, END_STREAM, and receive-underflow stream-window accounting; create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Test ordinary and pending CONNECT DATA across connection-window boundary, stream reset, non-2xx, and END_STREAM without skipped/double accounting; create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- For a policy-rejected promised stream, DATA in reserved(remote) takes the v0.121.0 connection-PROTOCOL_ERROR path before discard accounting; DATA after legal response HEADERS moved the stream to half-closed(local) is discarded with ordinary stream and connection credit. DATA+END_STREAM first charges/discards its full payload and padding, releases each stream/connection credit exactly once, then records remote closure. DATA tolerated after local RST commit or on its closed-stream race path consumes and can restore connection-level credit only and must never emit stream WINDOW_UPDATE.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Acceptance contract: Emit at most one RST_STREAM, account discarded DATA credit, tolerate bounded in-flight frames, discard queued output after reset, resolve END_STREAM/GOAWAY/shutdown races, and recycle only after connection effects. Track reset output as `ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByPeerReset, PolicyDormantAfterEndStream}`, reason as `ResetReason::{PolicyCancel, StreamError(StreamErrorCode)}`, and terminal state/stage independently. Peer RST_STREAM supersedes a zero-byte action and changes `PendingSemantics(_)` to `AbortedByPeerReset`; if `PendingFieldBlock`, it records deferred abort, completes HPACK synchronization, then enters `AbortedByPeerReset` without semantic or application publication. Remote END_STREAM instead makes only an uncommitted policy CANCEL dormant and retains its slot through every semantic stage. Only final `Valid` releases it; `Malformed(code)` re-arms it without allocation. Begun reset bytes remain immutable and preclude a second reset. GOAWAY alone neither aborts validation nor releases the reservation. Connection-fatal HPACK/framing failure transfers pending block/stage/reset cleanup exactly once to bounded connection shutdown. Field-block ownership, semantic ownership, wire closure, and reset action remain independent through the terminal decision.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone. Pause after every semantic stage and race valid/malformed terminal HEADERS/DATA, peer RST_STREAM, remote END_STREAM, HPACK failure, connection failure, GOAWAY, capacity exhaustion, cancellation, and every policy/error reset output offset. Require the dormant reservation to remain non-releasable/non-reusable at every intermediate stage; peer reset to supersede once while fragmented HPACK still drains before abort; zero CANCEL only when final valid completion wins before byte zero; exactly one re-armed RST_STREAM when malformed validation wins; exactly one immutable begun reset; bounded connection-owned cleanup on fatal failure; no publication after abort; no duplicate re-arm/release/restart; and no stranded block/stage. A connection failure after partial output records only the committed prefix and never reports a complete wire frame.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Rejected promised streams use the v0.118.0 orthogonal model: `PolicyDisposition::Rejecting` does not replace `WireStreamState`, reset state/reason/reservation, `RemoteClosureCause`, `TerminalValidation`, or active field-block ownership. `HeadersStart { end_stream: false, .. }` moves reserved(remote) to half-closed(local); with `end_stream: true` it then immediately closes, while `end_headers: false` retains CONTINUATION/HPACK as `PendingFieldBlock`. HPACK success enters `PendingSemantics(PseudoFields)`, and no intermediate semantic stage releases the dormant policy-CANCEL slot. Final valid completion releases it; malformed message re-arms it; peer reset supersedes the action and aborts after any mandatory HPACK drain; connection-fatal HPACK transfers cleanup to shutdown. Begun CANCEL bytes remain immutable and preclude a second reset. After local reset commit, late HEADERS/CONTINUATION still synchronize HPACK, and tolerated late DATA uses connection credit only.
- Define a total auditable `RejectedPushFrameDisposition` matrix keyed by policy disposition, RFC wire state, reset commit state/reason/reservation, remote-closure cause, terminal-validation state and `SemanticStage`, normalized inbound event including END_STREAM/END_HEADERS, header-section phase, and active field-block ownership, with no wildcard/default cell. Every cell declares protocol legality and exact error code/scope, ordered wire transitions, HPACK/semantic-stage action, application visibility, stream/connection flow-credit action, reset dormancy/re-arm/release/serialization action, peer-reset abort/deferred-abort action, and connection-fatal cleanup owner; generated/property tests enumerate the same key space so implementation and oracle cannot silently omit a combination.
- Implement RFC 9113's in-flight PUSH_PROMISE exception for an associated stream with a locally initiated reset. Retain its generation-checked tombstone from reset intent through the closed-stream classification horizon and any active field block; accept and completely decode PUSH_PROMISE plus every CONTINUATION in either ordering around the associated reset's wire commit, preserving valid HPACK changes. Validate and continuously track the promised ID, publish no promised request, and recreate no application, authority, push-policy, invalidation, or assembly provenance. Missing recycled associated-request provenance selects safe promised-stream rejection through the same pending-CANCEL state machine, not connection PROTOCOL_ERROR merely because the associated stream was locally reset. Illegal promised IDs remain connection PROTOCOL_ERROR and malformed HPACK remains connection-fatal; capacity failure retains bounded tracking or commits the typed shutdown path.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Test client-originated PUSH_PROMISE connection failure; every method/content/trailer/field-section case; TLS, cleartext, proxy, stale/cross-connection/cross-generation and foreign authority evidence without v0.190.0; sender/receiver associated-state perspectives; promised-ID, GOAWAY, reserved-slot/work and tombstone/cutoff exhaustion, reservation at MAX_CONCURRENT_STREAMS zero, open-time concurrency rejection, cancellation/atomic rollback/in-place rejection conversion/slot reclamation, and all earlier behavior. Exhaustively cross wire state, reset state/reason/output offset, every terminal state/stage, closure cause, normalized events, END flags, message phase, validity, HPACK, credit, peer reset, GOAWAY, connection failure, and capacity at each pause. Prove only final valid completion releases a dormant reset; malformed completion re-arms once; peer reset aborts without publication after required HPACK drain; begun reset remains sole and immutable; fatal failure transfers cleanup boundedly; and no block/stage/reset slot is stranded, reused early, or completed twice. Reset the associated stream around fragmented PUSH_PROMISE, recycle buffers, and vary mandatory-control backpressure; prove rejection-only handling never republishes authority and every record releases once after its validation/classification horizon.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Replay every RFC 9113 frame, stream-state, continuation, flow-control, SETTINGS, GOAWAY, push, priority, HPACK, malformed-field, flood, and shutdown vector for both roles; require exact connection/stream error code, state delta, publication trace, control output, work charge, and interop transcript before the HTTP/2 pentest exit.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Extend the v0.145.0 receiving-client PUSH_PROMISE transaction after the complete promised field block and all CONTINUATION input have decoded, HPACK is synchronized, and promised-request semantics, authority, safe/cacheable method, and caller push policy pass, but before promised-request publication. Atomically preflight/commit the existing promised-stream slot, matching `AssemblyInvalidationHandle`, independent pushed-provenance storage, and mandatory rejection-tombstone/cutoff tracking. If handle or provenance capacity is unavailable, retain the provisional slot, set only `PolicyDisposition::Rejecting`, leave its RFC wire state reserved(remote), queue `ResetReason::PolicyCancel`, and publish no promised request or assembly correlation. The ID is never momentarily untracked. Apply the exact v0.145.0 terminal pipeline: END_STREAM closes wire state, but an uncommitted policy reset remains dormant through `PendingFieldBlock` and every `PendingSemantics(stage)`. Only final valid terminal HEADERS/DATA releases it; malformed status/fields/trailers/Content-Length/body phase re-arms it; peer reset aborts publication after mandatory HPACK synchronization; fatal failure transfers cleanup to bounded connection shutdown. Begun reset bytes remain immutable. Reserved(remote) DATA and duplicate promised ID remain connection PROTOCOL_ERROR, and post-reset late DATA restores connection credit only. If mandatory tracking cannot be represented, retain the provisional slot and initiate typed bounded `PushRejectionTrackingUnavailable` shutdown. Every provisional slot, handle, provenance lease, reset reservation/command, terminal state/stage, field-block owner, tombstone, and shutdown record releases exactly once at its terminal horizon.
...
- Acceptance contract: Assign stable non-secret diagnostic codes to syntax, protocol scope/code, policy, capacity, cancellation, timeout, transport, and peer/local role; include bounded offsets/counters and generation identifiers, redact field values, credentials, nonces, and compression contents, and emit each security event once at its committed disposition. Distinguish local `AssemblyInvalidationCapacity`, `PushedAssemblyProvenanceCapacity`, `StreamTrackingUnavailable`, and `PushRejectionTrackingUnavailable`; the latter two report bounded connection shutdown without provenance. Record rejection, wire state, reset commit status/reason, reset-slot dormancy, terminal state plus bounded semantic-stage identifier, active field-block status, deferred peer-reset abort, final `AbortedByPeerReset`, and `RemoteClosureCause::{Reset, EndStream}` separately. Distinguish policy CANCEL released only after final valid completion, re-armed as stream error, immutable reset already begun, protocol reset already committed, connection-owned cleanup transfer, and connection failure; never report a frame that did not reach the wire or expose field contents.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Acceptance contract: Maintain stateful fuzz targets for HTTP/1 framing/transitions, HPACK tables/blocks, HTTP/2 frames/streams/flow/flood, Structured Fields, and translation; seed RFC/adversarial corpora, assert progress/capacity/no-panic/state-isolation oracles, minimize crashes, and replay every regression deterministically at the milestone gate. The HTTP/2 oracle spans rejection × wire state × reset state/reason/output offset × every terminal state/semantic stage × closure cause, associated-reset/PUSH_PROMISE ordering, HEADERS/DATA/CONTINUATION with END flags, message phase/validity, credit, HPACK, peer reset, GOAWAY, connection failure, and capacity injected after every stage. It asserts sealed monotonic transitions, no publication after abort, exact wire/HPACK/credit state, dormant-slot retention then single release/re-arm, deferred peer-reset HPACK drain, bounded connection-owned cleanup, no stranded block/stage, zero/one stream reset, continuous tracking, and no authority resurrection.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
