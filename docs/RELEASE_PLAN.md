...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete v0.56.0 `Connection` corpus under HTTP/1.0 and HTTP/1.1; require identical lexical decisions and exact version bindings but version-specific outcomes. Exhaust every v0.74.0 matrix cell and two-sided negotiation order: missing/uncommitted local signal, missing/invalid/stale received signal, local request/response commitment at every output offset, lifecycle incomplete/complete, policy/framing/budget loss at every output phase, newest-message invalidation, and forged/stale/cross-role/direction/hop/version/connection/exchange/head/generation `ValidatedHttp10KeepAlive`, `CommittedHttp10KeepAliveHead`, and `Http10ReusePermit`. Require the exact client, origin/server, and proxy-upstream pairings; prove proxy hops cannot forward or substitute authority, reuse appears only after both messages complete, a permit is consumed once, and no path pipelines. Cross rejected HTTP/1.0 CONNECT with every matrix/negotiation outcome and v0.73.0 error/action state; prove no reuse permit, tunnel, application publication, authority resolution, forwarding, successor parse, optimistic-input lease, HTTP/1.1 close proof, or Upgrade authority is created.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  `Http10ReusePermit`, output-phase persistence-loss action, and either-version
  ...
  keep-alive signal or the reuse permit; turn `ValidatedHttp10KeepAlive` into
  ...
  request persistence, forward received keep-alive authority, defer close to an
  illegal successor, mutate Frozen/HeadCommitted output, or enable HTTP/1.0
...
- Record early-final request generation, framing kind, disposition, bounded
  ...
  generation, local-head commitment, pair/hop match, reuse-permit state,
  ...
  local-capacity/414/431/400/501 outcome,
...
- Fuzz CONNECT 2xx, Upgrade/WebSocket 101, and the v0.163.0/v0.187.0/v0.188.0
  ...
  all two-signal orders and output commitment offsets, per-hop non-forwarding,
  ...
  `UnsupportedHttp10ConnectDisposition` through local capacity, target/work
  414, field/work 431, syntax/framing/content 400, and valid CONNECT 501
  precedence, request-plus-optimistic-bytes, all persistence matrix variants,
...
- Reject caller construction, copying, rebinding, or direct mutation of
  ...
  CommittedHttp10KeepAliveHead, or Http10ReusePermit,
  ...
  reuse before both message lifecycles complete; close deferred to a successor;
  ...
  wrong local-capacity/414/431/400/501 precedence; application-visible
...
- Independently size each configured early-final request's framing/progress/
  ...
  `Http10PersistenceDisposition`/`ValidatedHttp10KeepAlive`/
  `CommittedHttp10KeepAliveHead`/`Http10ReusePermit`, newest-message
  revocation, per-hop signal pairing, output-phase persistence-loss action, and
