...
| Outbound field-block completion and HPACK commit | provisional transaction semantics at `0.98.0`; encoder SETTINGS arbitration at `0.124.0`; HEADERS preflight/ownership at `0.137.0`; reset/scheduler at `0.138.0`/`0.142.0`; PUSH_PROMISE at `0.145.0`; reserved controls at `0.153.0`; audit/diagnostics/fuzz/compile-fail at `0.154.0`/`0.193.0`/`0.196.0`/`0.197.0`; sizing at `0.211.0` | Before exposure, reserve exact slots/entries using the minimum peer/local/slot cap and checked block/continuation bounds. Private rollback discards provisional HPACK; FramingCommitted drains; final acknowledgement publishes HpackCommitted. A peer table-size transition remains AppliedAwaitingAckCommit and blocks later block encoding/exposure until every owning nine-byte SETTINGS ACK is fully committed. |
...
| HEADER_TABLE_SIZE transaction arbitration | participant and three-state transition at `0.124.0`; scheduler/ACK-commit barrier at `0.139.0`/`0.142.0`/`0.153.0`; audit/diagnostics/fuzz/compile-fail at `0.154.0`/`0.193.0`/`0.196.0`/`0.197.0`; resource replay at `0.211.0` | AwaitingSafeApply collapses smallest/final maxima; AppliedAwaitingAckCommit retains every SETTINGS owner but no ACK authority; WireEnabled follows only full FIFO commitment of all owning ACKs. Private re-encode and every dependent HEADERS/PUSH_PROMISE exposure wait for WireEnabled. |
...
| `0.98.0` | HPACK encoder output commit and indexing policy | Requires Sensitive-field indexing policy; defines a serialized provisional field-block transaction, whole-block HpackCommitted publication, and rollback/connection-discard semantics distinct from later framing commitment; Unlocks Independent HPACK decode limits. |
...
| `0.124.0` | SETTINGS header-table encoder and header-list policy coupling | Requires Atomic HPACK header-block integration; defines AwaitingSafeApply, AppliedAwaitingAckCommit, and WireEnabled table-size states with smallest/final values and owner references but no ACK authority; re-encode and dependent block exposure wait for every owning ACK commit; Unlocks Pseudo-field ordering and uniqueness. |
...
| `0.137.0` | HTTP/2 outbound per-stream message command lifecycle | Requires inbound ownership plus both send ledgers; pre-materializes field blocks at checked `min(16_384, local cap, slot payload)`, enforces mandatory-prefix/block-byte/continuation bounds, separates HPACK states, and checks DATA arithmetic; Unlocks HTTP/2 body cancellation, reset, and flow-credit lifecycle. |
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; rolls back a whole Private block/HPACK transaction or drains a pre-reserved FramingCommitted block before reset, with final acknowledgement publishing HpackCommitted; Unlocks SETTINGS outstanding-ACK accounting. |
| `0.139.0` | SETTINGS outstanding-ACK accounting | Requires HTTP/2 body cancellation, reset, and flow-credit lifecycle; integrates frame-wide ordered participants with WaitingParticipants/AckEligible/AckFrozen 0..=8/AckCommitted states, FIFO wire commitment, retained dependents, and fatal/partial-output abandonment; Unlocks Bounded stream admission. |
...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; serializes HPACK/table transitions and all-effective FIFO ACK suffixes, then permits Private re-encode or dependent HEADERS/PUSH_PROMISE only after every owner reaches AckCommitted; Unlocks SETTINGS max-frame-size outbound integration. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.142.0 foundations; attaches ENABLE_PUSH to the shared SETTINGS transaction and retains exact PUSH_PROMISE prefix/continuation preflight; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.153.0` | Reserved control-output queues | Requires WINDOW_UPDATE churn defenses; freezes one nine-byte ACK per validated frame, retains its transaction/dependents through acknowledged offsets 0..=8, commits only at byte nine in FIFO order, and abandons the connection on partial-output failure; Unlocks HTTP/2 conformance audit and pentest. |
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits combined SETTINGS participant/ACK states, every ACK offset, multi-owner table-size barriers, stale tokens, backpressure/failure, and prior field-block/arithmetic/credit contracts; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.191.0` | Fixed-capacity caller-storage public API | Requires Authenticated origin authorization and HTTP/2 coalescing metadata; keeps SETTINGS transaction/ACK/participant and AckCommitted/WireEnabled promotion authority sealed while validating reserves and one-record APIs without allocator; Unlocks Optional alloc-backed convenience API. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports redacted SETTINGS disposition, ACK offset/commit, retained owners, HPACK transition, field layout, token, arena, and credit without misattributed ACK ownership; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts combined SETTINGS, ACK offsets 0..=9, FIFO/multi-owner barriers, transition collapse, backpressure/failure, stale tokens, field bounds, arena, and credit; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects transaction/ACK construction or extraction, early AckCommitted/WireEnabled promotion, owner release/rebind, participant reordering, and prior preflight/token/reuse violations; Unlocks Long-running soak and exhaustion campaign. |
...
| `0.211.0` | Deterministic CPU, stack, code-size, and amplification budgets | Requires Aesynx kernel integration tests; sizes transactions, participants, frozen nine-byte ACK cursors/tokens, retained multi-owner transition sets, FIFO prefixes, and fatal cleanup separately from field/HPACK/arena bounds; Unlocks 32-bit target campaign. |
