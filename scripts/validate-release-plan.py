...
def main() -> int:
    ...
    failures = [failure for failure in failures if not failure.startswith(("missing HTTP/1.0 Completing/work/publication contract:", "missing HTTP/1.0 phase/atomic-charge/initial-slot contract:"))]; required_http10_interrupt_work_role_contracts = ("`AdmissionWorkKind::{CommandShape, Method, Target, Field, Framing, Semantics, CloseInjection, OutputPreflight, ReservationPreflight}`", "Callers cannot construct a kind, select a numeric cost, or underprice work", "`try_charge_admission_work(kind) -> Result<AdmissionWorkCharge, AdmissionWorkChargeError>`", "`AdmissionWorkChargeError::{PermitBudgetExhausted, ConnectionBudgetExhausted, InvalidZeroCost, GenerationMismatch, LedgerInvariantFailure, ArithmeticOverflow}`", "invalid/zero kind cost, generation mismatch, broken ledger equation, arithmetic overflow, permit exhaustion, then connection exhaustion", "first two exhaustion variants are ordinary local resource closure", "zero cost, mismatch, invariant failure, and overflow are local security-invariant closure", "Every error preserves both ledgers, returns no charge record, and performs no governed work", "`AdmissionWorkCharge { kind, permit_generation, connection_generation, cost, permit_consumed_after, connection_consumed_after }`", "`Http10InitialExchangeReservation::{Client { request: PendingHttp10InitialRequest, private_output, resources }, Server { parser, resources }}`", "Client retains no command borrow, accepts/exposes no request byte, leaves private output unchanged", "Server leaves input wholly unconsumed", "publishes no connection/exchange generation", "release every tentative record, lease, count/work reservation, event slot, and connection-ledger owner exactly once", "`Http10CompletionInterrupt::{IdleDeadlineReached { connection_generation, permit_generation, observed_now }, PolicyRevoked { policy_generation }, TransportFailure { transport_generation, cause }, LocalClose, Cancellation}`", "`Http10CompletionFirstCause` latch", "first valid accepted interrupt wins permanently", "TransportFailure, PolicyRevoked, IdleDeadlineReached, LocalClose, then Cancellation", "Wrong/stale generation or deadline identity is state-neutral", "exact `observed_now >= expires_at` expires authority", "Interrupt handling is total and capacity-infallible for every variant in every phase", "In Resolving, skip/terminate reuse resolution and select close", "In DecisionHeld, revoke any provisional permit", "In Reclaiming, retain receipts, revoke provisional reuse, and continue only unreclaimed items", "In PublicationPending, rewrite the reserved event/next state in place", "expiry or policy revocation can never leave reusable authority", "no expired/revoked Reusable publication", "prohibit caller-selected costs", "both initial reservation roles at one-short/exact component capacity"); failures.extend(f"missing HTTP/1.0 interrupt/work-error/initial-role contract: {text}" for text in required_http10_interrupt_work_role_contracts if text not in detailed)
    if failures:
