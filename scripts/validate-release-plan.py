...
def main() -> int:
    ...
    required_contract_text = (
        ...
        "separate bounded reserved-push-slot/work budget before reservation", "preflight tombstone and cutoff capacity before releasing its slot or convert that slot in place", "continuously represented as a generation-bound slot, tombstone, or connection-shutdown record", "Store policy disposition, current RFC wire state, and local reset-output progress as independent generation-checked fields", "Represent rejection policy separately from `WireStreamState::{ReservedRemote, HalfClosedLocal, Closed}`", "`ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByPeer}`", "A queued reset is withdrawable only while zero frame octets have committed", "first committed reset octet makes that exact frame non-withdrawable", "typed bounded `StreamTrackingUnavailable` connection shutdown", "validated promised request, trusted authority/push-policy provenance, and reserved slot unpublished and rollback-capable", "promised slot includes or pre-reserves its v0.117.0 in-place rejection-tombstone/cutoff representation", "At v0.145.0 the transaction's participants are the promised-stream slot/work and rejection tracking", "Rejected promised streams use the v0.118.0 orthogonal model", "total auditable `RejectedPushFrameDisposition` matrix", "with no wildcard/default cell", "Every cell declares protocol legality and exact error code/scope, wire transition, HPACK action, application visibility, stream/connection flow-credit action, and reset queue/serialization action", "Implement RFC 9113's in-flight PUSH_PROMISE exception for an associated stream with a locally initiated reset", "accept and completely decode PUSH_PROMISE plus every CONTINUATION", "publish no promised request, and recreate no application, authority, push-policy, invalidation, or assembly provenance", "Missing recycled associated-request provenance selects safe promised-stream rejection", "Illegal promised IDs remain connection PROTOCOL_ERROR and malformed HPACK remains connection-fatal", "No v0.145.0-only pushed response has partial-retention or cross-request assembly authority",
