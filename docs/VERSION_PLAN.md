...
| Never-untracked HTTP/2 rejection state | storage at `0.117.0`; state matrix at `0.118.0`; push integration at `0.145.0`; assembly transaction at `0.181.0`; stable API at `0.191.0`; diagnostics at `0.193.0` | Preflight tombstone/cutoff capacity or retain the live/reserved slot in place; an accepted ID is always tracked. Policy rejection never substitutes for RFC wire state or reset-output progress. If tracking cannot be represented, emit typed `StreamTrackingUnavailable`/`PushRejectionTrackingUnavailable` bounded connection shutdown without forgetting the ID. |
| Rejected-push frame and CANCEL race matrix | model at `0.118.0`; error scope at `0.121.0`; credit at `0.136.0`; reset serialization at `0.138.0`; total push table at `0.145.0`; assembly use at `0.181.0`; diagnostics/fuzz at `0.193.0`/`0.196.0` | Track rejecting policy × reserved(remote)/half-closed(local)/closed wire state × queued/partially serialized/committed/superseded reset. A total no-default table fixes transition, HPACK, credit, error, publication, and reset action for every frame. HEADERS synchronizes and transitions; reserved(remote) DATA and promised-ID reuse are connection PROTOCOL_ERROR; peer reset suppresses only a zero-byte queued CANCEL; partially serialized reset finishes once. |
...
| HTTP/2 DATA and command ownership | `0.136.0`–`0.138.0` | Application acknowledgement, not parsing, releases inbound credit; rejected DATA distinguishes reserved(remote), half-closed(local), and post-reset connection-only credit; outbound commands and partial reset output use one generation-safe lifecycle. |
...
| `0.117.0` | Generation-checked stream table and tombstones | Requires HTTP/2 stream-identifier rules; retains independent policy/wire/reset fields plus locally reset stream tombstones through classification/field-block horizons so accepted IDs never become idle; Unlocks Exhaustive stream-state graph. |
| `0.118.0` | Exhaustive stream-state graph | Requires Generation-checked stream table and tombstones; defines the policy disposition × RFC wire state × reset-progress frame matrix without allowing rejection to fake closure; Unlocks HTTP/2 activation preface, first-SETTINGS, and deadline sequencing. |
...
| `0.136.0` | HTTP/2 inbound DATA ownership, acknowledgement, and credit release | Requires SETTINGS initial-window active-stream integration and atomic rollback; separates rejected reserved(remote) protocol error, half-closed(local) discard credit, and post-reset connection-only credit/no stream WINDOW_UPDATE; Unlocks HTTP/2 outbound per-stream message command lifecycle. |
...
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; defines queued/partially-serialized/committed/superseded reset races with zero-or-one wire emission and no duplicate/resume; Unlocks SETTINGS outstanding-ACK accounting. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.138.0 state/reset foundations; integrates the exact rejected-push frame matrix and rejection-only in-flight PUSH_PROMISE handling after local associated-stream reset, then creates rollback-capable continuously tracked admission for v0.181.0; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports rejection disposition, wire state, reset commit/supersession, and actual zero/one frame emission independently; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts rejected-push state/reset products and associated-reset/PUSH_PROMISE orderings with HPACK, credit, GOAWAY, and capacity oracles; Unlocks Compile-fail state and lifetime tests. |
