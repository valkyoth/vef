...
retention, and binds optional cross-request assembly to narrow reserved
...
| Bounded full-200 fallback invalidation | reserve at `0.181.0`; evidence at `0.181.1`; consume at `0.181.2` | Reserve a non-forgeable target/principal/partition/navigation handle from engine-only capacity at correlation admission. Keyed replacement stays exact; unkeyed replacement invalidates all Vary/validator siblings in that namespace and widens only across coding/domain children when refinement is unavailable. Peer-controlled Vary/identity/normalization exhaustion cannot select whole-arena rotation; only mandatory-reserve failure, internal corruption, or explicit caller policy can, with principal/tenant sharding bounding its blast radius. |
...
| `0.181.0` | Client correlation, cancellation, and retry tokens | Requires Partial-response media-type classification integration; reserves the mandatory shard-bound invalidation namespace; Unlocks Streaming partial-response and retained-prefix validation. |
| `0.181.1` | Streaming partial-response and retained-prefix validation | Requires the correlation and reserved invalidation handle; seals keyed/unkeyed fallback evidence and canonical-only normalized identity storage; Unlocks Cross-request partial assembly and header synthesis. |
| `0.181.2` | Cross-request partial assembly and header synthesis | Requires Streaming partial-response and retained-prefix validation; consumes fallback evidence with namespace-bounded invalidation and borrow-aware reclamation; Unlocks Retry safety, idempotency, and body-replayability contract. |
