...
- Atomic `try_charge_admission_work<'attempt, 'command>(&'attempt mut AdmissionAttempt<'command>, spec) -> Result<AdmissionWorkPermit<'attempt, 'command>, AdmissionWorkChargeError>` retains errors `PermitBudgetExhausted`, `ConnectionBudgetExhausted`, `InvalidZeroCost`, `GenerationMismatch`, `LedgerInvariantFailure`, and `ArithmeticOverflow` with existing precedence/classification. It checks unit bounds and multiplication/addition before both ledgers, then mutates both or neither. Every error performs no governed work. Private non-Copy/non-Clone `AdmissionWorkPermit<'attempt, 'command> { attempt: PhantomData<&'attempt mut AdmissionAttempt<'command>>, kind, charged_units, cost, permit_generation, connection_generation, permit_consumed_after, connection_consumed_after, attempt_generation }` is a structurally attempt-borrowed internal authorization capability, not protocol authority or diagnostics. The exclusive lifetime prevents it from outliving/crossing the attempt or coexisting with a wrong mutable attempt; numeric `attempt_generation` remains defense-in-depth and diagnostic evidence.
- Consuming that permit moves its exclusive attempt borrow into private `ChargedWorkWindow<'attempt, 'command, T>`, bound to the exact attempt cursor and exposing no more than `charged_units`. Byte/entry windows yield only their bounded slice/iterator prefix and derive progress from an internal cursor. Semantic, framing, output, and reservation windows expose only `take_unit()`, which must succeed before each governed operation and decrements the remaining units. No governed implementation receives the original unbounded iterator/slice while the window is active. `finish(self) -> AdmissionWorkReceipt` consumes the window and derives processed units internally; it accepts no caller-reported count, advances the bound attempt cursor from `cursor_before` to checked `cursor_after` by that internal progress, and cannot exceed the charged bound. Early malformed input still finishes the window. Unused charged units remain consumed but grant no later work.
- The non-authoritative diagnostic is `AdmissionWorkReceipt { attempt_generation, cursor_kind, cursor_before, cursor_after, kind, charged_units, processed_units, cost, permit_generation, connection_generation, permit_consumed_after, connection_consumed_after }`, where checked `processed_units == cursor_after - cursor_before <= charged_units`. It retains no borrow, cursor, window, permit, command contents, or work authority. A stale, cross-attempt, duplicated, or reused permit/window cannot inspect data, take a unit, finish, advance a cursor, or produce a receipt. Successful admission transfers ledger totals without recharge; receipts never authorize work or state transitions.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz/Kani target now and retain its minimized corpus. Test attempt generation at maximum-minus-one, maximum, and exhaustion; require exhaustion before inspection with both work ledgers unchanged and no attempt/permit/window/receipt. For every work kind, charge zero, one, and maximum units; consume every `0..=n` prefix after charging `n`; test byte/entry windows, every `take_unit()` boundary, maximum method/target/field bytes and field count, one-byte fragmentation, malformed/capacity retry, and checked arithmetic. Prove lifetimes reject permit escape, wrong-attempt mutable coexistence, and cross-attempt transfer; bounded windows make processing above the charge inexpressible; `finish()` derives actual progress internally; receipts contain exact attempt generation/cursor kind/before/after; and unused units confer no authority or refund. Use instrumented iterators and attempt direct access beyond each window. On every `RejectedLocalCommand` and `RetryableCapacity`, mutate every command byte before retry and prove the old borrow/cursors/permit/window cannot skip validation; new cursors start at zero and all repeated work is charged again against unchanged/non-resetting ledgers. Require cumulative ceilings and no quadratic/unpriced traversal.
...
- Acceptance extension: Audit v0.75.0 as one causal product: command-borrow-bound per-call attempts with checked non-wrapping generations; structurally attempt-borrowed private work permits; bounded byte/entry windows and decrementing unit windows; internally derived progress; and receipts carrying attempt generation plus cursor kind/before/after. Treat generation wrap/exhaustion after inspection or ledger mutation, permit lifetime escape/wrong-attempt coexistence, original unbounded access beside a window, post-hoc progress reporting, work without a successful yield/take, receipt-as-authority, residual unused authority, stale/cross-attempt/reused permits/windows, uncharged iterator traversal, or quadratic traversal as release-blocking.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz/Kani target now and retain its minimized corpus. Replay the complete `Connection` corpus and HTTP/1.0 causal product. Cross attempt-generation maximum/exhaustion, permit/window lifetimes, every charged `n` with internally observed progress `0..=n`, byte/entry window ends, `take_unit()` exhaustion, early malformed input, retries/rescans, command mutation, and arithmetic boundaries. Require exhaustion before inspection with unchanged ledgers; no lifetime escape/wrong-attempt coexistence/unbounded side access; window-derived actual progress; exact receipt attempt/cursor audit fields; no residual unused authority; stale/cross-attempt/reuse rejection; bounded linear work; and one reclamation/publication. Preserve all completion-binding, terminal-attribution, HTTP/1 lifecycle, transition, and rejection corpora.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- HTTP/1.0 API extension: expose neither reserved common/cause interrupt binding, completion/attempt generation constructors, resolution/interrupt constructors, revoked reuse identity, work kind/spec/units, private `AdmissionWorkPermit<'attempt, 'command>`, `ChargedWorkWindow`, nor attempt progress cursors. `Http10TerminalDecision` exposes no stored planned state. Every work permit structurally mutably borrows its exact attempt; callers cannot make it outlive/cross/coexist mutably with another attempt, access original unbounded work beside a window, report processed units, fabricate/reuse a permit/window, use `AdmissionWorkReceipt` as authority, or request an uncharged rescan.
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  method, command-borrow-bound attempt cursors and checked generation, O(1)
  unit metadata, structurally borrowed work permits, bounded windows and
  non-authoritative cursor-audit receipts, total
...
- HTTP/1.0 diagnostics extension: report whether the completion binding was reserved by initial/successor admission, common/cause comparison, resolution/interrupt facts, and the exact non-authoritative `AdmissionWorkReceipt` fields: attempt generation, cursor kind, cursor before/after, work kind, charged/processed units, cost and ledger-after values. Progress comes only from the consumed window's internal cursor. Diagnostics store neither command/attempt borrow, cursor, permit, nor window; receipts and other diagnostics never grant authority.
...
- HTTP/1.0 fuzz extension: cross completion and attempt-generation exhaustion, abandonment, exact/stale bindings, every terminal pair, command mutation between retries, structurally borrowed permit/window lifetimes, every charged `n` versus internally derived `0..=n`, bounded byte/entry exposure, `take_unit()` exhaustion, stale/cross-attempt/duplicated/reused permits/windows, receipt misuse, instrumented iterators, early malformed bytes, fragments, retries, and arithmetic errors. Assert pre-inspection exhaustion, unchanged ledgers, exact receipt cursor fields, no under-reporting path/residual authority, and no unpriced/quadratic work.
...
- HTTP/1.0 compile-fail extension: reject binding/generation construction, final-state storage, attempt/cursor escape, reason-only progress retention, `AdmissionWorkPermit<'attempt, 'command>` lifetime erasure/extension, wrong-attempt mutable coexistence, permit/window fabrication/Copy/Clone/reuse/cross-attempt transfer, original unbounded slice/iterator access while a window exists, receipt-to-permit conversion, caller-reported processed units, cursor rewind, and byte/entry/semantic/reservation work without bounded yield or successful `take_unit()`.
...
- HTTP/1.0 soak extension: cycle completion and attempt generations through saturation/exhaustion, mutate commands between retries, destroy/recreate attempts, and run every bounded-window prefix/take-unit split through maximum fragmented methods/targets/fields/output and early rejection until cumulative exhaustion. Require no ABA, post-exhaustion inspection/ledger mutation, lifetime/window escape, under-reported work, receipt audit drift, stale/reused authority, unused-unit reuse/refund, free retry/rescan, quadratic growth, or accounting drift.
...
- HTTP/1.0 role extension: neither role may allocate generations, retain attempts in reason-only outcomes, erase work-permit lifetimes, access unbounded work beside a window, report progress, obtain permits/windows, or turn receipts into authority. Pentest attempt-generation exhaustion/ABA, wrong-attempt coexistence, bounded-window bypass, under-reporting, missing `take_unit()`, permit/window duplication/staleness/cross-attempt use, receipt field forgery/escalation, unused-unit reuse, retry bypass, and fragmentation amplification.
...
- HTTP/1.0 sizing extension: independently bound pre-Active binding reservation, completion/attempt-generation domains, fact-only terminal records, one command-borrow-bound attempt/cursor set, granular specs, one structurally borrowed work permit/window, bounded byte/entry view or decrementing unit counter, O(1) metadata, and receipt fields for attempt generation/cursor kind/before/after. No stored final-state record, cross-call cursor, caller progress input, or simultaneous unbounded work view exists; duplicate binding, cursor, permit/window, receipt-as-authority, or reuse-permit ownership is forbidden.
...
- Independently size each configured early-final request's framing/progress/
  ...
  one-call command-borrow-bound cursors, checked attempt-generation domain,
  O(1) unit metadata, structurally borrowed work permits/bounded windows,
  diagnostic cursor-audit receipts and internally derived progress,
