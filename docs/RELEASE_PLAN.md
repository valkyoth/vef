...
- Each non-empty offer returns a sealed `OutputToken { generation, start, offered_end }` bound to one outstanding lease. Acknowledgement consumes that token exactly once and reports a delta in `0..=offered_end-start`; zero is a valid no-progress write that leaves the cursor unchanged but invalidates that offer token. Duplicate, oversized, stale-generation, cross-output, or out-of-order acknowledgement returns `InvalidState` without cursor/state mutation. The caller certifies that the unacknowledged suffix from a consumed lease will not later be written; VEF may re-offer it under a fresh token from the unchanged frozen record.
...
- Test Engine event, command, acknowledgement, and publication contract and all previously implemented relevant behavior with positive, negative, boundary, truncation, invalid-state, cancellation, capacity, and no-panic cases. Cover private encoding before exposure, first non-empty exposure, zero-length and every short acknowledgement, exact completion, stale/duplicate/oversized/out-of-order tokens, failure with an outstanding lease, caller-buffer reuse attempts, and generation rollover; prove only acknowledged prefixes count as written while every exposed byte remains immutable.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Store policy disposition, current RFC wire state, generation-checked reset output/lease/acknowledged-prefix state, remote-closure cause, terminal-validation state/stage, compression-workspace ownership, immutable terminal-field-section lease, and active field-block ownership independently; policy rejection, closure, HPACK completion, or an intermediate stage alone never releases another dimension. A policy-CANCEL reservation can become dormant only before output exposure. Once reset bytes are exposed, the exact frozen frame and owning stream/tombstone survive until every output token and acknowledged-prefix obligation resolves or connection-owned cleanup runs.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- `Frozen` has at most one generation-bound output token outstanding. Its `acknowledged` cursor advances only by a valid consumed token's exact reported delta and never beyond `offered`; after any positive prefix, emit only `frame[acknowledged..]`, never restart or regenerate. A zero acknowledgement consumes the token without changing the cursor. `Complete` requires acknowledgement of all 13 bytes. Peer reset/valid completion can select `SupersededBeforeExposure` only from `Reserved` with no non-empty exposure; a zero-capacity NeedOutput poll is not exposure. Reserved(remote) DATA and non-idle promised-ID reuse remain connection PROTOCOL_ERROR.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone. Cover policy/wire/closure/terminal/block state × every ResetOutput state/reason/generation/offered/acknowledged cursor × outstanding-token presence × normalized events. Prove only pre-exposure reservation changes reason, every frozen suffix is stable, invalid acknowledgement is state-neutral, closure never strands CONTINUATION or output ownership, and no slot/tombstone generation recycles early.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Track reset output, terminal stage/section lease, and wire closure independently. Peer RST_STREAM or valid remote completion supersedes only unexposed `Reserved`; malformed semantics re-arms PolicyCancel as StreamError only there. Once `Frozen`, later peer reset, malformed evidence, END_STREAM, or GOAWAY cannot replace/suppress it or create a second reset. Stream-slot/tombstone reuse waits for the frozen record and every output token/prefix obligation. Connection failure invalidates the lease via connection-owned cleanup and records only `acknowledged`, never `offered` or a complete frame. Caller-sensitive storage remains redacted and caller-scrubbed after lease termination.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone. Interleave policy-to-error re-arm, peer reset, valid/malformed terminal evidence, GOAWAY, connection failure, cancellation, and capacity before exposure; after exposure/before acknowledgement; after a zero write; after every acknowledged prefix 1..=12; and after completion. Inject stale, duplicate, oversized, cross-generation/out-of-order tokens, caller-buffer reuse, and stream-generation rollover. Prove pre-exposure replacement only, immutable 13-byte frozen identity after exposure, exact suffix continuation, state-neutral invalid acknowledgements, no duplicate reset, acknowledged-only failure diagnostics, and no early record/slot/tombstone reuse.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Rejected promised streams use the v0.118.0 orthogonal model: policy, wire/closure state, `ResetOutput`, output-token generation/cursor, compression workspace, immutable `TerminalFieldSectionLease`, terminal stage, and active block remain distinct. HPACK success moves the section lease into semantics. Final valid/malformed/peer-reset outcomes may change only an unexposed reserved reset; a frozen CANCEL remains the sole immutable frame and continues from its acknowledged suffix. Connection-fatal failure owns cleanup and records acknowledged bytes only.
- Define a total auditable `RejectedPushFrameDisposition` matrix keyed by policy/wire/closure state, every ResetOutput state/reason/generation/offered/acknowledged value and outstanding-token state, terminal stage, workspace/section lease state/capacity, normalized event/END flags, header phase, and active block, with no wildcard/default cell. Every cell declares protocol/error scope, wire transitions, output exposure/freeze/offer/ack/suffix action, HPACK/lease action, application visibility, credit, reset arbitration, peer-reset abort, redaction, and fatal cleanup owner.
...
- Test client-originated PUSH_PROMISE failure and all prior authority/admission cases. Cross every terminal/section state with reset reservation, first exposure, zero write, acknowledged prefixes 1..=12, completion, stale/duplicate/oversized/out-of-order tokens, peer reset, malformed re-arm, GOAWAY, failure, caller-buffer reuse, and stream-generation rollover. Also retain the dynamic-reference/section-lease cases. Prove pre-exposure reason replacement only, immutable frozen bytes, exact suffix continuation, acknowledged-only diagnostics, and no block/stage/reset/output/section lease stranded, aliased, reused early, or completed twice.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Assign stable non-secret diagnostic codes to syntax, protocol scope/code, policy, capacity, cancellation, timeout, transport, and peer/local role; include bounded offsets/counters and generation identifiers, redact field/credential/compression/section contents, and emit each event once. Distinguish local `AssemblyInvalidationCapacity`, `PushedAssemblyProvenanceCapacity`, `StreamTrackingUnavailable`, and `PushRejectionTrackingUnavailable`. Record ResetOutput state/reason/generation, whether non-empty exposure occurred, offered and acknowledged counts, outstanding-token presence, and completion separately; never report offered-but-unacknowledged bytes as written or a complete frame after failure. Record terminal/section/workspace dispositions without raw contents and preserve redaction/caller-scrub rules.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Acceptance contract: Maintain stateful fuzz targets for HTTP/1, HPACK, HTTP/2, Structured Fields, and translation with deterministic minimized replay. The HTTP/2 oracle spans ResetOutput reservation/reason × exposure × offered/acknowledged prefix 0..=13 × token generation/validity × terminal/section/workspace states × END/peer reset/GOAWAY/failure × caller-buffer and stream-generation reuse. It asserts no post-exposure byte/identity mutation, exact suffix-only progress after positive acknowledgement, valid zero-write retry, state-neutral invalid tokens, acknowledged-only failure diagnostics, no early tombstone reuse, section ownership, reset arbitration, and no stranded state.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
