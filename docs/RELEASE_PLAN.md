...
- Acceptance extension: Audit v0.75.0 as one causal product: initial/successor reservation of the future completion binding before Active publication; exact common binding plus cause evidence for every interrupt; independent reuse-resolution record and optional first interrupt with a non-stored exhaustive final-state method; and granular sealed unit specs whose O(1) derivation uses only stored lengths/counts/cursors. Treat post-publication generation allocation, Active without a binding, stale reservation reuse, cached final-state authority, uncharged iterator traversal, inspection before charge, reusable charges, cursor rewind, free retry/rescan, or quadratic traversal as release-blocking.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete `Connection` corpus and HTTP/1.0 causal product. Cross initial/successor completion-generation exhaustion, cancellation, abandonment, and slot reuse; common/cause bindings in every phase and after publication/successor; every resolution/interrupt pair through the exhaustive final-state method; and all granular work kinds at zero/one/max units with instrumented iterators, maximum fields/bytes, one-byte fragmentation, early malformed bytes, retries/rescans, and arithmetic boundaries. Require pre-publication failure, stale neutrality, no cached final authority, charge-before-inspection, single-use charges, monotonic cursors, bounded linear work, and one reclamation/publication. Preserve all earlier HTTP/1 lifecycle, transition, and rejection corpora.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- HTTP/1.0 API extension: expose neither reserved common/cause interrupt binding, completion generation, resolution/interrupt constructors, revoked reuse identity, work kind/spec/units, nor progress cursors. `Http10TerminalDecision` exposes no stored or caller-settable planned state. Callers cannot allocate/rebind a completion generation after Active publication, replay a stale reservation, collapse attribution, pre-traverse iterators for units, select work quantity/cost, reuse a charge, rewind progress, or request an uncharged rescan.
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  method, O(1) stored unit metadata/cursors and single-use charges, total
...
- HTTP/1.0 diagnostics extension: report whether the completion binding was reserved by initial/successor admission, common and cause-specific binding comparison, resolution record, optional first interrupt, exhaustively computed final state, work kind/units/base/per-unit/total, O(1) unit source, progress cursor, single-use retry/rescan charge, and typed error. Diagnostics store no second final-state authority; revoked reuse identity and diagnostics never grant authority.
...
- HTTP/1.0 fuzz extension: cross initial/successor completion-generation exhaustion, abandonment, cancellation, reservation-slot reuse, exact/stale common bindings, and every cause generation in all phases and after publication/successor admission. Combine every resolution/interrupt pair through the non-stored final-state method; fuzz instrumented iterators, zero/one/max units, max fields/bytes, early malformed bytes, one-byte fragments, retries/rescans, and arithmetic errors. Assert pre-publication failure, independent attribution, charge-before-inspection, single-use charges, cursor monotonicity, and no unpriced/quadratic work.
...
- HTTP/1.0 soak extension: cycle initial/successor completion-generation reservation, abandonment, slot reuse, stale interrupt replay, and every resolution/interrupt pair across saturation; run instrumented maximum fragmented methods/targets/fields/output, early rejection, and repeated retries/rescans until cumulative exhaustion. Require no Active-without-binding, ABA, cached-final-state drift, charge reuse/refund, cursor rewind, free unit derivation/rescan, quadratic growth, or accounting drift.
...
- HTTP/1.0 role extension: neither role may forge/reserve a completion binding, collapse resolution with interrupt, store a caller-controlled final state, use revoked identity, derive units by traversal, reuse charges, or control cursors. Pentest generation exhaustion after apparent admission, abandoned-slot replay, inconsistent-final-state injection, instrumented-iterator pre-scan, inspection-before-charge, early-rejection refund, cursor rewind, retry bypass, and fragmentation amplification.
...
- HTTP/1.0 sizing extension: independently bound pre-Active common/cause binding reservation, completion-generation domain, resolution/interrupt records and exhaustive final-state computation, revoked identity, granular kinds/spec units/base/per-unit calculation, O(1) stored lengths/counts, progress cursors, processing chunks, fragmentation, and retry rescans. No stored planned-state record exists; duplicate binding, identity, spec, cursor, charge, or permit ownership is forbidden.
...
- Independently size each configured early-final request's framing/progress/
  ...
  O(1) unit metadata and single-use charge ownership,
