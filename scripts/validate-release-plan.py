...
def main() -> int:
    ...
    required_contract_text += ("sole bounded `Connection` field parser", "return sealed `ValidatedConnectionOptions` bound to the exact ordered field set", "No later milestone may reparse or independently normalize `Connection`", "`ReceivedValidatedCloseOption` can be refined only from the exact v0.56.0 `ValidatedConnectionOptions`", "do not parse, split, case-fold, trim, or normalize `Connection` again", "Consume the exact ordered nominations in v0.56.0 `ValidatedConnectionOptions`", "Reject HTTP/1.0 CONNECT as unsupported before request publication", "Default-close is transport-lifecycle evidence only", "HTTP/1.0 CONNECT remains rejected whether default-close, keep-alive, explicit close, or mandatory-close fallback applies", "sole `Connection` parser and sealed `ValidatedConnectionOptions` consumers", "one atomic runtime transition that consumes the exact `CommittedDownstreamSuccess`", "pre-reserved permit slot", "clear `PrematureInputLatch` snapshot", "matching `DownstreamSuccessFrozen` bridge generation", "duplicate/stale acknowledgements, repeated completion hooks, cancellation races, and generation reuse", "At final commitment inject duplicate and stale acknowledgements", "at most one permit and success event"); failures.extend(f"missing concrete security contract: {contract_text}" for contract_text in required_contract_text if contract_text not in detailed)
