...
| Reusable media-type grammar | `0.157.5`, consumed from `0.158.0` | Add dependency-free `vef-media-type`, seal exact field/generation evidence, validate generated OPTIONS Content-Type before serialization, and prohibit temporary parsers in role or protocol code. |
...
| Variant identity versus evidence | `0.181.1`–`0.181.2` | Separate semantic exact-request/Vary/principal/privacy/navigation identity from per-request/response provenance evidence; assembly verifies fresh evidence but compares only identity, while trailer Vary remains NoRecombine. |
