...
`0.157.1`–`0.157.4`, `0.180.1`–`0.180.5`, `0.181.1`–`0.181.2`, and
`0.182.1`.
...
| Exact media-type classification | `0.180.5`, consumed at `0.181.1` | Add dependency-free `vef-media-type`; parse bounded type/subtype/parameters exactly, reject malformed/duplicate Content-Type, classify multipart/byteranges without substring matching, and never grant multipart top-level Content-Range authority. |
| Explicit partial delivery state | lands at `0.181.1` | Select StreamOnly, RetainOnly, or StreamAndRetain before body consumption; retain before publication/acknowledgement, preflight known 206 capacity, and expose an incomplete prefix only when every received octet was retained. |
...
| Engine-minted variant identity | `0.181.1`–`0.181.2` | Derive bounded Vary selection from the exact original request, bind opaque principal/privacy/navigation state, distinguish absent/empty and legal combining, reject caller normalization, make trailer Vary NoRecombine, and keep physical lease generation out of replacement identity. |
| Conflicting partial quarantine | lands at `0.181.2` | Sort intervals under bounded work, deduplicate equal overlaps, fail unequal octets as `ConflictingPartialContent`, publish nothing, quarantine the context/validator association, and account comparisons with a dedicated local-work budget. |
...
| Unbypassable response semantics | `0.54.0`, `0.137.0`, `0.157.2`, `0.180.1`–`0.181.2`, `0.182.1`, `0.183.0`, `0.191.0`, `0.192.0`, `0.194.0`, `0.197.0` | Make `vef-semantics` depend on core/auth/conditions, require frozen sealed request/response objects in both engines, remove raw public serialization, retain validation through facade/fixed/alloc/features, and compile-fail bypasses. |
...
| `0.180.4` | Outbound conditional and range request validation | Requires Bounded byte ranges and single-range response planning; Unlocks Dependency-free media type grammar and partial-content classification. |
| `0.180.5` | Dependency-free media type grammar and partial-content classification | Requires Outbound conditional and range request validation; Unlocks Client correlation, cancellation, and retry tokens. |
| `0.181.0` | Client correlation, cancellation, and retry tokens | Requires Dependency-free media type grammar and partial-content classification; Unlocks Streaming partial-response and retained-prefix validation. |
