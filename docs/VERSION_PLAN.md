...
| Never-untracked HTTP/2 rejection state | storage at `0.117.0`; END_STREAM state graph at `0.118.0`; flag/block sequencing at `0.120.0`/`0.123.0`; push integration at `0.145.0`; assembly transaction at `0.181.0`; diagnostics at `0.193.0` | Preflight tombstone/cutoff capacity or retain the live/reserved slot in place; an accepted ID is always tracked. Policy, wire state, reset progress, remote-closure cause, and active block remain independent. If tracking cannot be represented, emit typed `StreamTrackingUnavailable`/`PushRejectionTrackingUnavailable` bounded shutdown without forgetting the ID. |
| Rejected-push frame and CANCEL race matrix | model at `0.118.0`; flags/HPACK at `0.120.0`/`0.123.0`; message phases at `0.130.0`–`0.131.0`; credit at `0.136.0`; closure/reset race at `0.138.0`; total push table at `0.145.0`; assembly at `0.181.0`; diagnostics/fuzz at `0.193.0`/`0.196.0` | Key the no-default table by normalized event with END_STREAM/END_HEADERS, header phase, active block, wire/reset state, and `RemoteClosureCause`. HEADERS+END_STREAM performs two transitions without abandoning CONTINUATION; DATA+END_STREAM credits before closure; reset or END_STREAM suppresses only a zero-byte CANCEL, while partial serialization finishes once. |
...
| `0.117.0` | Generation-checked stream table and tombstones | Requires HTTP/2 stream-identifier rules; retains independent policy/wire/reset/remote-closure/field-block fields through classification horizons so accepted IDs never become idle; Unlocks Exhaustive stream-state graph. |
| `0.118.0` | Exhaustive stream-state graph | Requires Generation-checked stream table and tombstones; normalizes END_STREAM separately so HEADERS can cause two transitions while CONTINUATION ownership survives closure; Unlocks HTTP/2 activation preface, first-SETTINGS, and deadline sequencing. |
...
| `0.120.0` | HTTP/2 frame legality and fragmented-header-block sequencing | Requires HTTP/2 activation preface, first-SETTINGS, and deadline sequencing; normalizes HEADERS/DATA/CONTINUATION flags and separates closure from active block ownership; Unlocks HTTP/2 error scope, typed deltas, and isolated stream mutation. |
| `0.121.0` | HTTP/2 error scope, typed deltas, and isolated stream mutation | Requires HTTP/2 frame legality and fragmented-header-block sequencing; keeps reserved(remote) DATA, illegal promised-ID reuse, and malformed HPACK connection-fatal even on rejecting/synchronization-only paths; Unlocks HTTP/2 graceful GOAWAY and bounded shutdown sequencing. |
...
| `0.123.0` | Atomic HPACK header-block integration | Requires HTTP/2 graceful GOAWAY and bounded shutdown sequencing; completes fragmented HPACK after HEADERS+END_STREAM closes wire state and releases block ownership independently; Unlocks SETTINGS header-table encoder and header-list policy coupling. |
...
| `0.130.0` | HTTP/2 content-length, DATA, trailers, and END_STREAM reconciliation | Requires HTTP/2 response mapping; orders DATA padding/framing/policy work before its separate END_STREAM closure; Unlocks Informational responses and trailers. |
| `0.131.0` | Informational responses and trailers | Requires HTTP/2 content-length, DATA, trailers, and END_STREAM reconciliation; crosses initial/informational/final/trailer phase with END_STREAM/END_HEADERS and retains blocks after closure; Unlocks Cookie field combination and Set-Cookie preservation. |
...
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; broadens supersession to remote reset/END_STREAM with separate cause, suppresses only zero-byte queued CANCEL, finishes one partial frame, and preserves active blocks; Unlocks SETTINGS outstanding-ACK accounting. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.138.0 state/reset foundations; completes the total rejected-push matrix with END_STREAM/END_HEADERS, header phase, credit, closure cause, CANCEL race, and in-flight push after associated reset; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.181.0` | Client correlation, cancellation, and retry tokens | Requires Partial-response media-type classification plus v0.145.0 push admission; atomically preflights pushed resources, changes policy only on capacity, and inherits normalized END_STREAM/block/closure/reset handling; Unlocks Streaming partial-response and retained-prefix validation. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports rejection, wire state, block status, reset commit/supersession, Reset/EndStream closure cause, and actual zero/one frame emission independently; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts state/reset/closure × END_STREAM/END_HEADERS × header phase × HPACK/credit/output boundary products; Unlocks Compile-fail state and lifetime tests. |
