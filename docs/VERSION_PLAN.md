...
| `0.20.0` | Minimal synchronous I/O contracts | Requires Role, profile, and policy types; separates offered from acknowledged prefixes and restricts each scalar/vectored offer to one protocol-record suffix; Unlocks Runtime-neutral readiness and poll contracts. |
...
| `0.24.0` | Deterministic fake transport and driver harness | Requires Cancellation, close, and bounded-backpressure contracts; interleaves exposure, zero/short acknowledgement, invalid tokens, failure, and buffer reuse; Unlocks Engine event, command, acknowledgement, and publication contract. |
| `0.25.0` | Engine event, command, acknowledgement, and publication contract | Requires Deterministic fake transport and driver harness; makes first exposure irrevocable and binds each single-use generation token to one record suffix so acknowledgement cannot cross records or batch hooks; Unlocks Requirement, applicability, and errata evidence system. |
...
| `0.27.0` | Foundation Kani campaign, audit, and pentest | Requires Requirement, applicability, and errata evidence system; model-checks frozen-byte immutability, token linearity, prefix progress, and reuse exclusion; Unlocks HTTP/1 role and parser profiles. |
...
| `0.92.0` | HPACK table-size update and SETTINGS coupling | Requires HPACK eviction and oversize-entry behavior; defines received/acknowledged/selected/physical limits, local-policy update obligations, initial-below-4096 behavior, and safe decoder advertisement; Unlocks HPACK caller-owned ring lookup. |
...
| `0.109.0` | PING frame codec | Requires SETTINGS syntax, role, directional values, and ACK rules; owns copied per-request reply transactions plus bounded unique-key local ACK correlation, while exact output scheduling waits for later milestones; Unlocks GOAWAY frame codec. |
...
| `0.124.0` | SETTINGS header-table encoder and header-list policy coupling | Requires Atomic HPACK header-block integration; reductions immediately clamp selected capacity and record selected deltas, increases change only received ceiling, and per-frame obligations await ACK without conflating raw ceilings with debt; Unlocks Pseudo-field ordering and uniqueness. |
...
| `0.130.0` | HTTP/2 content-length, DATA, trailers, and END_STREAM reconciliation | Requires HTTP/2 response mapping; owns ContentAndPhase, finalizes valid DATA only after all applicable checks, and sends terminal HEADERS onward; Unlocks Informational responses and trailers. |
| `0.131.0` | Informational responses and trailers | Requires HTTP/2 content-length, DATA, trailers, and END_STREAM reconciliation; final HEADERS validation transfers the exact lease into unpublished message ownership or sends it to cleanup; Unlocks Cookie field combination and Set-Cookie preservation. |
...
| `0.135.0` | SETTINGS initial-window active-stream integration and atomic rollback | Requires Connection flow control; applies ordered INITIAL_WINDOW_SIZE through one shared transaction participant, with atomic window/reservation rollback and no ACK ownership; Unlocks HTTP/2 inbound DATA ownership, acknowledgement, and credit release. |
...
| `0.137.0` | HTTP/2 outbound per-stream message command lifecycle | Requires inbound ownership plus both send ledgers; pre-materializes field blocks, makes Private lease unsignaled debt, and transfers it only at first non-empty exposure to the guaranteed block; Unlocks HTTP/2 body cancellation, reset, and flow-credit lifecycle. |
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; restores the exact debt lease when rolling back a whole Private block, or drains transferred FramingCommitted debt before reset; Unlocks SETTINGS outstanding-ACK accounting. |
| `0.139.0` | SETTINGS outstanding-ACK accounting | Requires HTTP/2 body cancellation, reset, and flow-credit lifecycle; byte-nine FIFO commitment advances each frame's wire-acknowledged ceiling snapshot and merges only its selected delta into debt; Unlocks Bounded stream admission. |
...
| `0.141.0` | SETTINGS max-concurrent-streams admission integration | Requires Bounded stream admission; attaches admission/MAX_CONCURRENT_STREAMS to the shared frame transaction without ACK authority; Unlocks Bounded outbound scheduling. |
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; orders frozen suffix/CONTINUATION obligations before FIFO PING ACK priority, then other work, while preserving exact WINDOW_UPDATE and ACK/debt completion; Unlocks SETTINGS max-frame-size outbound integration. |
| `0.143.0` | SETTINGS max-frame-size outbound integration | Requires Bounded outbound scheduling; attaches MAX_FRAME_SIZE/resegmentation to the shared transaction, preserves preflight blocks across reductions, and computes DATA without truncation; Unlocks GOAWAY cutoff and retry classification. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.142.0 foundations; applies identical debt leasing/transfer across PUSH_PROMISE while attaching ENABLE_PUSH to the shared SETTINGS transaction; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.147.0` | Independent HTTP/2 rate and work budgets | Requires ALPN and cleartext prior-knowledge selection; charges PING correlation lookup/reply creation and all other independent work before mutation; Unlocks Rapid-reset defenses. |
...
| `0.150.0` | PING flood defenses | Requires SETTINGS amplification defenses; charges ACK/non-ACK frames, forbids reply coalescing/dropping, and chooses reserved capacity or bounded shutdown; Unlocks CONTINUATION bomb defenses. |
| `0.151.0` | CONTINUATION bomb defenses | Requires PING flood defenses; Unlocks WINDOW_UPDATE churn defenses. |
| `0.152.0` | WINDOW_UPDATE churn defenses | Requires CONTINUATION bomb defenses; bounds private same-target coalescing without changing frozen target/increment or exceeding the 31-bit advertised-window ceiling; Unlocks Reserved control-output queues. |
| `0.153.0` | Reserved control-output queues | Requires WINDOW_UPDATE churn defenses; owns per-PING exact offsets 0..=17, WINDOW_UPDATE offsets 0..=13, and SETTINGS ACK byte-nine commitment without coalescing obligations; Unlocks HTTP/2 conformance audit and pentest. |
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits PING ownership/correlation/scheduling, receive-credit frozen-output boundaries, four-limit HPACK, and every prior ACK/debt/block boundary; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.191.0` | Fixed-capacity caller-storage public API | Requires Authenticated origin authorization and HTTP/2 coalescing metadata; seals PING reply/correlation, HPACK ACK/debt, and receive-credit/update authority behind fixed-capacity generation-checked APIs; Unlocks Optional alloc-backed convenience API. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports redacted PING transaction/correlation disposition beside HPACK and receive-credit ownership without opaque payload contents; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; crosses PING FIFO/correlation and offsets 0..=17 alongside HPACK and receive-credit state while proving exact ownership and conservation; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects forged/rebound PING output/correlation, HPACK authority, and early/retargeted receive-credit commitment; Unlocks Long-running soak and exhaustion campaign. |
...
| `0.211.0` | Deterministic CPU, stack, code-size, and amplification budgets | Requires Aesynx kernel integration tests; sizes PING FIFO/correlation/tombstones and frozen 17-byte records alongside HPACK, receive ledgers, control queues, checked arithmetic, and amplification; Unlocks 32-bit target campaign. |
