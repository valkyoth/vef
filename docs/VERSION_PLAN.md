...
| Long-lived Vary identity storage | lands at `0.181.1` | Retain exact bytes through immutable request leases or exclusive-slice/sealed-arena engine-frozen storage, prohibit alias/trust/DMA mutation, redact sensitive values, require caller scrub after release, and downgrade locally on capacity/release. |
| One-time Vary normalization | lands at `0.181.1` | Activate profile-capped work/output accounting during identity creation, store complete raw and canonical sensitive values without digests, preserve field-defined equivalence exactly, and make every later comparison parse-free. |
| Full-200 fallback invalidation | `0.181.1`–`0.181.2` | Emit a keyed replacement when possible; otherwise conservatively invalidate all sibling contexts in the retained target/principal/partition/navigation/coding/domain scope, or rotate the assembly arena generation when even that scope is unavailable. |
...
| `0.181.0` | Client correlation, cancellation, and retry tokens | Requires Partial-response media-type classification integration; Unlocks Streaming partial-response and retained-prefix validation. |
