...
- Acceptance contract: Arbitrate each valid peer SETTINGS_HEADER_TABLE_SIZE with `HpackEncoderTransaction` through a bounded connection-owned `PendingEncoderTableSizeTransition::{None, AwaitingSafeApply { owners: SettingsTransactionSet, smallest_seen, final_value }, AppliedAwaitingAckCommit { owners: SettingsTransactionSet, smallest_seen, final_value }, WireEnabled { smallest_seen, final_value }}`. It never owns, counts, queues, or serializes SETTINGS ACKs; each generation-bound owner is the v0.108.0 frame-wide `InboundSettingsTransaction`. With no active encoder transaction, atomically apply the received limit, move to AppliedAwaitingAckCommit, and complete each HPACK participant. With Private unexposed HPACK, roll back provisional encoder mutations and complete field-block slots before applying and reporting Effective; delay replacement encoding rather than retaining provisional output across the ACK barrier. With FramingCommitted, retain owner references while the pre-reserved block drains without interleaving; after final END_HEADERS-frame acknowledgement, publish the old HpackCommitted transaction, apply the pending transition, report HPACK Effective, and permit the frame-wide ACK scheduler to proceed. Across duplicate values, multiple entries, or multiple owning SETTINGS transactions while unresolved, retain the smallest observed maximum and final maximum in receive order. Preserve every owner reference and FIFO commitment dependency until the final byte of every owning nine-byte ACK moves its transaction to AckCommitted; only then promote WireEnabled and permit the next field block to begin with one size update when equal or the RFC-required smallest-then-final pair when different. A connection-fatal HPACK result before ACK exposure cancels the owning transaction; partial ACK output, stale acknowledgement tokens, or transport failure abandons the connection and never promotes WireEnabled. Local re-encoding/storage failure after WireEnabled is ordinary backpressure and cannot retroactively affect committed ACKs. No later HEADERS/PUSH_PROMISE block may encode or expose while a provisional transaction, AwaitingSafeApply, or AppliedAwaitingAckCommit state remains unresolved. Treat peer MAX_HEADER_LIST_SIZE as outbound guidance while preserving independent hard inbound decode limits.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone.
- Cross no-active, Private, and every FramingCommitted initial/CONTINUATION/final acknowledgement prefix with one/multiple combined SETTINGS frames, duplicate HEADER_TABLE_SIZE entries, decreasing/increasing sequences, ACK output offsets 0..=9, output backpressure, stale tokens, local re-encode capacity failure, another participant pending/failing, reset, cancellation, GOAWAY, and connection failure. Prove exact owner references without ACK ownership, Private rollback, old-transaction publication before pending transition, HPACK participant completion at apply, AckEligible distinct from AckCommitted, FIFO commitment of every owner, no replacement encoding or dependent block exposure through ACK offset eight, WireEnabled only at all-owner byte nine, smallest/final update order at next-block start, no later stale/provisional encoding, and no double update/ACK.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Serialize encoder work and SETTINGS_HEADER_TABLE_SIZE transitions: no new field-block encoding is runnable while a Private/FramingCommitted transaction or `PendingEncoderTableSizeTransition` is unresolved. A setting supersedes Private work, applies the transition, and reports HPACK Effective, but replacement encoding waits; behind FramingCommitted, retain the owning SETTINGS transaction while completing the block, then apply and report Effective. Schedule the one all-effective frame ACK in transaction FIFO order, freeze its exact nine-byte record and suffix, and retain the transaction plus transition owners through acknowledged offsets 0..=8. Only final-byte acknowledgement moves AckFrozen to AckCommitted; after every transition owner commits, promote WireEnabled and permit Private re-encoding or the next HEADERS/PUSH_PROMISE block and its size-update prefix. Invalid/stale tokens change nothing, output backpressure preserves the suffix/barrier, and transport failure abandons the connection without dependent exposure.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Cross SETTINGS ACK offsets 0..=9, multiple table-size-owning transactions,
  smallest/final collapse, exact suffix backpressure, stale tokens, and failure
  before byte nine; prove FIFO AckCommitted promotion and zero dependent
  HEADERS/PUSH_PROMISE exposure until every owner commits.
...
- Distinguish Private, FramingCommitted, HpackCommitted, AbandonedWithConnection, AwaitingSafeApply/AppliedAwaitingAckCommit/WireEnabled encoder transition, and WaitingParticipants/AckEligible/AckFrozen/AckCommitted/AbortedConnection SETTINGS disposition; report frame generation, ordered-entry count, participant kinds/pending count, one ACK-slot state, acknowledged offset 0..=9, FIFO ordinal, retained-owner count, redacted smallest/final values, field layout, one-record token identity, and provisional/published HPACK generation without settings contents. Never attribute ACK ownership to a participant, report AckEligible or offset eight as committed, diagnose first exposure as HPACK publication, collapse frame ACKs with size updates, or report dependent field exposure after participant/output failure.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Add transitions for every combined-entry `InboundSettingsTransaction`: full validation/reservation, wire-order HPACK/window/frame-size/admission/push/extension participation, duplicate/unknown entries, participant pending/failure, transaction/ACK capacity, WaitingParticipants/AckEligible/AckFrozen/AckCommitted/AbortedConnection, ACK offsets 0..=9, FIFO serialization/commitment across frames, FramingCommitted delay, delayed Private re-encode, AwaitingSafeApply/AppliedAwaitingAckCommit/WireEnabled, multiple owners, smallest/final collapse, output backpressure, stale tokens, and failure before the final ACK byte. Assert exactly one ACK or connection-fatal cancellation, retained owners through byte eight, no later ACK overtaking, and no dependent HEADERS/PUSH_PROMISE encoding or exposure before all-owner commitment. Also cover every local field cap/bound, HPACK state, checked arithmetic, and scalar/vectored cross-slot acknowledgement. No participant may emit an ACK and no generated acknowledgement may complete more than one output frame.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
