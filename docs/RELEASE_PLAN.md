...
- Before minting that identity, preflight every selected request field name, ordered raw field line/value, absence marker, and sealed-normalizer identifier into one explicit long-lived storage mode: immutable generation-bound `VariantFieldLease<'a>` values over the original request storage, or an engine-written caller-provided fixed-capacity `VariantIdentityStorage<'a>` that freezes as an immutable lease. Never truncate, silently omit, or use a digest/hash as equality authority; opaque caller replacement tokens are outside 1.0, so equality uses the complete retained bytes under the declared sealed normalizer. Identity-storage exhaustion yields local `VariantIdentityCapacity` and permanently downgrades only that response to `NoRecombine`, never a peer violation. Releasing/recycling borrowed request storage or copied-identity storage invalidates its evidence and every dependent segment/context admission.
...
- Test sealed Content-Type absent/non-multipart/multipart/malformed/duplicate/invalid-boundary classifications and prove multipart never reaches top-level Content-Range. Exhaust all public delivery preferences and engine-selected permits under every chunk split, preference/permit mismatch, forged/stale/cross-response permit, storage-before-publication order, acknowledgement/recycle attempt, backpressure, cancellation, and capacity boundary. Prove 206 retention preflights before body, local capacity is not peer error, RetainOnly failure cancels, StreamAndRetain incomplete-200 exhaustion permanently continues StreamOnly/NoRetainedPrefix, and cancellation exposes a prefix only when every received octet was retained. Test zero-retention ranges beyond every profile, terminal proof, safe leases, DMA fencing, and head/trailer ETag finalization. For variant selection exhaust bounded/case-insensitive Vary deduplication, `*`, exact request values, absent/empty and combinable/non-combinable repeats, sealed normalizers, trailer Vary, identity equality across fresh response generations, and evidence provenance. Exercise borrowed `VariantFieldLease` and copied `VariantIdentityStorage` with maximum values, exact capacity/one-byte shortage, no truncation/digest/token substitution, storage release/recycling, stale leases, sensitive Cookie/Authorization/unknown caller-secret fields, redacted Debug/diagnostics/errors/traces, cross-principal/privacy attempts, local `VariantIdentityCapacity` permanent NoRecombine, every split/limit, and no panic/allocation.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
...
- Exhaust safe split/separate-arena output acquisition, compile-fail alias/mutation/recycle/decode attempts, stale physical and identity-storage lease generations, bad copy schedules, output exhaustion, cancellation, and DMA fencing. Combine distinct request/connection/storage generations only after fresh identity-storage/evidence admission and semantic equality; reject released leases, forged/stale/rebound evidence, caller-normalized identities, standalone-only, multipart, unknown-unit, principal/partition/navigation/Vary/coding/domain mismatch, and `Vary: *`, while proving response generations do not poison identity equality. Test requested budgets zero/one/exact-profile/above-profile/maximum integer, local rejection before sorting/output, forged/stale/cross-plan `ActiveOverlapBudget`, checked exhaustion at every sort/comparison charge, saturating telemetry, and reset/enlarge/replace attempts. Use sorted-interval models to prove bounded work, equal-overlap deduplication, and zero output on WorkLimit/conflict. Exhaust quarantine clearance restrictions; head/trailer validator finalization; head-published ordinals under body delay/retry/Date manipulation; full/prefix/continuous/disjoint unions; corrected headers/lengths; backpressure; and no panic/allocation.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
