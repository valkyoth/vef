...
def main() -> int:
    ...
    required_contract_text = tuple(text for text in required_contract_text if text not in ("can be minted only for legal success-following input", "upstream HTTP/1 101 plus over-read WebSocket bytes in the same read", "downstream RFC 8441 DATA before complete 2xx transport commitment", "Discard their owned bytes exactly once without HTTP reparsing or early credit restoration", "never enter Active, and never forward them after later success", "`PendingHttp1Transition { overread: OverreadLease, terminal: Option<PreActiveTerminalCause>, bridge_generation, leg, exchange_generation, transport_generation }`", "`PlainTcpEof`, `TlsCloseNotify`, `FatalTlsAlert`, `TransportReset`, and `Cancellation`", "disposes bytes and terminal ownership exactly once", "Assert premature input creates no transferable lease or Active state", "Cross HTTP/1 legal over-read with plain TCP EOF, TLS close_notify, fatal TLS alert, reset, and cancellation at every bridge phase")) + ("`TransitionInputProvenance::{SuccessFollowingTransitionInput, PermittedOptimisticConnectInput(OptimisticConnectPermit), UnpermittedOptimisticConnectInput { reason: MissingCommittedCloseProof }, ForbiddenOptimisticWebSocketInput, ForbiddenHttp1ConnectUdpInput}`", "can be minted only from the first two variants", "`OptimisticConnectCloseProof::{ReceivedValidatedCloseOption, LocallyCommittedCloseHead}`", "fully validated `Connection: close` option in that exact request head", "exact serialized close-bearing request head reached v0.54.0 `HeadCommitted`", "queued bytes or configured close intent alone grant no authority", "Missing proof selects the strict unpermitted disposition: discard once, close, never HTTP-reparse, and never later promote even if CONNECT succeeds", "HTTP/2 ordinary CONNECT remains permitted only through the existing v0.130.0 PendingConnect generation", "charge the complete flow-controlled payload against both stream and connection windows", "reclaim it only through the existing v0.136.0 `ReceiveCredit` ledger", "If the provisional 2xx remains `AcceptedPrivate` and no byte was exposed", "emit only reserved `RST_STREAM(PROTOCOL_ERROR)`—no HTTP response", "`Frozen` or `FramingCommitted`, including first exposure with zero acknowledged bytes", "finish the exact immutable HEADERS/CONTINUATION suffix contiguously through END_HEADERS", "commit HPACK and framing normally", "only then emit the reserved reset", "final END_HEADERS acknowledgement was processed before the DATA in the same combined call", "connection-owned cleanup without fabricating response completion, Active, permit, or reset completion", "`CommittedDownstreamSuccess` records full wire commitment only", "`PrematureInputLatch` is clear", "Only the engine-minted `BridgeActivationPermit`", "Later same-buffer SETTINGS, PING, HEADERS, and DATA for unrelated streams continue normally", "`PendingHttp1Transition { overread: Option<OverreadLease>, terminal: Option<PreActiveTerminalCause>, bridge_generation, leg, exchange_generation, transport_generation }`", "valid 101/2xx followed by immediate termination with zero bytes", "`PlainTcpEof`, `TlsCloseNotify`, `TlsTruncationEof`, `FatalTlsAlert`, `TransportReset`, `TransportFailure`, and `Cancellation`", "disposes optional bytes and terminal ownership exactly once", "both exact close-proof constructors", "every initial HEADERS acknowledgement offset", "every CONTINUATION boundary and offset", "connection failure at every immutable suffix prefix", "valid HTTP/1 success head plus terminal with zero over-read bytes", "map them without collapse to `PreActiveTerminalCause::{PlainTcpEof, TlsCloseNotify, TlsTruncationEof, FatalTlsAlert, TransportReset, TransportFailure, Cancellation}`")
    required_contract_text += ("sole bounded, version-neutral `Connection` field parser", "return sealed `ValidatedConnectionOptions` bound to the exact ordered field set, exact HTTP version", "No later milestone may reparse or independently normalize `Connection`", "At this stop activate only the HTTP/1.1 persistence interpretation", "`ReceivedValidatedCloseOption` can be refined only from the exact v0.56.0 `ValidatedConnectionOptions`", "do not parse, split, case-fold, trim, or normalize `Connection` again", "Define sealed `Http10PersistenceDisposition` over role, received message kind/direction, connection generation, and newest-message generation", "Default-close client request generation emits `Connection: close`", "A proxy or gateway receiving an HTTP/1.0 request selects `IntermediaryDownstreamClose`", "`OriginRequestCandidate`", "`ClientUpstreamCandidate`", "`IntermediaryUpstreamCandidate`", "`InvalidRoleDirection`", "two-sided, per-hop evidence", "received-peer evidence `ValidatedHttp10KeepAlive`", "`CommittedHttp10KeepAliveHead` only when that exact request or response head reaches `HeadCommitted`", "For a client or proxy-upstream response, ordering is mandatory", "first correlate the response with the oldest matching v0.57.0 `HeadCommitted` request", "move that exact request's `CommittedHttp10KeepAliveHead` into a sealed `CorrelatedHttp10KeepAliveRequest` owned by the response lifecycle", "then revoke every other stale or unmatched local signal", "Missing, duplicate, or generation-mismatched correlation closes as ambiguous", "The matched signal survives admission of its response", "After the correlation transfer above, admission of a newer received message atomically revokes", "the correlated signal owned by that exact response remains until pairing or terminal cleanup", "Maintain one engine-owned `Http10ReuseLedger { connection_generation, hop, remaining }`", "initialized once from the configured maximum and monotonically non-increasing", "the ledger has nonzero remaining allowance, and checked deadline construction succeeds", "The permit binds the exact ledger generation and observed remaining count but does not decrement it", "Configured maximum zero mints no permit", "checked idle-deadline addition overflow selects local close without a permit", "A proxy constructs evidence and ledger independently on each hop", "received keep-alive evidence is never forwarded", "Define the sole successor-admission edge as `Http10ConnectionState::Reusable { permit: Http10ReusePermit } -> ActiveExchange { next_exchange_generation, remaining_after_admission }`", "`Http10SuccessorAdmissionOutcome::{Admitted, RetryableCapacity { permit }, CloseLocal { reason }}`", "`RetryableCapacity` covers only local slot/storage capacity", "`CloseLocal` covers `ZeroReuseAllowance`, `RequestCountExhausted`, `WorkBudgetExhausted`, `PolicyRevoked`, `IdleDeadlineReached`, `DeadlineArithmeticOverflow`, `CorrelationInvariantFailure`, and `GenerationExhausted`", "every reason is local policy/resource/invariant state, never peer misconduct", "It revokes the permit, preserves the ledger count, returns all input wholly unconsumed", "Exact deadline equality (`now >= deadline`) is expired", "Generation uses checked increment only", "maximum/exhaustion closes the entire connection context", "On `Admitted`, atomically consume the non-Copy permit, checked-decrement the connection/hop ledger exactly once", "store its `remaining_after_admission` in both the ledger and `ActiveExchange`", "The next permit can only observe that carried count", "Repeated capacity failures never decrement it", "cancellation/failure after admission never refunds it", "The combined-call fast path exists only when acknowledgement causes the exact outbound response lifecycle transition into `MessageCommitted`", "for fixed-length output this is the final body byte", "for chunked output the terminating chunk plus all trailers", "for bodyless output final head acknowledgement may also complete the message", "Full acknowledgement of a nonfinal head/body record does not qualify", "zero, short, or full acknowledgement that does not reach `MessageCommitted` leaves successor input wholly unconsumed as `PrematureHttp10Successor`", "If message commitment cannot mint a permit", "successor input remains wholly unconsumed and the connection closes", "no failure rolls back to `Reusable`", "Test reuse maxima 0, 1, and N across complete exchanges", "exact `now == deadline`", "checked deadline-add overflow", "generation maximum-minus-one, maximum, and checked exhaustion while retaining stale tokens", "Loss of persistence uses the exact v0.74.0 `AcceptedPrivate`/`Frozen`/`HeadCommitted`/`MessageCommitted` close boundary", "At `AcceptedPrivate`, replace the current private head with a revalidated head containing `Connection: close` before exposure", "At `Frozen`, including first exposure with zero acknowledged bytes, never mutate the head", "A client that receives an HTTP/1.0 response without valid keep-alive closes and emits no next request", "No HTTP/1.0 keep-alive path enables pipelining", "cannot construct `ReceivedValidatedCloseOption`, authorize optimistic CONNECT, bind HTTP/1.1 Upgrade", "Consume the exact ordered nominations in v0.56.0 `ValidatedConnectionOptions` for either HTTP/1.0 or HTTP/1.1", "CONNECT classification precedence is total", "Local storage, response-reserve, or local work-capacity failure returns its local capacity action", "Before target/field classification, aggregate start-line byte or parse-work exhaustion and overlong method token select typed", "`StartLinePeerLimitAndClose::{AggregateBytes, AggregateWork, MethodBytes}`", "Overlong or malformed version selects `StartLineVersionAndClose`", "A request target within its own limit that makes the complete start line exceed its aggregate limit remains `AggregateBytes`, not 414", "Request-target byte/work limit alone selects the existing 414-and-close disposition", "Field-line, field-count, field-section, or field-parse-work limit selects the existing 431-and-close disposition", "Other syntax, invalid target form, malformed fields/framing, or CONNECT-content violation selects the existing bounded 400-and-close path", "Only a completely bounded and syntactically recognized HTTP/1.0 head with method CONNECT and valid authority-form target mints typed `UnsupportedHttp10ConnectDisposition`", "Authority-form recognition exists solely to classify this unsupported method", "one-under, exact, and one-over every start-line/component boundary", "local client builder returns zero-output `UnsupportedVersionMethod`", "freeze exactly the 70-byte HTTP/1.0-compatible response `HTTP/1.0 501 Not Implemented\\r\\nConnection: close\\r\\nContent-Length: 0\\r\\n\\r\\n` plus mandatory transport close", "never resolve or forward the authority", "Reserve all 70 bytes and the close action before disposition publication", "`UnsupportedHttp10ConnectAction::{Flush501ThenClose, CloseTransportNow}`", "An invalid, stale, cross-record, or oversized acknowledgement is completely state-neutral", "Input-only delivery with a live 501 token likewise returns `DriverCommitOrderViolation` state-neutrally", "After a valid zero, short, or full acknowledgement is applied first", "any accompanying new peer input remains wholly unconsumed and selects `CloseTransportNow`", "no bounded-input backpressure state", "A short acknowledgement followed by this close never fabricates response commitment", "discard them exactly once", "attempt application publication, authority resolution, forwarding, successor parsing, and tunnel creation", "one atomic runtime transition that consumes the exact `CommittedDownstreamSuccess`", "pre-reserved permit slot", "clear `PrematureInputLatch` snapshot", "matching `DownstreamSuccessFrozen` bridge generation", "duplicate/stale acknowledgements, repeated completion hooks, cancellation races, and generation reuse", "At final commitment inject duplicate and stale acknowledgements", "at most one permit and success event"); failures.extend(f"missing concrete security contract: {contract_text}" for contract_text in required_contract_text if contract_text not in detailed)
    obsolete_http10_contracts = ("the ledger has nonzero remaining allowance, and checked deadline construction succeeds", "Configured maximum zero mints no permit", "checked idle-deadline addition overflow selects local close without a permit", "Define the sole successor-admission edge as `Http10ConnectionState::Reusable { permit: Http10ReusePermit } -> ActiveExchange { next_exchange_generation, remaining_after_admission }`", "`Http10SuccessorAdmissionOutcome::{Admitted, RetryableCapacity { permit }, CloseLocal { reason }}`", "`RetryableCapacity` covers only local slot/storage capacity", "`CloseLocal` covers `ZeroReuseAllowance`, `RequestCountExhausted`, `WorkBudgetExhausted`, `PolicyRevoked`, `IdleDeadlineReached`, `DeadlineArithmeticOverflow`, `CorrelationInvariantFailure`, and `GenerationExhausted`", "every reason is local policy/resource/invariant state, never peer misconduct", "On `Admitted`, atomically consume the non-Copy permit, checked-decrement the connection/hop ledger exactly once", "store its `remaining_after_admission` in both the ledger and `ActiveExchange`", "The next permit can only observe that carried count", "The combined-call fast path exists only when acknowledgement causes the exact outbound response lifecycle transition into `MessageCommitted`", "for fixed-length output this is the final body byte", "for chunked output the terminating chunk plus all trailers", "for bodyless output final head acknowledgement may also complete the message", "If message commitment cannot mint a permit", "successor input remains wholly unconsumed and the connection closes", "checked deadline-add overflow", "generation maximum-minus-one, maximum, and checked exhaustion while retaining stale tokens"); failures = [failure for failure in failures if failure not in tuple(f"missing concrete security contract: {text}" for text in obsolete_http10_contracts)]; required_http10_contracts = ("Positive HTTP/1.0 reuse framing is restricted to a semantically bodyless message or exact fixed-length Content-Length message at `MessageCommitted`", "Transfer-Encoding, chunked coding, trailers, and HTTP/1.1 transfer semantics remain rejected", "A valid close-delimited HTTP/1.0 response may complete only by closure and can never preserve the connection or mint reuse authority", "`Http10PermitMintOutcome::{Minted, CloseWithoutReuse { reason }}`", "`Http10PermitMintCloseReason::{ZeroConfiguredAllowance, LedgerExhausted, NegotiationUnavailable, FramingNotReusable, PolicyRevoked, DeadlineArithmeticOverflow, CorrelationInvariantFailure}`", "`Minted` creates one sealed permit inside engine-owned `Reusable`", "Every close-without-reuse reason creates no permit, returns same-call successor input wholly unconsumed, and closes without a peer protocol error", "Configured maximum zero and ledger exhaustion are mint failures, not admission outcomes", "Checked idle-deadline addition overflow is also a mint failure", "ActiveExchange { next_exchange_generation, reuse_remaining_snapshot }", "`Http10SuccessorAdmissionOutcome::{Admitted, RetryableCapacity { reason: Http10CapacityReason }, CloseLocal { reason }}`", "`RetryableCapacity` carries only a freely constructible reason", "permit remains exclusively engine-owned inside `Reusable` and is never returned, borrowed, or exposed to the caller", "`PermitLedgerMismatch` and correlation failure are local invariant/security events", "checked-decrement `Http10ReuseLedger.remaining` exactly once", "store the result back only in that ledger", "capture immutable `reuse_remaining_snapshot` in `ActiveExchange`", "`Http10ReuseLedger.remaining` is the sole mutable reuse-authorization source", "The next permit reads only the ledger", "Chunked/trailered output has no HTTP/1.0 positive path", "first obtains `Http10PermitMintOutcome`; only `Minted` may immediately run successor admission", "`CloseWithoutReuse` leaves input wholly unconsumed and closes", "extract, retain, rebind, or duplicate the engine-owned permit", "generation maximum-minus-one/maximum/exhaustion with stale tokens", "Negatively test Transfer-Encoding, chunked framing, terminating chunks, trailers, close-delimited response bodies"); failures.extend(f"missing revised HTTP/1.0 security contract: {text}" for text in required_http10_contracts if text not in detailed); failures.extend(["MAX_FRAME_SIZE is still applied after acknowledgement"] if "after acknowledgement while retaining independent inbound hard limits" in detailed else [])
    required_http10_last_allowance_contracts = ("`remaining` means exactly the number of future successor admissions still authorized, excluding the currently active exchange", "Configured zero therefore selects the v0.74.0 close disposition for the initial exchange before its local head leaves `AcceptedPrivate`", "If admission decrements one to zero, that one successor is admitted", "zero `reuse_remaining_snapshot` cannot generate a local keep-alive signal or `CommittedHttp10KeepAliveHead`", "This last-allowance close decision cannot wait for end-of-message permit minting", "configured maximum one means exactly one successor whose local head announces close, then no second permit", "`Http10NextExchangeReservation` containing the next-exchange record, correlation owner, parser/event/output storage leases", "one request/exchange-count debit", "minimum parser-work reserve needed to reach safe backpressure", "It is all-or-nothing: every one-short storage, queue, count, or work failure releases the incomplete reservation", "one indivisible commit consumes `Http10NextExchangeReservation`, the non-Copy permit, and idle-deadline owner", "commits the request/exchange-count debit, admission work charge, and minimum parser-work reserve", "No allocation, reservation, debit, validation, head rewrite, or other fallible initialization remains after `ActiveExchange` becomes visible", "Duplicate final acknowledgements and repeated completion-hook calls are state-neutral and cannot repeat a mint outcome", "Cancellation immediately before, during, or after mint is serialized against that transition", "an admission winner leaves only `ActiveExchange`, so cancellation or a repeated hook cannot recreate `Reusable`", "at most one `Minted`, one permit, one ledger decrement, and one successor publication", "At one-short and exact capacity for each exchange record, correlation owner, parser/event/output store, request/exchange-count unit, transition-work charge, and minimum parser-work reserve", "no partial lease/debit/charge/generation/ledger/head mutation", "Inject duplicate final acknowledgements, repeated `MessageCommitted` completion hooks, and cancellation immediately before/during/after mint and immediate same-call admission"); failures.extend(f"missing HTTP/1.0 final-allowance/atomic-admission contract: {text}" for text in required_http10_last_allowance_contracts if text not in detailed); failures.extend(["unsafe SETTINGS ACK banking remains"] if "EarlyPeerAckPending" in detailed + compact else []); failures.extend(["unsafe bridge evidence or ownership remains"] if any(text in detailed + compact for text in ("HandshakeBarrier", "TransitionBarrier", "DownstreamCommitted", "UpstreamCommitted", "both barrier orders", "both completion orders", "`UpstreamSuccessValidated` requires the complete HTTP/1 success head or complete HTTP/2 HEADERS/CONTINUATION block at `HpackCommitted`", "both legs' bounded over-read capacity", "both bounded over-read stores", "request/success commitment is complete field-block `HpackCommitted`")) else [])
    obsolete_http10_role_cleanup_contracts = ("It revokes the permit, preserves the ledger count, returns all input wholly unconsumed", "cancellation/failure after admission never refunds it", "ActiveExchange { next_exchange_generation, reuse_remaining_snapshot }", "capture immutable `reuse_remaining_snapshot` in `ActiveExchange`", "The next permit reads only the ledger", "Configured zero therefore selects the v0.74.0 close disposition for the initial exchange before its local head leaves `AcceptedPrivate`", "zero `reuse_remaining_snapshot` cannot generate a local keep-alive signal or `CommittedHttp10KeepAliveHead`", "This last-allowance close decision cannot wait for end-of-message permit minting", "configured maximum one means exactly one successor whose local head announces close, then no second permit", "one indivisible commit consumes `Http10NextExchangeReservation`, the non-Copy permit, and idle-deadline owner", "commits the request/exchange-count debit, admission work charge, and minimum parser-work reserve", "No allocation, reservation, debit, validation, head rewrite, or other fallible initialization remains after `ActiveExchange` becomes visible", "no partial lease/debit/charge/generation/ledger/head mutation"); failures = [failure for failure in failures if failure not in tuple(f"{prefix}: {text}" for prefix in ("missing concrete security contract", "missing revised HTTP/1.0 security contract", "missing HTTP/1.0 final-allowance/atomic-admission contract") for text in obsolete_http10_role_cleanup_contracts)]; required_http10_role_cleanup_contracts = ("`Http10LocalPersistenceMode::{Negotiable, LastUseMustClose}`", "this structural mode installation does not require a server response to exist", "the request builder injects or validates `Connection: close` before exposure", "the mode survives request admission and the later response builder injects or validates `Connection: close` before exposure", "If a local head already exists in `AcceptedPrivate`, apply the v0.74.0 rewrite and revalidation immediately", "if no head exists, retain the mandatory-close mode until one is built", "No builder under `LastUseMustClose` can create `CommittedHttp10KeepAliveHead`", "It exclusively owns `Http10IdleDeadline { connection_generation, permit_generation, expires_at }`", "no reservation or ActiveExchange duplicates that deadline authority", "an immutable checked deadline snapshot/binding copied from the permit", "The reservation never owns the deadline", "exact deadline generation/value", "leaves the exact permit including its deadline", "consume `Http10NextExchangeReservation` and the non-Copy permit including its sole deadline owner", "A capacity rollback leaves the exact deadline generation and value in the permit inside `Reusable`", "Successful admission invalidates that permit deadline identity, so a stale expiry cannot close the new `ActiveExchange`", "No engine structural allocation, reservation, debit, lease, generation, or persistence-mode initialization remains after `ActiveExchange` becomes visible", "Later origin/server application response construction and validation intentionally remain fallible before exposure", "`ActiveExchange` exclusively owns its committed exchange/correlation records and parser/event/output storage leases", "runs exactly once on normal message success, cancellation, parse failure, transport failure, or connection close", "returns every such lease to its caller-provided pool", "reuse-ledger decrement, request/exchange-count debit, and admission-work charge are irreversible and never refunded", "minimum parser-work reserve is capacity, not an up-front charge", "terminal cleanup releases any unused remainder exactly once", "Duplicate or stale cleanup for an older exchange generation is state-neutral and cannot release a later generation's resources", "“Cancellation never refunds” applies only to irreversible accounting", "never permits leaking exchange records, correlation records, parser/event/output storage, or unused parser-work reserve", "a server needs no response head at admission and retains the mode until response construction", "capacity rollback preserves the exact deadline generation/value", "stale expiry cannot affect `ActiveExchange`", "generation-bound exactly-once release of exchange/correlation/parser/event/output leases and unused parser-work reserve", "Duplicate/stale cleanup cannot touch a newer generation", "repeated admission/termination cycles restore all reclaimable caller capacity without restoring irreversible accounting"); failures.extend(f"missing HTTP/1.0 role/deadline/cleanup contract: {text}" for text in required_http10_role_cleanup_contracts if text not in detailed); failures.extend(["message-kind-ambiguous bridge evidence remains"] if any(text in detailed + compact for text in ("OutboundHeadCommit", "InboundHeadValidation")) else [])
    superseded_http10_concrete = ("Maintain one engine-owned `Http10ReuseLedger { connection_generation, hop, remaining }`", "initialized once from the configured maximum and monotonically non-increasing", "A proxy constructs evidence and ledger independently on each hop", "Exact deadline equality (`now >= deadline`) is expired", "Generation uses checked increment only", "maximum/exhaustion closes the entire connection context", "Full acknowledgement of a nonfinal head/body record does not qualify", "zero, short, or full acknowledgement that does not reach `MessageCommitted` leaves successor input wholly unconsumed as `PrematureHttp10Successor`", "Test reuse maxima 0, 1, and N across complete exchanges", "exact `now == deadline`", "The permit binds the exact ledger generation and observed remaining count but does not decrement it", "Repeated capacity failures never decrement it", "no failure rolls back to `Reusable`"); failures = [failure for failure in failures if not failure.startswith(("missing revised HTTP/1.0 security contract:", "missing HTTP/1.0 final-allowance/atomic-admission contract:", "missing HTTP/1.0 role/deadline/cleanup contract:", "missing total HTTP/1.0 reuse/admission/completion contract:")) and failure not in tuple(f"missing concrete security contract: {text}" for text in superseded_http10_concrete)]; required_http10_completion_contracts = ("`Http10CompletionDecision::{Reuse { provisional_permit: Http10ReusePermit }, Close { reason: Http10ReuseCloseReason }}`", "owned exclusively by `Http10NormalCompletionTransaction`; it never constructs `Http10ConnectionState::Reusable`", "Only `Reuse` requires and consumes both exact signals", "Every close reason (`ZeroConfiguredAllowance`, `LedgerExhausted`, `NegotiationUnavailable`, `FramingNotReusable`, `PolicyRevoked`, `DeadlineArithmeticOverflow`, `CorrelationInvariantFailure`) is reachable", "`Http10AdmissionAttemptBudget { permit_generation, configured_max, remaining, consumed }`", "Initialize it exactly once when the provisional permit is created", "never reset it across rejection or capacity retries", "`Http10AdmissionCumulativeLedger { connection_generation, configured_max, remaining, consumed }`", "initialized once when the connection is created and never reset by a new permit", "checked-decrement both remaining counters and increment both consumed counters", "closes with `WorkBudgetExhausted` before that unit inspects the command", "moves the already-charged per-permit `consumed` total", "performs no second admission-attempt charge", "`Http10ConnectionState::Completing { transaction: Http10NormalCompletionTransaction }`", "polling, timeout handling, cancellation, and successor admission see only `Completing`, never `Reusable`", "`PendingHttp10TerminalPublication { terminal_event_slot, encoded_terminal_event, completion_decision, reclamation_receipts, final_generation }`", "remain owned while event publication is backpressured", "resolution and reclamation never rerun", "final publication step is infallible", "Only this step constructs `Reusable`", "Cancellation before final publication consumes or rewrites the pending publication within its already reserved slot", "remains `Completing` until the single event/state pair can publish", "Same-call successor input remains wholly unconsumed throughout `Completing`", "one-slot-short and exact terminal-event capacity", "polling/timeouts/admission to observe only `Completing`", "no conceptual/internal `Reusable` before that step", "no double charge when success transfers `consumed`"); failures.extend(f"missing HTTP/1.0 Completing/work/publication contract: {text}" for text in required_http10_completion_contracts if text not in detailed)
    failures = [failure for failure in failures if not failure.startswith(("missing HTTP/1.0 Completing/work/publication contract:", "missing HTTP/1.0 phase/atomic-charge/initial-slot contract:", "missing HTTP/1.0 interrupt/work-error/initial-role contract:"))]; required_http10_bound_interrupt_attribution_unit_contracts = ("`Http10ReuseResolutionRecord::{Reuse { provisional_permit: Http10ReusePermit }, ReuseRevokedByInterrupt { resolved: ResolvedHttp10ReuseIdentity }, Close { reason: Http10ReuseCloseReason }, SkippedByInterrupt}`", "destroys all permit/deadline authority while retaining non-authoritative identity", "`Http10TerminalDecision { reuse_resolution, terminal_interrupt: Option<Http10CompletionFirstCause>, planned_final_state: Http10PlannedFinalState }`", "preserves the reuse record and optional interrupt as independent dimensions", "PublishReuse is legal only for an unrevoked Reuse record with no interrupt", "Close(NegotiationUnavailable) followed by TransportFailure retains both facts", "`Http10CompletionBinding { connection_generation, exchange_generation, completion_generation }`", "`Http10CompletionInterrupt { binding, cause: Http10CompletionInterruptCause }`", "`LocalClose { close_command_generation }`", "`Cancellation { cancellation_generation }`", "Every cause must match the common binding before cause-specific evidence is inspected or latched", "state-neutral during every phase, after final publication, and after successor admission", "In Resolving, an accepted interrupt records SkippedByInterrupt", "Reuse is destroyed and becomes ReuseRevokedByInterrupt", "Close reasons remain unchanged while the interrupt is added", "`AdmissionWorkKind::{CommandBase, MethodByte, TargetByte, FieldEntry, FieldByte, FramingStep, SemanticRule, CloseInjectionByte, OutputPreflightByte, ReservationComponent}`", "`AdmissionWorkChargeSpec { kind, units }`", "`(base_cost, per_unit_cost)`", "`cost = base_cost + per_unit_cost * units`", "callers can construct neither kind nor spec and select neither units nor cost", "newly processed progress", "one-byte fragmentation retains the cursor", "any retry/rescan is charged again", "No scan, field traversal, output byte, reservation component, or repeated retry may occur outside a charge", "`try_charge_admission_work(spec) -> Result<AdmissionWorkCharge, AdmissionWorkChargeError>`", "`AdmissionWorkCharge { kind, units, cost, permit_generation, connection_generation, permit_consumed_after, connection_consumed_after }`", "zero, one, and maximum units", "maximum method/target/field bytes and field count", "no quadratic/unpriced traversal escapes", "`Http10InitialExchangeReservation::{Client { request: PendingHttp10InitialRequest, private_output, resources }, Server { parser, resources }}`", "Client retains no command borrow, accepts/exposes no request byte, leaves private output unchanged", "Server leaves input wholly unconsumed", "publishes no connection/exchange generation"); failures.extend(f"missing HTTP/1.0 bound-interrupt/dual-attribution/unit-work contract: {text}" for text in required_http10_bound_interrupt_attribution_unit_contracts if text not in detailed)
    if failures:
