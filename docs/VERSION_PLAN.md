...
| Never-untracked HTTP/2 rejection state | storage at `0.117.0`; terminal state/section lease at `0.118.0`; block handoff at `0.120.0`/`0.123.0`; semantic owners at `0.125.0`–`0.131.0`; push at `0.145.0`; assembly at `0.181.0`; diagnostics at `0.193.0` | Retain policy, wire, reset, terminal stage, compression workspace, immutable field-section lease, closure, and block independently. Sealed transitions prevent aliasing, intermediate release, or impossible combinations; failure transfers cleanup without forgetting the ID. |
| Rejected-push terminal validation and reset arbitration | model at `0.118.0`; error/abort precedence at `0.121.0`; HPACK handoff at `0.123.0`; semantic pipeline at `0.125.0`–`0.131.0`; credit at `0.136.0`; reset lifecycle at `0.138.0`; total table at `0.145.0`; assembly at `0.181.0`; diagnostics/fuzz at `0.193.0`/`0.196.0` | END_STREAM makes policy CANCEL dormant through PendingFieldBlock and every PendingSemantics stage. Only the final owner releases it; malformed re-arms it; peer reset aborts after required HPACK drain; fatal failure transfers cleanup; GOAWAY alone preserves validation; begun reset bytes remain unique and immutable. |
...
| `0.117.0` | Generation-checked stream table and tombstones | Requires HTTP/2 stream-identifier rules; retains reset, terminal stage, compression workspace, field-section lease, closure, and block ownership independently; Unlocks Exhaustive stream-state graph. |
...
| `0.120.0` | HTTP/2 frame legality and fragmented-header-block sequencing | Requires HTTP/2 activation preface, first-SETTINGS, and deadline sequencing; crosses terminal state/stage with reset, END flags, closure, and active block ownership; Unlocks HTTP/2 error scope, typed deltas, and isolated stream mutation. |
| `0.121.0` | HTTP/2 error scope, typed deltas, and isolated stream mutation | Requires HTTP/2 frame legality and fragmented-header-block sequencing; orders fatal cleanup, stream error, immutable output, and peer-reset abort while preserving mandatory HPACK synchronization and prohibiting post-abort publication; Unlocks HTTP/2 graceful GOAWAY and bounded shutdown sequencing. |
...
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; defines exactly-once section transfer/release/shutdown ownership across valid, malformed, peer reset, GOAWAY, and fatal failure; Unlocks SETTINGS outstanding-ACK accounting. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.138.0 foundations; completes the table across compression/section ownership and capacity, stages, table mutation, caller reuse, abort/cleanup, reset arbitration, and output offset; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.181.0` | Client correlation, cancellation, and retry tokens | Requires Partial-response media-type classification plus v0.145.0 push admission; independently preflights push/section resources and inherits immutable section transfer, abort cleanup, provenance, and reset arbitration; Unlocks Streaming partial-response and retained-prefix validation. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports section lease generation/disposition and cleanup owner without contents while preserving sensitivity/never-index redaction and caller-scrub policy; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts stage × section capacity/lease × dynamic eviction/table update × caller reuse × abort/failure and proves exact bytes/provenance plus single ownership disposition; Unlocks Compile-fail state and lifetime tests. |
