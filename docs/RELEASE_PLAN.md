...
- Define sealed engine-only `AdmissionWorkKind::{CommandShape, Method, Target, Field, Framing, Semantics, CloseInjection, OutputPreflight, ReservationPreflight}` with one fixed positive checked cost per variant. Callers cannot construct a kind, select a numeric cost, or underprice work. The sole work-authority edge is atomic `try_charge_admission_work(kind) -> Result<AdmissionWorkCharge, AdmissionWorkChargeError>`, where `AdmissionWorkChargeError::{PermitBudgetExhausted, ConnectionBudgetExhausted, InvalidZeroCost, GenerationMismatch, LedgerInvariantFailure, ArithmeticOverflow}` remains fully typed. Apply deterministic error precedence: invalid/zero kind cost, generation mismatch, broken ledger equation, arithmetic overflow, permit exhaustion, then connection exhaustion. The first two exhaustion variants are ordinary local resource closure; zero cost, mismatch, invariant failure, and overflow are local security-invariant closure.
- The operation derives cost internally, preflights both remaining counters, both consumed additions, generations, and `configured_max == remaining + consumed` before mutation. Only complete success atomically updates both ledgers by the entire cost. Every error preserves both ledgers, returns no charge record, and performs no governed work. Non-Copy `AdmissionWorkCharge { kind, permit_generation, connection_generation, cost, permit_consumed_after, connection_consumed_after }` is linear diagnostic evidence consumed by the governed unit, never authorization. Rejection/capacity retains the modified permit budget; successful admission transfers the charged total without recommit/recharge.
- Before transition, transactionally assemble role-specific `Http10NextExchangeReservation::{Client { request: PendingHttp10SuccessorRequest, private_output, correlation, resources }, Server { parser, local_persistence_mode, correlation, resources }}`. Shared resources include records, parser/event/output leases, tentative count debit, parser reserve, deadline snapshot, and generation. The event lease covers the largest terminal event. Initial construction uses `Http10InitialExchangeReservation::{Client { request: PendingHttp10InitialRequest, private_output, resources }, Server { parser, resources }}` and reserves the identical slot before initial Active publication, including configured-zero/default-close.
- Initial failure is role-total: Client retains no command borrow, accepts/exposes no request byte, leaves private output unchanged, and publishes no connection/exchange generation; Server leaves input wholly unconsumed and likewise publishes no generation. Both release every tentative record, lease, count/work reservation, event slot, and connection-ledger owner exactly once. Every reservation is all-or-nothing; one-short capacity preserves preexisting state apart from already atomically charged work. Successor construction also validates policy, deadline/binding, ledger count, and checked generation; terminal reasons close locally without peer blame.
...
- `ActiveExchange` exclusively owns its committed exchange/correlation/evidence records and parser/event/output leases. Normal success enters `Completing` with total internal phases `Http10CompletingPhase::{Resolving, DecisionHeld { decision }, Reclaiming { terminal_decision, reclamation_state }, PublicationPending { publication }}`. Define sealed `Http10CompletionInterrupt::{IdleDeadlineReached { connection_generation, permit_generation, observed_now }, PolicyRevoked { policy_generation }, TransportFailure { transport_generation, cause }, LocalClose, Cancellation}` plus one immutable generation-bound `Http10CompletionFirstCause` latch. The first valid accepted interrupt wins permanently; if multiple valid interrupts arrive in one engine call, deterministic tie precedence is TransportFailure, PolicyRevoked, IdleDeadlineReached, LocalClose, then Cancellation. Later interrupts cannot replace attribution. Wrong/stale generation or deadline identity is state-neutral; exact `observed_now >= expires_at` expires authority.
- Interrupt handling is total and capacity-infallible for every variant in every phase. In Resolving, skip/terminate reuse resolution and select close. In DecisionHeld, revoke any provisional permit and replace reuse with first-cause close. In Reclaiming, retain receipts, revoke provisional reuse, and continue only unreclaimed items. In PublicationPending, rewrite the reserved event/next state in place. Every interrupt remains Completing through backpressure, uses the existing slot, never reruns resolution/reclamation, and never reacquires capacity. Before final publication, expiry or policy revocation can never leave reusable authority; transport/local-close/cancellation behave identically for ownership while retaining their distinct first-cause event. Only final infallible pending-publication consumption constructs Reusable/close.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. For engine-derived `AdmissionWorkKind`, test every fixed schedule entry and prohibit caller-selected costs. Cross permit-only exhaustion, connection-only exhaustion, both exhausted, invalid zero schedule, generation mismatch, each ledger-equation corruption, every addition/subtraction overflow, and fixed error precedence. Every typed failure preserves both ledgers and performs no work; only success returns one diagnostic charge.
- Cross all five completion interrupts singly and in every same-call/later-call order at each Resolving, DecisionHeld, partial-Reclaiming, PublicationPending, and backpressure point. Require immutable first cause/tie precedence, exact-deadline expiry, stale/wrong-generation neutrality, provisional-permit revocation, receipt-guided cleanup, no rerun/capacity acquisition, and no expired/revoked Reusable publication. For both initial reservation roles at one-short/exact component capacity, prove client no-borrow/no-byte/output-unchanged and server wholly-unconsumed input, no generation publication, and exactly-once release of every tentative lease/ledger owner.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance extension: Audit v0.75.0 as one causal product: all five sealed completion interrupts in every phase with immutable first cause, fixed same-call precedence, exact-deadline expiry, and stale neutrality; engine-only fixed-price `AdmissionWorkKind` with six typed charge errors; and role-specific initial Client/Server failure ownership. Treat revoked/expired Reusable, interrupt cause replacement, caller-priced work, invariant-error collapse, retained client borrow, consumed server input, changed client output, leaked tentative owner, or failed generation publication as release-blocking.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete `Connection` corpus and exhaust the HTTP/1.0 role/direction, correlation, framing, output, and ledger products. Cross all completion interrupt variants/orders/ties/stale bindings in every phase; every engine work kind and typed error; and Client/Server initial reservation failure at every component. Require immutable first cause, no expired/revoked Reusable, exact typed classification, no governed work on charge error, no caller ownership loss or failed generation, one resolution/reclamation/publication, and no capacity reacquisition. Preserve all HTTP/1 lifecycle/FIFO/early-final/CONNECT/Upgrade/WebSocket and v0.73.0 rejection corpora.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- HTTP/1.0 API extension: expose neither completion-interrupt construction/first-cause mutation, work-kind construction/numeric cost/error collapse, nor role-specific initial reservation ownership. Callers cannot forge stale-valid bindings, underprice work, downgrade invariants to exhaustion, retain client command borrows, consume server input, mutate client output, or publish a generation on initial failure.
...
- HTTP/1.0 diagnostics extension: report immutable first completion interrupt, ignored later/stale interrupts, same-call tie winner, phase, engine-derived work kind/cost, exact `AdmissionWorkChargeError`, and initial role/rollback receipts. Never expose interrupt, pricing, charge, or reservation authority.
...
- HTTP/1.0 fuzz extension: cross all five interrupts in every order/tie and phase, exact/stale/wrong-generation deadlines, policy and transport generations, all engine work kinds and six typed error paths, and both initial roles at each capacity boundary. Assert immutable attribution, no revoked reuse/error collapse/governed failure work, no retained borrow/input/output mutation, exact tentative release, and no failed generation.
...
- HTTP/1.0 compile-fail extension: reject interrupt/first-cause/work-kind/cost/error construction or rebinding, numeric charge entry points, typed-error erasure, role-swapped initial reservation, retained client borrow, server input consumption, client output mutation, and extraction of tentative owners or generation on failure.
...
- HTTP/1.0 soak extension: cycle all interrupt causes/races through every phase/backpressure point, every typed work failure across repeated permits, and Client/Server initial failures across capacity high-water marks. Require stable first cause, no expired/revoked reuse, error collapse, ownership leak, failed generation, or capacity drift.
...
- HTTP/1.0 role extension: neither role may forge completion interrupts/bindings, select work kind/cost, collapse typed errors, cross initial reservation variants, or retain/consume/mutate client command, server input, or client output on failure. Pentest stale timeout, policy/transport races, underpricing, invariant downgrade, and initial-owner theft.
...
- HTTP/1.0 sizing extension: independently bound five interrupts, first-cause latch and tie resolution in all phases, every fixed work-kind cost and six typed errors, and Client/Server initial reservation plus rollback receipts. Duplicate interrupt/cause/charge/command/input/output/slot/ledger/generation ownership is forbidden.
