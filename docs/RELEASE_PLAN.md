...
- Define total reuse resolution after both message lifecycles are terminal over optional generation-bound evidence; it is not preconditioned on successful negotiation. Apply one documented precedence: correlation integrity, policy, framing, configured-zero, exhausted ledger, missing exact two-sided negotiation, checked deadline arithmetic, then reuse. Resolution returns unpublished `Http10CompletionDecision::{Reuse { provisional_permit: Http10ReusePermit }, Close { reason: Http10ReuseCloseReason }}` owned exclusively by `Http10NormalCompletionTransaction`; it never constructs `Http10ConnectionState::Reusable`. Only `Reuse` requires and consumes both exact signals. Client/proxy-upstream resolution consumes correlated committed-request plus most-recent validated response evidence; origin/server resolution consumes validated request plus committed response evidence. The provisional permit binds the exact ledger generation and observed remaining count without decrementing it and exclusively owns `Http10IdleDeadline { connection_generation, permit_generation, expires_at }`; no transaction snapshot, reservation, or ActiveExchange duplicates that authority. Every close reason (`ZeroConfiguredAllowance`, `LedgerExhausted`, `NegotiationUnavailable`, `FramingNotReusable`, `PolicyRevoked`, `DeadlineArithmeticOverflow`, `CorrelationInvariantFailure`) is reachable, creates no permit, leaves same-call successor input wholly unconsumed, and closes without a peer protocol error. A proxy resolves evidence and ledger independently on each hop; received keep-alive evidence is never forwarded or rebound.
...
- Define the sole successor-admission edge as `Http10ConnectionState::Reusable { permit: Http10ReusePermit } -> ActiveExchange { next_exchange_generation, reuse_remaining_snapshot, local_persistence_mode, admission_attempt_work_consumed }`. Its total result is `Http10SuccessorAdmissionOutcome::{Admitted, RejectedLocalCommand { reason: Http10LocalCommandReason }, RetryableCapacity { reason: Http10CapacityReason }, CloseLocal { reason }}`. `RejectedLocalCommand` covers invalid method/target/ordered fields, illegal framing, semantic invalidity, and conflicting caller `Connection`; it retains no caller borrow, accepts/exposes no command byte, preserves the permit/deadline/ledger/output/structural state in `Reusable`, and permits a corrected command before the same absolute deadline. `RetryableCapacity` likewise carries only a freely constructible reason.
- Store non-Copy `Http10AdmissionAttemptBudget { permit_generation, configured_max, remaining, consumed }` inside each provisional/published permit. Initialize it exactly once when the provisional permit is created, transfer it unchanged into `Reusable`, and never reset it across rejection or capacity retries. Also maintain engine-owned connection-lifetime `Http10AdmissionCumulativeLedger { connection_generation, configured_max, remaining, consumed }`, initialized once when the connection is created and never reset by a new permit. Before every governed validation/preflight unit, checked-decrement both remaining counters and increment both consumed counters; insufficient allowance in either closes with `WorkBudgetExhausted` before that unit inspects the command. Charges are monotonic and irreversible. Rejection/capacity changes only these two ledgers and retains the modified permit budget. Pure close-injection/output-capacity failure is retryable after charging only work already performed. Successful admission moves the already-charged per-permit `consumed` total into immutable Active diagnostics/accounting and performs no second admission-attempt charge.
- Before transition, transactionally assemble one internal non-Copy/non-Clone role-specific `Http10NextExchangeReservation::{Client { request: PendingHttp10SuccessorRequest, private_output, correlation, resources }, Server { parser, local_persistence_mode, correlation, resources }}`. Shared resources include the next-exchange record, parser/event/output storage leases, one request/exchange-count debit, the minimum parser-work reserve needed to reach safe backpressure, immutable checked permit-deadline snapshot/binding, and checked next generation; the reservation never owns the deadline. The event lease is sized and reserved for the largest normal or cancellation terminal event so either can be encoded/replaced without later capacity acquisition. Construction validates current policy, `now < permit.deadline.expires_at`, exact deadline generation/value, and permit-to-ledger binding/count. It is all-or-nothing: every one-short storage, queue, count, or work failure releases incomplete ownership and leaves the exact permit/deadline, input, ledger, generation, and output unchanged apart from already charged attempt work. `CloseLocal` covers `RequestCountExhausted`, `WorkBudgetExhausted`, `PolicyRevoked`, `IdleDeadlineReached`, `PermitLedgerMismatch`, `CorrelationInvariantFailure`, and `GenerationExhausted`; every reason is local policy/resource/invariant state, revokes the permit/deadline, preserves the ledger count, returns input wholly unconsumed, and closes without blaming the peer. Exact deadline equality is expired and checked generation exhaustion never wraps.
- On `Admitted`, atomically: consume `Http10NextExchangeReservation` and the non-Copy permit including its sole deadline owner and attempt budget; acquire the exchange/correlation records; install parser/event/output leases; commit the request/exchange-count debit and minimum parser-work reserve; transfer the already-charged attempt total without charging it again; checked-decrement `Http10ReuseLedger.remaining` exactly once; capture immutable `reuse_remaining_snapshot`; install the fresh generation and selected persistence mode; and only then publish `ActiveExchange`. Capacity rollback leaves the exact deadline and modified attempt budget in the permit inside `Reusable`. Successful admission invalidates that deadline identity. No engine structural allocation or initialization remains after publication; later server response construction stays fallible before exposure. Only the reuse ledger authorizes later reuse, while both attempt ledgers remain monotonic accounting. Client output and server input cannot precede admission.
- `ActiveExchange` exclusively owns its committed exchange/correlation records, negotiation evidence, and parser/event/output storage leases. Non-normal cancellation, parse failure, transport failure, or connection close uses generation-bound non-Copy cleanup exactly once. Normal success atomically enters `Http10ConnectionState::Completing { transaction: Http10NormalCompletionTransaction }`; polling, timeout handling, cancellation, and successor admission see only `Completing`, never `Reusable`. The transaction verifies both lifecycles; resolves reuse while correlation/evidence remain owned; stores the unpublished `Http10CompletionDecision`; releases every non-transferred exchange/correlation/parser/output lease and unused parser reserve; and transfers the reserved event lease into linear `PendingHttp10TerminalPublication { terminal_event_slot, encoded_terminal_event, completion_decision, reclamation_receipts, final_generation }`. The event slot, decision, receipts, and generation remain owned while event publication is backpressured; resolution and reclamation never rerun. Once this object exists, the final publication step is infallible and atomically consumes it to publish exactly one terminal event and construct either `Http10ConnectionState::Reusable { permit }` or closing state. Only this step constructs `Reusable`.
- Cancellation before final publication consumes or rewrites the pending publication within its already reserved slot, revokes any provisional permit, selects terminal close, and remains `Completing` until the single event/state pair can publish; it never exposes reuse or reacquires capacity. Cancellation after publication acts only on the published state. Duplicate hooks are neutral. Ledger/count/admission-attempt/consumed-parser-work charges never refund, while all non-transferred caller storage and unused reserve return once; stale cleanup cannot affect a newer generation.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Exhaust every role/evidence/lifecycle/framing/policy/generation combination and independently reach each ordered close reason. For client admission test malformed/overlong syntax, fields, framing, semantics, conflicting `Connection`, corrected retry, and close-injection capacity. Across invalid/capacity retry sequences prove exact checked charge-before-work against both `Http10AdmissionAttemptBudget` and connection cumulative ledger, no reset on the same permit, a fresh bounded permit budget but unchanged cumulative ledger on later permits, pre-inspection exhaustion, unchanged deadline, and no double charge when success transfers `consumed`. Test one-short/exact capacity for each reservation component.
- Inject cancellation/duplicate hooks before every completion step and while terminal publication is backpressured. At one-slot-short and exact terminal-event capacity, require the slot to transfer into `PendingHttp10TerminalPublication`, polling/timeouts/admission to observe only `Completing`, resolution/reclamation not to rerun, same-call input to stay unconsumed, cancellation to rewrite using held capacity and revoke provisional reuse, and final publication to infallibly emit exactly one event/state pair. Require no conceptual/internal `Reusable` before that step. Also test stale cleanup/expiry, bodyless/fixed positive paths, negative framing, and rejected CONNECT cross-products.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance extension: Audit v0.75.0 as one causal product: total resolution to unpublished `Http10CompletionDecision`; `Completing` as the only prepublication connection state; per-permit `Http10AdmissionAttemptBudget` plus connection-lifetime cumulative ledger with checked charge-before-work and transfer-without-double-charge; and `PendingHttp10TerminalPublication` retaining the event slot, encoded event, decision, receipts, and generation through backpressure. Treat early Reusable, reset/free/double work, released/reacquired slot, rerun resolution/reclamation, visible provisional authority, split event/state publication, or duplicate terminal event as release-blocking.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete `Connection` corpus under both versions through persistence and stripping; exhaust the HTTP/1.0 role/direction, correlation, signal-order, framing, output-phase, and ledger-maxima products. Audit every close decision, validated client-command rejection, role reservation boundary, permit deadline, per-permit/cumulative work charge, Completing transition, terminal-event slot transfer, publication-backpressure state, cancellation rewrite, reclamation receipt, and final atomic event/state publication. Require charge-before-work, no reset or double charge, one resolution/reclamation execution, one decision/permit/decrement/reservation/terminal event/successor, and no Reusable before final publication. Preserve all HTTP/1 lifecycle/FIFO/early-final/CONNECT/Upgrade/WebSocket and v0.73.0 capacity/start-line/414/431/400/501 precedence corpora, including exact rejection output and acknowledgement boundaries.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- HTTP/1.0 API extension: expose neither construction nor borrowing of `Http10CompletionDecision`, `Completing` transaction, either attempt-work ledger, provisional permit, event slot, pending terminal publication, nor reclamation receipts. Public polling reports Completing until one atomic event/state publication; callers cannot reset budgets, double-charge success, reacquire publication capacity, or drive resolution/reclamation twice.
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  lifecycle-bound exactly-once Completing state, `Http10CompletionDecision`,
...
- HTTP/1.0 diagnostics extension: report resolution precedence, Completing phase, per-permit and cumulative work totals before/after charge, transferred consumed total, pending-publication backpressure, cancellation rewrite, reclamation receipts, and final generation. Never include contents or provisional authority; emit only the single final terminal event after atomic state publication.
...
- HTTP/1.0 fuzz extension: generate per-permit and cumulative work exhaustion over invalid/capacity retries and successive permits, successful transfer without a second charge, one-short/exact terminal-event slots, Completing polling/timeouts/admission, publication backpressure, and cancellation rewrite at every completion step. Assert no early Reusable, reset/free/double work, slot loss/reacquisition, rerun resolution/reclamation, visible provisional authority, or duplicate event/state publication.
...
- HTTP/1.0 compile-fail extension: reject construction, copying, extraction, retention, or cross-generation rebinding of completion decisions, Completing transactions, either attempt ledger, provisional permits, terminal-event slots, pending publications, and reclamation receipts. Also reject budget reset/double-charge APIs, early Reusable construction, capacity reacquisition, rerun completion, and separate event/state publication.
...
- HTTP/1.0 soak extension: cycle invalid/corrected/capacity retries across many permits until both work ledgers exhaust, then cycle terminal-event backpressure and cancellation rewrites across generation/capacity high-water marks. Require no reset or double charge, no event-slot/decision/receipt leak, no rerun or early Reusable, one terminal event/state, exact capacity restoration, and no accounting refund.
...
- HTTP/1.0 role extension: neither client nor server may observe or retain the unpublished decision, Completing transaction, attempt ledgers, provisional permit, event slot, pending publication, or receipts. Pentest retry and publication for budget reset/double charge, free-work amplification across permits, early reuse observation, slot theft/reacquisition, rerun completion, split event/state, and cancellation-created reuse.
...
- HTTP/1.0 sizing extension: independently bound every completion decision, Completing transaction, per-permit attempt budget, connection cumulative ledger, pending terminal publication, reserved event slot, encoded event, reclamation receipts, backpressure state, and cancellation rewrite. Include invalid retries across repeated permits and one-short/exact publication capacity; duplicate decision, permit, deadline, slot, receipt, cleanup, or storage ownership is forbidden.
...
- Independently size each configured early-final request's framing/progress/
  ...
  `Http10PersistenceDisposition`/`ValidatedHttp10KeepAlive`/
  `CommittedHttp10KeepAliveHead`/`CorrelatedHttp10KeepAliveRequest`/
  `Http10ReuseLedger`/`Http10CompletionDecision`/`Http10ReusePermit`, newest-
