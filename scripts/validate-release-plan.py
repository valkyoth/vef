...
def main() -> int:
    ...
    failures = [failure for failure in failures if not failure.startswith("missing HTTP/1.0 Completing/work/publication contract:")]; required_http10_phase_charge_contracts = ("`try_charge_admission_work(cost) -> Result<AdmissionWorkCharge, WorkBudgetExhausted>`", "Preflight nonzero `cost`, both remaining counters, both consumed additions, generations, and both ledger invariants before mutation", "one indivisible commit decrements both remaining counters and increments both consumed counters by the entire cost", "Failure mutates neither ledger, performs no governed work", "`AdmissionWorkCharge { permit_generation, connection_generation, cost, permit_consumed_after, connection_consumed_after }`", "linear diagnostic/accounting evidence consumed by the governed unit, never authorization", "never recommits or recharges attempt work", "`ActiveExchange { next_exchange_generation, reuse_remaining_snapshot, local_persistence_mode, admission_attempt_work_consumed }`", "`Http10InitialExchangeReservation`", "reserve the identical maximum-size terminal-event lease before publishing the initial `ActiveExchange`", "including configured-zero/default-close mode", "consumes no peer input and publishes no exchange", "`Http10CompletingPhase::{Resolving, DecisionHeld { decision }, Reclaiming { terminal_decision, reclamation_state }, PublicationPending { publication: PendingHttp10TerminalPublication }}`", "In `Resolving`, it skips reuse resolution", "In `DecisionHeld`, it revokes any provisional permit", "In `Reclaiming`, it revokes provisional reuse if still held", "continues only unreclaimed items from the receipts", "In `PublicationPending`, it rewrites the held maximum-size event slot and next state in place", "Cancellation is total and capacity-infallible in every phase", "never reacquires terminal capacity", "cost zero/one/greater-than-one", "insufficient either side", "consumed-add overflow", "failure mutates neither and performs no governed work", "initial `Http10InitialExchangeReservation` at one-slot-short/exact event capacity"); failures.extend(f"missing HTTP/1.0 phase/atomic-charge/initial-slot contract: {text}" for text in required_http10_phase_charge_contracts if text not in detailed)
    if failures:
