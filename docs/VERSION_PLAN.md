...
| HTTP/2 DATA and command ownership | inbound at `0.130.0`/`0.136.0`; outbound ledger/commands at `0.133.0`–`0.135.0`/`0.137.0`–`0.143.0` | Inbound framing/credit precedes END_STREAM closure. Outbound DATA atomically reserves both windows, commits at exposure, never refunds frozen debit, and preserves whole-frame/reset ordering under settings, cancellation, and contention. |
...
| `0.133.0` | Stream flow control | Requires Cookie field combination and Set-Cookie preservation; defines signed outbound available/reserved-unexposed/committed-debited stream credit and negative-window blocking; Unlocks Connection flow control. |
| `0.134.0` | Connection flow control | Requires Stream flow control; atomically pairs exact DATA reservations across stream and connection so queued contenders cannot oversubscribe; Unlocks SETTINGS initial-window active-stream integration and atomic rollback. |
...
| `0.136.0` | HTTP/2 inbound DATA ownership, acknowledgement, and credit release | Requires SETTINGS initial-window active-stream integration and atomic rollback; keeps reserved(remote) DATA a connection error through reset prefix 12, credits half-closed(local) DATA exactly, and uses connection-only post-reset credit only after actual closure; Unlocks HTTP/2 outbound per-stream message command lifecycle. |
...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; serializes ACK-gated transitions, debt merge/lease/rollback/first-exposure transfer, post-exposure next-block debt, and exact field-block completion; Unlocks SETTINGS max-frame-size outbound integration. |
...
| `0.153.0` | Reserved control-output queues | Requires WINDOW_UPDATE churn defenses; commits per-frame acknowledged-ceiling snapshots at byte nine and merges optional selected deltas without clearing debt or conflating ACK/ceiling/debt ownership; Unlocks HTTP/2 conformance audit and pentest. |
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits four-limit bounds, immediate reduction/no automatic increase, selected-only debt, initial/local changes, decoder storage, and every prior ACK/debt/block boundary; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.191.0` | Fixed-capacity caller-storage public API | Requires Authenticated origin authorization and HTTP/2 coalescing metadata; validates table physical capacity from exclusive caller storage and seals ceiling/selection/ACK/debt mutation authority; Unlocks Optional alloc-backed convenience API. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports received/acknowledged/selected/physical limits separately plus redacted transition/debt ownership without header contents; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; crosses physical arenas, peer 0/4096/u32::MAX, local policy changes, initial reductions, ACK offsets, and debt/block boundaries while proving table equivalence; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects forged ceiling acknowledgements, out-of-bound selection, raw-ceiling debt, unsafe decoder advertisement, and prior debt/transaction/token violations; Unlocks Long-running soak and exhaustion campaign. |
...
| `0.211.0` | Deterministic CPU, stack, code-size, and amplification budgets | Requires Aesynx kernel integration tests; measures encoder/decoder physical layouts, four-limit state, per-frame snapshots/selected deltas, debt leases, FIFO prefixes, and fatal cleanup; Unlocks 32-bit target campaign. |
