...
| HTTP/2 push admission and assembly binding | lifecycle at `0.145.0`; handle gate at `0.181.0`; partial/full consumption at `0.181.1`–`0.181.2`; stabilize at `0.191.0`; compile-fail at `0.197.0` | First require complete safe/cacheable/content-free promised requests, trusted authority, separate slot/work limits, and rollback-capable prepublication admission. Once assembly exists, finish PUSH_PROMISE/CONTINUATION and HPACK/semantic checks, derive target from the promised request and security namespaces only from the associated local request/caller policy, then atomically reserve slot plus invalidation handle. Capacity rejection publishes nothing, preserves HPACK and the associated stream, rolls back exactly once, and emits mandatory-control stream-local CANCEL. Accepted pushed 200/206 uses the same exact correlation and invalidation machinery as ordinary responses. |
...
| Linear, bounded full-200 fallback invalidation | reserve at `0.181.0`; evidence at `0.181.1`; consume at `0.181.2`; stabilize at `0.191.0`; compile-fail at `0.197.0` | Before local request output, or after complete pushed-request validation but before publication, reserve a non-Copy/non-Clone exact-correlation handle from isolated per-shard capacity. Local exhaustion backpressures with zero bytes/no correlation; push exhaustion synchronizes HPACK, rolls back slot/handle, publishes nothing, and rejects only the promised stream. Hold through ordinary or pushed informational/final states and terminal backpressure, release once, and independently reserve for retries. Keyed/unkeyed replacement is identical; arena rotation remains corruption/caller-policy only. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff and retry classification; creates rollback-capable promised-request admission with validated trusted provenance for the later v0.181.0 handle gate; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.181.0` | Client correlation, cancellation, and retry tokens | Requires Partial-response media-type classification plus v0.145.0 push admission; reserves one linear handle before local output or atomically with a validated promised slot before push publication, with synchronized stream-local push rejection on capacity; Unlocks Streaming partial-response and retained-prefix validation. |
| `0.181.1` | Streaming partial-response and retained-prefix validation | Requires the exact ordinary or promised correlation and its held invalidation handle; pushed 206 cannot retain/assemble without both, and pushed/ordinary 200 enters identical keyed/unkeyed evidence; Unlocks Cross-request partial assembly and header synthesis. |
| `0.181.2` | Cross-request partial assembly and header synthesis | Requires Streaming partial-response and retained-prefix validation; consumes ordinary/pushed fallback evidence with namespace-bounded invalidation and borrow-aware reclamation, while rotation is corruption/caller-policy only; Unlocks Retry safety, idempotency, and body-replayability contract. |
