...
| Outbound field-block commitment | HEADERS ownership at `0.137.0`; reset arbitration at `0.138.0`; scheduler obligation at `0.142.0`; PUSH_PROMISE reuse at `0.145.0`; reserved controls at `0.153.0`; audit/diagnostics/fuzz at `0.154.0`/`0.193.0`/`0.196.0` | The whole block may roll back only while Private. First initial-frame exposure creates one connection-scoped Committed obligation through END_HEADERS; individual CONTINUATION disposition cannot permit supersession or interleaving, transport failure abandons only with the connection, and the initial END_STREAM hook remains frame-scoped. |
...
| Bounded outbound DATA staging | resource basis at `0.8.0`; concrete arena at `0.137.0`; segmentation at `0.143.0`; audit/public API at `0.154.0`/`0.191.0`; diagnostics/fuzz/compile-fail at `0.193.0`/`0.196.0`/`0.197.0`; resource replay at `0.211.0` | Copy each exact frame into an exclusive caller-provided arena slot. A local nonzero payload cap is independent of peer MAX_FRAME_SIZE, source data may release after copy, staged bytes cannot alias or recycle early, byte and entry capacities are separate, and exhaustion is local backpressure. |
...
| `0.20.0` | Minimal synchronous I/O contracts | Requires Role, profile, and policy types; separates bytes offered from transport-acknowledged prefixes and zero/short writes; Unlocks Runtime-neutral readiness and poll contracts. |
...
| `0.25.0` | Engine event, command, acknowledgement, and publication contract | Requires Deterministic fake transport and driver harness; makes first non-empty exposure irrevocable and defines single-use generation-token exact-prefix acknowledgement plus reclamation; Unlocks Requirement, applicability, and errata evidence system. |
...
| `0.137.0` | HTTP/2 outbound per-stream message command lifecycle | Requires inbound ownership plus both send ledgers; introduces exclusive `OutboundFrameArena` slots and a local payload cap, defines ordinary disposition and exact DATA reservation→debit, and makes first initial HEADERS exposure commit its entire field block through END_HEADERS while the initial END_STREAM hook stays frame-scoped; Unlocks HTTP/2 body cancellation, reset, and flow-credit lifecycle. |
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; may supersede a whole Private field block or unrelated AcceptedPrivate output, never an individual committed CONTINUATION, finishes committed block/Frozen suffix before reset, and preserves reset offset/closure rules; Unlocks SETTINGS outstanding-ACK accounting. |
...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; treats a committed field block as the highest framing obligation through END_HEADERS, reserves controls while they wait, and otherwise preserves Frozen suffix/hook ownership; Unlocks SETTINGS max-frame-size outbound integration. |
| `0.143.0` | SETTINGS max-frame-size outbound integration | Requires Bounded outbound scheduling; segments DATA by the minimum peer limit, local payload cap, both credits, and remainder before arena allocation, with independent byte/entry backpressure and no peer-sized allocation; Unlocks GOAWAY cutoff and retry classification. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.142.0 foundations; reuses connection-scoped Private/Committed field-block ownership for outbound PUSH_PROMISE and completes the rejected-push table across every frame/reset/lease/failure race; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.153.0` | Reserved control-output queues | Requires WINDOW_UPDATE churn defenses; keeps required control capacity reserved but serialization blocked behind a committed field block through END_HEADERS; Unlocks HTTP/2 conformance audit and pentest. |
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits whole Private/Committed HEADERS and PUSH_PROMISE blocks, every CONTINUATION/control/reset prefix, independent END_STREAM completion, exact arena lifetimes, local payload caps, independent byte/entry exhaustion, and atomic DATA credit; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.191.0` | Fixed-capacity caller-storage public API | Requires Authenticated origin authorization and HTTP/2 coalescing metadata; exposes exclusive `OutboundFrameArena` construction, slot lifetimes, local payload sizing, and typed byte/entry capacity without allocator or peer-sized storage; Unlocks Optional alloc-backed convenience API. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; distinguishes field-block/ordinary disposition, frame hook/reset ordering, arena slot/byte/entry capacity, and available/reserved/committed DATA credit without payload disclosure or false completion; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts Private/Committed field blocks, ordinary/reset output, arena slot/byte/entry lifetimes, local/peer segmentation limits, and dual-credit ledgers across every exposure/control/failure/generation race; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects OutputToken/arena-slot duplication or rebinding, staged-byte access/reuse while owned, frozen substitution, invalid acknowledgement, and early stream/tombstone reuse; Unlocks Long-running soak and exhaustion campaign. |
...
| `0.211.0` | Deterministic CPU, stack, code-size, and amplification budgets | Requires Aesynx kernel integration tests; replays outbound arena sizing, local payload caps, peer-maximum framing, and independent slot-byte/entry exhaustion within every resource profile; Unlocks 32-bit target campaign. |
