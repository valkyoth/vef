...
def main() -> int:
    ...
    required_contract_text = (
        ...
        "separate bounded reserved-push-slot/work budget before reservation",
        ...
        "successfully completed, framing- and semantics-valid correlated 200",
        ...
        "A full union yields complete 200 with corrected Content-Length", "`ConflictingPartialContent`", "`RequestedOverlapBudget`", "`ActiveOverlapBudget`", "before sorting, output acquisition, or comparison", "cannot reset, clone, enlarge, or be replaced", "A 304, successful revalidation retaining the conflicting validator", "A new navigation or variant-selection identity creates a separate context", "`FullRepresentationFallback::{Keyed, Unkeyed}`", "before serializing any request byte or creating an assembly-participating correlation", "engine-only fixed per-shard reserve that Vary fields", "typed local `AssemblyInvalidationCapacity`, emits zero request bytes, creates no correlation, never rotates or clears an existing arena", "ordinary Sans-I/O backpressure until capacity is released", "cannot continue as NoRecombine", "mandatory caller-supplied principal/tenant shard identity", "linear non-Copy/non-Clone capability bound to exactly one correlation", "terminal-event backpressure", "release it exactly once", "A retry cannot duplicate or rebind the old handle", "Fixed per-shard and per-connection assembly-admission limits", "one shard's exhaustion cannot borrow or consume another shard's reserve", "concurrent HTTP/2 requests and stalled streams", "duplicate release", "distinct principal/tenant shard identities cannot share an arena", "`ConservativeInvalidationScope::Reserved { handle: AssemblyInvalidationHandle", "`ConservativeInvalidationScope::AssemblyArenaGeneration(ArenaRotationCause)`", "`ArenaRotationCause::{DetectedSemanticCorruption, DetectedStorageCorruption, ExplicitCallerPolicy}`", "internal invariant breach or trusted-storage integrity failure", "malformed/semantic/conflicting peer input cannot mint a corruption cause", "`AssemblyInvalidationCapacity`, `LeaseHeld`", "The invalidation handle remains non-Copy/non-Clone, exact-correlation-bound", "reject invalidation-handle Copy/Clone/duplication/rebinding/early or double release/retry reuse", "`SemanticallyInvalid`", "`PhysicallyReclaimable`", "typed local `LeaseHeld`/capacity", "cannot rotate the arena or evict outside the reserved namespace", "no slot reuse or new mutable alias until every lease drops/acknowledges",
