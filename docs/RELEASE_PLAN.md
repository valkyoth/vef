...
- Extend the v0.145.0 receiving-client PUSH_PROMISE transaction after the complete promised field block and all CONTINUATION input have decoded, HPACK is synchronized, and promised-request semantics, authority, safe/cacheable method, and caller push policy pass, but before promised-request publication. Derive the handle target from that exact validated promised request; derive principal, privacy partition, tenant shard, and navigation namespace only from the associated locally initiated request and caller-supplied push policy, never peer identity claims. Atomically reserve both the existing promised-stream slot and the matching `AssemblyInvalidationHandle`, or roll both back with no promised request or assembly correlation. If either capacity is unavailable, finish the entire field-block sequence, install only the bounded mandatory-control rejection tombstone, publish nothing, leave the associated stream and unrelated contexts unchanged, and schedule stream-local `RST_STREAM(CANCEL)` for the promised ID as permitted by RFC 9113 Section 6.6. While reset output is backpressured, drain any already-arriving promised-stream HEADERS/CONTINUATION through the synchronization-only HPACK path, discard DATA with normal receive-credit accounting under the rejection tombstone, and grant no response, partial-retention, or assembly authority; rollback releases each acquired slot/handle reservation exactly once.
- Make `AssemblyInvalidationHandle` a linear non-Copy/non-Clone capability bound to exactly one ordinary or promised-request correlation. Reserve it once before local request output or promised-request publication; for accepted push hold it through reserved(remote), open response, informational response, body/trailers, reset, GOAWAY, connection failure, and terminal-event backpressure. Release it exactly once only after final completion and terminal delivery, cancellation, reset, connection failure, or a committed retry disposition. A retry cannot duplicate or rebind the old handle: the old correlation reaches its release disposition and the new correlation independently reserves a new handle before output. Fixed per-shard and per-connection assembly-admission limits bound stalled correlations and accepted pushes, and one shard's exhaustion cannot borrow or consume another shard's reserve.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Test Client correlation, cancellation, and retry tokens and all previously implemented relevant behavior with positive, negative, boundary, truncation, invalid-state, cancellation, capacity, and no-panic cases. Exhaust handles with concurrent HTTP/2 requests, accepted pushes, and stalled streams at every per-shard/per-connection limit; prove local `AssemblyInvalidationCapacity` emits no request bytes, creates no tracked correlation, applies resumable backpressure, never rotates/clears an existing arena, and cannot consume another shard's reserve. For push floods distinguish promised-slot/work exhaustion from invalidation-handle exhaustion; vary atomic reservation order/failure, associated stream state, cross-principal/partition/tenant/navigation attempts, response HEADERS/CONTINUATION/DATA arriving before RST commit, HPACK table mutations, receive credit, and mandatory-control backpressure; prove full field-block synchronization, no promised publication/partial authority, exact rollback/release, one stream-local CANCEL, and no associated-stream or unrelated-context mutation. Exercise accepted-push reserved/open/informational/body/trailer states, pushed full 200 and 206 correlation handoff, cancellation at every phase, reset, GOAWAY, connection failure, terminal-event backpressure, retry disposition/new-handle reservation, duplicate release, stale/rebound/Copy/Clone attempts, and explicit whole-shard assembly disable/clear ordering; prove every admitted handle is released exactly once and distinct principal/tenant shard identities cannot share an arena.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
