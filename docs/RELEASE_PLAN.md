...
- Maintain one engine-owned `Http10ReuseLedger { connection_generation, hop, remaining }`, initialized once from the configured maximum and monotonically non-increasing for that connection/hop. `remaining` means exactly the number of future successor admissions still authorized, excluding the currently active exchange. Configured zero therefore selects the v0.74.0 close disposition for the initial exchange before its local head leaves `AcceptedPrivate`, emits `Connection: close`, and can never generate `CommittedHttp10KeepAliveHead`. If admission decrements one to zero, that one successor is admitted, but its new local request or response head must be generated or rewritten and revalidated with `Connection: close` while still `AcceptedPrivate`; its zero `reuse_remaining_snapshot` cannot generate a local keep-alive signal or `CommittedHttp10KeepAliveHead`, regardless of peer signaling. This last-allowance close decision cannot wait for end-of-message permit minting. Thus configured maximum one means exactly one successor whose local head announces close, then no second permit.
- Define separate `Http10PermitMintOutcome::{Minted, CloseWithoutReuse { reason }}` and `Http10PermitMintCloseReason::{ZeroConfiguredAllowance, LedgerExhausted, NegotiationUnavailable, FramingNotReusable, PolicyRevoked, DeadlineArithmeticOverflow, CorrelationInvariantFailure}`. Permit minting occurs only after both matching signals and both message lifecycles complete. `Minted` creates one sealed permit inside engine-owned `Reusable`; client/proxy-upstream minting consumes correlated committed-request plus most-recent validated response evidence, while origin/server minting consumes validated request plus committed response evidence. The permit binds the exact ledger generation and observed remaining count but does not decrement it. Every close-without-reuse reason creates no permit, returns same-call successor input wholly unconsumed, and closes without a peer protocol error; correlation invariant failure is a local security event. Configured maximum zero and ledger exhaustion are mint failures, not admission outcomes. Checked idle-deadline addition overflow is also a mint failure. A proxy constructs evidence and ledger independently on each hop; received keep-alive evidence is never forwarded, transformed into local commitment, or reused across downstream/upstream connections.
...
- Define the sole successor-admission edge as `Http10ConnectionState::Reusable { permit: Http10ReusePermit } -> ActiveExchange { next_exchange_generation, reuse_remaining_snapshot }`. Its total result is `Http10SuccessorAdmissionOutcome::{Admitted, RetryableCapacity { reason: Http10CapacityReason }, CloseLocal { reason }}`. `RetryableCapacity` carries only a freely constructible reason; the non-Copy/non-Clone permit remains exclusively engine-owned inside `Reusable` and is never returned, borrowed, or exposed to the caller. Before transition, transactionally assemble one internal non-Copy/non-Clone `Http10NextExchangeReservation` containing the next-exchange record, correlation owner, parser/event/output storage leases, one request/exchange-count debit, charged admission/transition work plus the minimum parser-work reserve needed to reach safe backpressure, the idle-deadline owner, and the checked next generation. Construction additionally validates current policy and permit-to-ledger binding/count. It is all-or-nothing: every one-short storage, queue, count, or work failure releases the incomplete reservation and leaves permit, input, ledgers, budgets, generation, output, and `Reusable` unchanged. Retryable capacity returns all input wholly unconsumed, accepts/exposes no command or byte, and emits no peer error. `CloseLocal` covers `RequestCountExhausted`, `WorkBudgetExhausted`, `PolicyRevoked`, `IdleDeadlineReached`, `PermitLedgerMismatch`, `CorrelationInvariantFailure`, and `GenerationExhausted`; zero configured allowance, ledger exhaustion, negotiation/framing failure, and deadline arithmetic overflow are excluded because they occur during permit minting. Every admission-close reason is local policy/resource/invariant state, never peer misconduct. It revokes the permit, preserves the ledger count, returns all input wholly unconsumed, admits no command or byte, and moves the whole connection context to closing. `PermitLedgerMismatch` and correlation failure are local invariant/security events. Exact deadline equality (`now >= deadline`) is expired. Generation uses checked increment only; maximum/exhaustion closes the entire connection context before input/command acceptance and never numerically wraps while stale capabilities can exist.
- On `Admitted`, atomically: consume `Http10NextExchangeReservation`, the non-Copy permit, and idle-deadline owner; acquire the exchange record and correlation owner; install the parser/event/output leases; commit the request/exchange-count debit, admission work charge, and minimum parser-work reserve; checked-decrement `Http10ReuseLedger.remaining` exactly once; store the result back only in that ledger; capture immutable `reuse_remaining_snapshot` in `ActiveExchange`; install the fresh non-overlapping generation; apply the last-allowance `AcceptedPrivate` close rule; and only then publish `ActiveExchange`. This one indivisible commit consumes `Http10NextExchangeReservation`, the non-Copy permit, and idle-deadline owner before publication and commits the request/exchange-count debit, admission work charge, and minimum parser-work reserve. No allocation, reservation, debit, validation, head rewrite, or other fallible initialization remains after `ActiveExchange` becomes visible. `Http10ReuseLedger.remaining` is the sole mutable reuse-authorization source; the snapshot is diagnostic/binding evidence only, is never modified, and is never consulted to mint another permit. The next permit reads only the ledger. Peer signaling, a new exchange, or builder configuration cannot reset it. Repeated capacity failures never decrement it, and cancellation/failure after admission never refunds it. A client or proxy-upstream API cannot accept or expose the next request before this transition; an origin/server cannot consume the next request byte before it.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete v0.56.0 `Connection` corpus under HTTP/1.0 and HTTP/1.1; require identical lexical decisions and exact version bindings but version-specific outcomes. Exhaust every v0.74.0 matrix cell and two-sided negotiation order: missing/uncommitted local signal, missing/invalid/stale received signal, response admission before/after oldest-request correlation, duplicate/mismatched/no outstanding request, unrelated-signal revocation, exact correlated-signal survival and cleanup, local request/response commitment at every output offset, lifecycle incomplete/complete, policy/framing/budget loss at every output phase, newest-message invalidation, and forged/stale/cross-role/direction/hop/version/connection/exchange/head/generation evidence. Require the exact client, origin/server, and proxy-upstream pairings and per-hop non-forwarding. Test reuse maxima 0, 1, and N across complete exchanges: zero makes the initial local head close with no signal; one admits exactly one successor whose private local head announces close, whose zero snapshot cannot mint a signal, and which yields no second permit; N decrements and applies that same final boundary. Test every `Http10PermitMintOutcome` reason separately from every admission result/reason. Prove zero/ledger exhaustion, negotiation/framing failure, policy-at-mint, deadline-add overflow, and correlation-at-mint create no permit; prove `PermitLedgerMismatch` is a local admission invariant close. At one-short and exact capacity for each exchange record, correlation owner, parser/event/output store, request/exchange-count unit, transition-work charge, and minimum parser-work reserve, require all-or-nothing `Http10NextExchangeReservation`, no partial lease/debit/charge/generation/ledger/head mutation, no fallible work after publication, and exactly one atomic commit on success. Repeat capacity failure then success and compile-fail attempts to extract, retain, rebind, or duplicate the engine-owned permit or reservation; require a reason-only result, permit still solely in `Reusable`, unchanged input/state, one later transaction, no reset, and no cancellation refund. Inject duplicate final acknowledgements, repeated `MessageCommitted` completion hooks, and cancellation immediately before/during/after mint and immediate same-call admission; require at most one `Minted`, permit, ledger decrement, successor publication, correct cancellation winner ownership, and no reconstruction of `Reusable` after admission. Test exact `now == deadline`, generation maximum-minus-one/maximum/exhaustion with stale tokens, and full-context close without wrap/ABA. Positively test same-call reuse only for bodyless and exact fixed-length final completion. Negatively test Transfer-Encoding, chunked framing, terminating chunks, trailers, close-delimited response bodies, and full head/nonfinal body acknowledgement; none can mint `ValidatedHttp10KeepAlive`, `Http10ReusePermit`, or admit a successor, and input stays wholly unconsumed before close. Prove the ledger alone authorizes future minting and `reuse_remaining_snapshot` is immutable/non-authoritative. Cross rejected HTTP/1.0 CONNECT with every matrix/negotiation outcome and v0.73.0 error/action state; prove no reuse permit, tunnel, application publication, authority resolution, forwarding, successor parse, optimistic-input lease, HTTP/1.1 close proof, or Upgrade authority is created.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  `Http10ReusePermit`, internal linear `Http10NextExchangeReservation`, total
  reason-only-capacity `Http10SuccessorAdmissionOutcome`, the sole
  `Reusable -> ActiveExchange` successor transition, checked generation-
  exhaustion owner, exact future-admission allowance, last-allowance
  `AcceptedPrivate` close action, output-phase persistence-loss action, and
  ...
  or modify the ActiveExchange snapshot as authorization; advertise keep-alive
  from an initial-zero or one-to-zero allowance; conflate mint and admission
  failures; repeat mint after duplicate acknowledgement/hook/cancellation;
  classify a
...
- Record early-final request generation, framing kind, disposition, bounded
  ...
  initial-zero/one-to-zero private-head close decision, permit deadline,
  exactly-once mint/cancellation winner, next-exchange reservation component
  acquisition/release/commit, independent count/work debit, successor
  outcome/reason, permit/input ownership, exact bodyless/fixed
...
- Fuzz CONNECT 2xx, Upgrade/WebSocket 101, and the v0.163.0/v0.187.0/v0.188.0
  ...
  mint/admission reason, initial-zero and one-to-zero private-head close before
  exposure, reason-only capacity retry with engine-retained permit, every
  `Http10NextExchangeReservation` component one short/exact and atomic
  storage/count/work/deadline/generation commit, duplicate final
  ...
  no-pipelining rule. Require no false keep-alive advertisement, no partial
  reservation/debit/lease, no fallible work after Active publication, and at
  most one mint, permit, decrement, reservation commit, and successor.
...
- Reject caller construction, copying, rebinding, or direct mutation of
  ...
  work/deadline/generation component; post-Active fallible initialization;
  ...
  snapshot minting `CommittedHttp10KeepAliveHead`, or deferred private-head
  close; duplicate mint/hook/acknowledgement or cancellation recreating
  `Reusable`; zero/count/work/policy/deadline/correlation/generation
  ...
  decrementing the ledger; snapshot modified or consulted as authorization;
...
- Independently size each configured early-final request's framing/progress/
  ...
  initial-zero/one-to-zero close marker, exactly-once mint/cancellation state,
  permit deadline, bounded `Http10NextExchangeReservation` with exchange/
  correlation/parser/event/output leases, count/work charges and parser-work
  reserve, separate mint and successor results/reasons/ownership, exact
