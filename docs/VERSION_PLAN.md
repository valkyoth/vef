...
| HEADER_TABLE_SIZE transaction arbitration | participant transition at `0.124.0`; scheduler/ACK-commit barrier at `0.139.0`/`0.142.0`/`0.153.0`; audit/diagnostics/fuzz/compile-fail at `0.154.0`/`0.193.0`/`0.196.0`/`0.197.0`; resource replay at `0.211.0` | AwaitingSafeApply collapses transaction-local smallest/final maxima; AppliedAwaitingAckCommit retains every SETTINGS owner but no ACK authority. Full FIFO ACK commitment merges those values into separate debt without losing its older minimum. |
...
| `0.90.0` | HPACK dynamic table storage | Requires HPACK static table; requires decoded output to own/pin independent bytes rather than borrow evictable entries; Unlocks HPACK eviction and oversize-entry behavior. |
| `0.91.0` | HPACK eviction and oversize-entry behavior | Requires HPACK dynamic table storage; proves eviction/overwrite/size reduction cannot mutate prior decoded output; Unlocks HPACK table-size update and SETTINGS coupling. |
...
| `0.98.0` | HPACK encoder output commit and indexing policy | Requires Sensitive-field indexing policy; defines serialized provisional field blocks plus linear table-update debt that Private encoding leases, rollback restores, and first exposure later transfers; Unlocks Independent HPACK decode limits. |
...
| `0.108.0` | SETTINGS syntax, role, directional values, and ACK rules | Requires SETTINGS frame codec; prevalidates ordered entries, reserves one frame transaction/ACK, and defines WaitingParticipants/AckEligible/AckFrozen 0..=8/AckCommitted/AbortedConnection without equating eligibility to wire commitment; Unlocks PING frame codec. |
...
| `0.124.0` | SETTINGS header-table encoder and header-list policy coupling | Requires Atomic HPACK header-block integration; keeps AwaitingSafeApply/AppliedAwaitingAckCommit ACK ownership separate from linear unsignaled debt, merging each committed change without losing the older minimum; Unlocks Pseudo-field ordering and uniqueness. |
...
| `0.139.0` | SETTINGS outstanding-ACK accounting | Requires HTTP/2 body cancellation, reset, and flow-credit lifecycle; integrates frame-wide participant/ACK states and FIFO wire commitment, whose byte-nine hook releases transition owners and merges values into separate unsignaled debt; Unlocks Bounded stream admission. |
...
| `0.153.0` | Reserved control-output queues | Requires WINDOW_UPDATE churn defenses; commits ACKs at byte nine, then merges transitions into independent debt without clearing it; frozen ACK and Private debt lifetimes cannot alias or consume each other; Unlocks HTTP/2 conformance audit and pentest. |
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits every ACK/exposure/CONTINUATION boundary, multi-owner transitions, debt merge/lease/rollback/transfer, stale tokens, backpressure/failure, and encoder/decoder equivalence; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.191.0` | Fixed-capacity caller-storage public API | Requires Authenticated origin authorization and HTTP/2 coalescing metadata; seals SETTINGS/ACK promotion plus debt merge/lease/restore/transfer authority while validating fixed reserves without allocator; Unlocks Optional alloc-backed convenience API. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports redacted ACK disposition, transition, debt owner/state/minimum/final, field-block transfer, token, arena, and credit without exposing header contents; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts 0→4096 and 4096→0→4096 changes across ACK, Wire-ready debt, Private, exposure, CONTINUATION, rollback, backpressure/failure, and stale-token boundaries; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects fabricated debt, duplicate lease/restore/transfer, early clearing, cross-block rebinding, ACK/debt authority confusion, and prior transaction/token violations; Unlocks Long-running soak and exhaustion campaign. |
...
| `0.211.0` | Deterministic CPU, stack, code-size, and amplification budgets | Requires Aesynx kernel integration tests; sizes ACK/transition owners independently from one connection debt, one Private lease, transferred FramingCommitted debt, FIFO prefixes, and fatal cleanup; Unlocks 32-bit target campaign. |
