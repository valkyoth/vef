...
- Maintain one engine-owned `Http10ReuseLedger { connection_generation, hop, remaining }`, initialized once from the configured maximum and monotonically non-increasing for that connection/hop. `remaining` means exactly the number of future successor admissions still authorized, excluding the currently active exchange. Store immutable `Http10LocalPersistenceMode::{Negotiable, LastUseMustClose}` in every active exchange. Configured zero installs `LastUseMustClose` for the initial exchange. If admission decrements one to zero, that one successor is admitted and its `ActiveExchange` atomically receives `LastUseMustClose`; this structural mode installation does not require a server response to exist. For a client/proxy-upstream exchange, the successor request command participates in the reservation. On that path, the request builder injects or validates `Connection: close` before exposure, with validation complete before Active publication. For an origin/server exchange, the mode survives request admission and the later response builder injects or validates `Connection: close` before exposure. If a local head already exists in `AcceptedPrivate`, apply the v0.74.0 rewrite and revalidation immediately; if no head exists, retain the mandatory-close mode until one is built. No builder under `LastUseMustClose` can create `CommittedHttp10KeepAliveHead`, regardless of peer signaling or a zero `reuse_remaining_snapshot`. Thus configured maximum one means exactly one successor whose local head eventually announces close, then no second permit.
- Define separate `Http10PermitMintOutcome::{Minted, CloseWithoutReuse { reason }}` and `Http10PermitMintCloseReason::{ZeroConfiguredAllowance, LedgerExhausted, NegotiationUnavailable, FramingNotReusable, PolicyRevoked, DeadlineArithmeticOverflow, CorrelationInvariantFailure}`. Permit minting occurs only after both matching signals and both message lifecycles complete. `Minted` creates one sealed permit inside engine-owned `Reusable`; client/proxy-upstream minting consumes correlated committed-request plus most-recent validated response evidence, while origin/server minting consumes validated request plus committed response evidence. The permit binds the exact ledger generation and observed remaining count but does not decrement it. It exclusively owns `Http10IdleDeadline { connection_generation, permit_generation, expires_at }`; no reservation or ActiveExchange duplicates that deadline authority. Every close-without-reuse reason creates no permit, returns same-call successor input wholly unconsumed, and closes without a peer protocol error; correlation invariant failure is a local security event. Configured maximum zero and ledger exhaustion are mint failures, not admission outcomes. Checked idle-deadline addition overflow is also a mint failure. A proxy constructs evidence and ledger independently on each hop; received keep-alive evidence is never forwarded, transformed into local commitment, or reused across downstream/upstream connections.
...
- Define the sole successor-admission edge as `Http10ConnectionState::Reusable { permit: Http10ReusePermit } -> ActiveExchange { next_exchange_generation, reuse_remaining_snapshot, local_persistence_mode }`. Its total result is `Http10SuccessorAdmissionOutcome::{Admitted, RetryableCapacity { reason: Http10CapacityReason }, CloseLocal { reason }}`. `RetryableCapacity` carries only a freely constructible reason; the non-Copy/non-Clone permit remains exclusively engine-owned inside `Reusable` and is never returned, borrowed, or exposed to the caller. Before transition, transactionally assemble one internal non-Copy/non-Clone `Http10NextExchangeReservation` containing the next-exchange record, correlation owner, parser/event/output storage leases, one request/exchange-count debit, charged admission/transition work plus the minimum parser-work reserve needed to reach safe backpressure, an immutable checked deadline snapshot/binding copied from the permit, the checked next generation, and the selected local persistence mode. The reservation never owns the deadline. Construction validates current policy, `now < permit.deadline.expires_at`, exact deadline generation/value, and permit-to-ledger binding/count. It is all-or-nothing: every one-short storage, queue, count, or work failure releases the incomplete reservation and leaves the exact permit including its deadline, input, ledgers, budgets, generation, output, and `Reusable` unchanged. Retryable capacity returns all input wholly unconsumed, accepts/exposes no command or byte, and emits no peer error. `CloseLocal` covers `RequestCountExhausted`, `WorkBudgetExhausted`, `PolicyRevoked`, `IdleDeadlineReached`, `PermitLedgerMismatch`, `CorrelationInvariantFailure`, and `GenerationExhausted`; zero configured allowance, ledger exhaustion, negotiation/framing failure, and deadline arithmetic overflow are excluded because they occur during permit minting. Every admission-close reason is local policy/resource/invariant state, never peer misconduct. It revokes the permit and its deadline, preserves the ledger count, returns all input wholly unconsumed, admits no command or byte, and moves the whole connection context to closing. `PermitLedgerMismatch` and correlation failure are local invariant/security events. Exact deadline equality (`now >= deadline`) is expired. Generation uses checked increment only; maximum/exhaustion closes the entire connection context before input/command acceptance and never numerically wraps while stale capabilities can exist.
- On `Admitted`, atomically: consume `Http10NextExchangeReservation` and the non-Copy permit including its sole deadline owner; acquire the exchange record and correlation owner; install the parser/event/output leases; commit the request/exchange-count debit and admission work charge; install the minimum parser-work reserve; checked-decrement `Http10ReuseLedger.remaining` exactly once; store the result back only in that ledger; capture immutable `reuse_remaining_snapshot`; install the fresh non-overlapping generation and selected `Http10LocalPersistenceMode`; and only then publish `ActiveExchange`. A capacity rollback leaves the exact deadline generation and value in the permit inside `Reusable`. Successful admission invalidates that permit deadline identity, so a stale expiry cannot close the new `ActiveExchange`. No engine structural allocation, reservation, debit, lease, generation, or persistence-mode initialization remains after `ActiveExchange` becomes visible. Later origin/server application response construction and validation intentionally remain fallible before exposure; `LastUseMustClose` remains authoritative throughout and any builder failure emits no bytes, closes locally, and enters terminal cleanup. `Http10ReuseLedger.remaining` is the sole mutable reuse-authorization source; the snapshot is diagnostic/binding evidence only, is never modified, and is never consulted to mint another permit. Peer signaling, a new exchange, or builder configuration cannot reset it. Repeated capacity failures never decrement it. A client or proxy-upstream API cannot accept or expose the next request before this transition; an origin/server cannot consume the next request byte before it.
- `ActiveExchange` exclusively owns its committed exchange/correlation records and parser/event/output storage leases. A generation-bound non-Copy cleanup authority runs exactly once on normal message success, cancellation, parse failure, transport failure, or connection close and returns every such lease to its caller-provided pool. The reuse-ledger decrement, request/exchange-count debit, and admission-work charge are irreversible and never refunded. The minimum parser-work reserve is capacity, not an up-front charge: parsing consumes it incrementally into irreversible work charges, and terminal cleanup releases any unused remainder exactly once. Duplicate or stale cleanup for an older exchange generation is state-neutral and cannot release a later generation's resources. “Cancellation never refunds” applies only to irreversible accounting; it never permits leaking exchange records, correlation records, parser/event/output storage, or unused parser-work reserve.
- The combined-call fast path exists only when acknowledgement causes the exact outbound HTTP/1.0 response lifecycle transition into `MessageCommitted`: for fixed-length output this is the final promised body byte, and for semantically bodyless output final head acknowledgement may also complete the message. Chunked/trailered output has no HTTP/1.0 positive path. Full acknowledgement of a nonfinal head/body record does not qualify. One engine-owned lifecycle-bound mint state permits exactly one transition from completed negotiation signals to `Minted`, terminal no-reuse, or cancellation. Duplicate final acknowledgements and repeated completion-hook calls are state-neutral and cannot repeat a mint outcome. Cancellation immediately before, during, or after mint is serialized against that transition: a cancellation winner creates no permit; a mint winner installs exactly one engine-owned permit that cancellation revokes through one terminal path; an admission winner leaves only `ActiveExchange`, so cancellation or a repeated hook cannot recreate `Reusable`. At the `MessageCommitted` edge, the driver first obtains `Http10PermitMintOutcome`; only `Minted` may immediately run successor admission before parsing. Invalid acknowledgement remains state-neutral; zero, short, or full acknowledgement that does not reach `MessageCommitted` leaves successor input wholly unconsumed as `PrematureHttp10Successor`. `CloseWithoutReuse` leaves input wholly unconsumed and closes. Across mint, immediate same-call admission, cancellation, and repeated hooks there is at most one `Minted`, one permit, one ledger decrement, and one successor publication. After `Admitted`, any consumed input, accepted command, or exposed byte can only complete or abandon `ActiveExchange`; no failure rolls back to `Reusable`.
- Preserve the phase invariant: HTTP/1 has one octet-level inbound/outbound interpretation, bounded body ownership, exact transition handoff, typed dispositions, and no HTTP/0.9 fallback.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 1945, RFC 6455 opening-handshake requirements, RFC 9110, RFC 9111 preservation requirements, RFC 9112 and errata, the RFC 9298 non-applicability decision, and RFC 9931.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete v0.56.0 `Connection` corpus under HTTP/1.0 and HTTP/1.1; require identical lexical decisions and exact version bindings but version-specific outcomes. Exhaust every v0.74.0 matrix cell and two-sided negotiation order: missing/uncommitted local signal, missing/invalid/stale received signal, response admission before/after oldest-request correlation, duplicate/mismatched/no outstanding request, unrelated-signal revocation, exact correlated-signal survival and cleanup, local request/response commitment at every output offset, lifecycle incomplete/complete, policy/framing/budget loss at every output phase, newest-message invalidation, and forged/stale/cross-role/direction/hop/version/connection/exchange/head/generation evidence. Require the exact client, origin/server, and proxy-upstream pairings and per-hop non-forwarding. Test reuse maxima 0, 1, and N across complete exchanges: zero installs `LastUseMustClose` initially; one admits exactly one successor with that mode and no second permit; N applies the same final boundary. For a client request builder, an origin response builder supplied only after request parsing, and a pre-existing private head, prove close injection/revalidation before exposure and prohibit `CommittedHttp10KeepAliveHead`; prove a server needs no response head at admission and retains the mode until response construction. Test every `Http10PermitMintOutcome` reason separately from every admission result/reason. Prove zero/ledger exhaustion, negotiation/framing failure, policy-at-mint, deadline-add overflow, and correlation-at-mint create no permit; prove `PermitLedgerMismatch` is a local admission invariant close. At one-short and exact capacity for each exchange record, correlation owner, parser/event/output store, request/exchange-count unit, transition-work charge, and minimum parser-work reserve, require all-or-nothing `Http10NextExchangeReservation`, no partial lease/debit/charge/generation/ledger/mode mutation, no fallible engine structural work after publication, and exactly one atomic commit on success. Prove the permit is the only idle-deadline owner, the reservation carries only an immutable checked snapshot, capacity rollback preserves the exact deadline generation/value, admission consumes and invalidates it, and stale expiry cannot affect `ActiveExchange`. Repeat capacity failure then success and compile-fail attempts to extract, retain, rebind, or duplicate the engine-owned permit or reservation; require a reason-only result, permit still solely in `Reusable`, unchanged input/state, one later transaction, and no accounting refund. Inject duplicate final acknowledgements, repeated `MessageCommitted` completion hooks, and cancellation immediately before/during/after mint and immediate same-call admission; require at most one `Minted`, permit, ledger decrement, successor publication, correct cancellation winner ownership, and no reconstruction of `Reusable` after admission. At normal success, cancellation, parse failure, transport failure, and connection close after every post-admission point, require generation-bound exactly-once release of exchange/correlation/parser/event/output leases and unused parser-work reserve; require reuse-ledger, request-count, consumed parser work, and admission-work charges to remain charged. Duplicate/stale cleanup cannot touch a newer generation, and repeated admission/termination cycles restore all reclaimable caller capacity without restoring irreversible accounting. Test exact `now == deadline`, generation maximum-minus-one/maximum/exhaustion with stale tokens, and full-context close without wrap/ABA. Positively test same-call reuse only for bodyless and exact fixed-length final completion. Negatively test Transfer-Encoding, chunked framing, terminating chunks, trailers, close-delimited response bodies, and full head/nonfinal body acknowledgement; none can mint `ValidatedHttp10KeepAlive`, `Http10ReusePermit`, or admit a successor, and input stays wholly unconsumed before close. Prove the ledger alone authorizes future minting and `reuse_remaining_snapshot` is immutable/non-authoritative. Cross rejected HTTP/1.0 CONNECT with every matrix/negotiation outcome and v0.73.0 error/action state; prove no reuse permit, tunnel, application publication, authority resolution, forwarding, successor parse, optimistic-input lease, HTTP/1.1 close proof, or Upgrade authority is created.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Close every applicable RFC 1945/6455/9110/9112/9931 requirement and erratum with a named passing test or recorded disposition, replay the HTTP/0.9 cross-protocol and HTTP/1 smuggling corpora against client/server/intermediary profiles, and block exit on any divergent framing, publication, close, capacity, or bounded-work result. Audit the complete HTTP/1 causal product: the v0.56.0 sole version-neutral `Connection` lexer and version-bound consumers; the v0.74.0/v0.75.0 total role × direction persistence matrix, two-sided per-hop signals, response-correlation-first transfer, connection/hop monotonic `Http10ReuseLedger`, exact future-admission meaning of `remaining`, role-feasible `Http10LocalPersistenceMode` retained through later server response construction, separate exactly-once permit-mint and successor-admission outcomes, permit-exclusive idle-deadline ownership and snapshot-only reservation binding, engine-only permit retention on capacity retry, all-or-nothing `Http10NextExchangeReservation` and atomic resource/budget commit, generation-bound exactly-once terminal lease reclamation with non-refundable accounting, checked generation exhaustion without wrap, bodyless/fixed-only HTTP/1.0 `MessageCommitted` same-call boundary, explicit chunked/Transfer-Encoding/trailer/close-delimited non-reuse, cancellation/duplicate-hook race serialization, most-recent-message invalidation, output-phase close boundary, and no-pipelining rule; `AcceptedPrivate -> Frozen -> HeadCommitted -> MessageCommitted/Abandoned`; outstanding-response FIFO insertion and oldest matching; 100/early-final behavior; the total disposition legality matrix and both typed transport actions over fixed/chunked/trailer output; persistence/writability/source/budget; successor/reuse/417 rules; complete-100-before-101 output; HTTP/1.1 CONNECT 2xx and Upgrade/WebSocket 101 publication; and v0.73.0 local-capacity/start-line-component/414/431/400/501 precedence, classification-only authority-form, exact 70-byte role-specific HTTP/1.0 rejection, `Flush501ThenClose`/`CloseTransportNow`, invalid-acknowledgement neutrality, final-byte commitment, later-input rejection, capacity, and failure states.
- Preserve the phase invariant: HTTP/1 has one octet-level inbound/outbound interpretation, bounded body ownership, exact transition handoff, typed dispositions, and no HTTP/0.9 fallback.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 1945, RFC 6455 opening-handshake requirements, RFC 9110, RFC 9111 preservation requirements, RFC 9112 and errata, the RFC 9298 non-applicability decision, and RFC 9931.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete v0.56.0 `Connection` corpus under both HTTP versions through every persistence/refinement/stripping consumer; exhaust the v0.74.0/v0.75.0 role/direction matrix, correlation-before-revocation and ambiguous-response closure, every two-signal order, per-hop non-forwarding, reuse ledger maxima 0/1/N with `Negotiable`/`LastUseMustClose` across client request, delayed server response, and pre-existing private-head paths, every separate permit-mint and successor-admission reason, repeated reason-only capacity retry with unextractable engine permit, every reservation component one short/exact with no partial resource or budget mutation, exact permit-owned deadline preservation on retry and stale-expiry rejection after admission, deadline equality/add-overflow at the correct phase, `PermitLedgerMismatch`, non-refundable accounting with exact lease reclamation, duplicate acknowledgements/hooks and cancellation on each side of mint/immediate admission, generation max-minus-one/max/exhaustion with stale cleanup tokens, and exact positive `MessageCommitted` fast paths only for bodyless/fixed responses. At each post-admission terminal point require every reclaimable record/lease and unused parser reserve to return exactly once, no later-generation release, and every irreversible ledger/count/work debit to remain charged. Require at most one mint, permit, ledger decrement, reservation commit, successor publication, cleanup, and correct terminal owner. Negatively replay Transfer-Encoding, chunked, trailers, and close-delimited bodies at every split; require no evidence/permit/successor and wholly unconsumed successor input before close. Require the ledger as sole mutable authority and snapshot as immutable diagnostics; no false keep-alive advertisement, impossible server-head dependency at admission, duplicate deadline owner, storage leak disguised as no-refund, engine-structural fallibility after publication, wrap, count reset, double decrement, premature parsing, peer-error misclassification, or rollback. Replay local capacity, aggregate start-line byte/work, overlong method/version, malformed version, target-within-cap aggregate overflow, target/work 414, field/work 431, syntax/framing/content 400, and fully valid unsupported CONNECT 501 precedence at one-under/exact/one-over boundaries, including classification-only authority-form. For every role-specific rejection assert the exact 70-byte image, original optimistic discard, every 0..=70 acknowledgement offset, state-neutral invalid/stale/cross/oversized acknowledgement and input-only call, valid-acknowledgement-first `CloseTransportNow`, capacity fallback, EOF/cancellation/failure, no parse/retain/enqueue/backpressure, and no publication/resolution/forwarding/successor/tunnel/fabricated completion. Replay every request/response head and request-body record acknowledgement offset and input call order, persistence loss at `AcceptedPrivate`/`Frozen`/`HeadCommitted`/`MessageCommitted`, early response/body timing, fixed/chunked delimiter/trailers, each disposition, response close signaling, simultaneous response body, cancellation, 417 retry, FIFO/successor generation, supported HTTP/1.1 CONNECT/101 over-read boundary, partial failure, and optimistic WebSocket attempt; require exact commitment, suppression/continuation, reuse/close, ownership, and no-reparse outcomes.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  lifecycle-bound exactly-once mint state, `Http10PermitMintOutcome`,
  `Http10ReusePermit` with sole deadline ownership, internal linear
  `Http10NextExchangeReservation` with deadline snapshot only, total
...
- Record early-final request generation, framing kind, disposition, bounded
  ...
  commitment, pair/hop match, permit-mint outcome/reason, reuse-ledger before/
  ...
  exactly-once mint/cancellation winner, next-exchange reservation component
...
- Fuzz CONNECT 2xx, Upgrade/WebSocket 101, and the v0.163.0/v0.187.0/v0.188.0
  ...
  non-forwarding, separate permit-mint and `Reusable -> ActiveExchange`
  admission transitions for both role entry points; ledger maxima 0/1/N, every
  mint/admission reason, initial-zero/one-to-zero mode across client request,
  ...
  before/during/after mint and same-call admission, exact
  ...
  structural fallibility after Active publication, and at most one mint,
  ...
  atomic permit-mint edge, and bridge-generation teardown/reuse.
...
- Independently size each configured early-final request's framing/progress/
  ...
  `Http10PersistenceDisposition`/`ValidatedHttp10KeepAlive`/
  `CommittedHttp10KeepAliveHead`/`CorrelatedHttp10KeepAliveRequest`/
  `Http10ReuseLedger`/`Http10PermitMintOutcome`/`Http10ReusePermit`, newest-
  message revocation, per-hop signal pairing, response-correlation owner, sole
  mutable future-admission count, immutable ActiveExchange snapshot,
  `Http10LocalPersistenceMode`, exactly-once mint/cancellation state, permit-
  ...
  separate mint and successor results/reasons/ownership, exact
