...
- Acceptance contract: Arbitrate each valid peer SETTINGS_HEADER_TABLE_SIZE with `HpackEncoderTransaction` through a bounded connection-owned `PendingEncoderTableSizeTransition::{None, Pending { smallest_seen, final_value, settings_ack_obligations }}`. With no active transaction, atomically apply the received limit, reserve/queue one SETTINGS ACK for each accepted SETTINGS frame, and require the next field block to start with the necessary encoder size update. With a Private unexposed transaction, roll back its provisional encoder mutations and complete field-block slot/entry reservation, apply the transition, make its reserved ACK eligible ahead of ordinary output, then re-encode under the new limit; re-encoding/storage failure is typed local backpressure and cannot retain or indefinitely delay the mandatory ACK. With FramingCommitted, retain each setting and ACK obligation in reserved capacity while the pre-reserved block drains without interleaving; after final END_HEADERS-frame acknowledgement, publish the existing HpackCommitted transaction, apply the pending transition, serialize every owed ACK before ordinary output, and require the next block's first representations to carry the table update. Across duplicate values, multiple entries, or multiple SETTINGS frames while unresolved, retain the smallest observed maximum and the final maximum in receive order; emit one HPACK size update when equal or the RFC-required smallest-then-final pair when different, while still acknowledging every SETTINGS frame exactly once. No later field block may encode while either a provisional transaction or pending encoder-size transition is unresolved. Treat peer MAX_HEADER_LIST_SIZE as outbound guidance while preserving independent hard inbound decode limits.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone.
- Cross no-active, Private, and every FramingCommitted initial/CONTINUATION/final acknowledgement prefix with one/multiple SETTINGS frames, duplicate HEADER_TABLE_SIZE entries, decreasing/increasing sequences, local re-encode capacity failure, reset, cancellation, GOAWAY, and connection failure. Prove exact Private rollback, old-transaction publication before pending transition, no ACK interleaving, one ACK per SETTINGS frame, ACK eligibility independent of re-encode success, smallest/final update order at the next block start, no later-block encoding against stale/provisional state, and no double update/ACK.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Apply INITIAL_WINDOW_SIZE deltas to every active stream with checked signed arithmetic and all-or-nothing rollback on overflow; no stream may observe a partial settings transition. Before committing any valid delta, atomically revoke every affected unexposed DATA reservation and release its paired connection reservation, apply the delta, then leave queued frames blocked until they re-reserve under the new windows. Frozen/exposed DATA retains its committed debit and exact suffix; a reduction can leave a stream window negative, in which case no later DATA frame becomes exposable until credit recovers. Failure rolls back windows, reservation ownership, and queue eligibility together.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Serialize encoder work and SETTINGS_HEADER_TABLE_SIZE transitions: no new field-block encoding is runnable while a Private/FramingCommitted transaction or `PendingEncoderTableSizeTransition` is unresolved. A setting supersedes Private work and makes its reserved ACK eligible before ordinary output; behind FramingCommitted, reserve every ACK obligation but release it only after final-frame acknowledgement/HpackCommitted and transition application.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Test Reserved control-output queues and all previously implemented relevant behavior with positive, negative, boundary, truncation, invalid-state, cancellation, capacity, and no-panic cases. Queue each required control during every initial/CONTINUATION prefix of a FramingCommitted outbound field block; for multiple HEADER_TABLE_SIZE-bearing SETTINGS frames prove one reserved ACK each while only smallest/final HPACK updates collapse. Prove capacity remains reserved, Private rollback releases ACK ahead of failing re-encode, FramingCommitted serialization waits through final-frame acknowledgement/HpackCommitted, and transport failure emits no interleaved control byte.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Public constructors reject overflow in `checked_add(9, max_outbound_frame_payload)`, zero/undersized `field_fragment_cap`, enabled mandatory-prefix shortage, inconsistent `max_outbound_field_block_bytes`/`max_outbound_continuations_per_block`, insufficient actual-cap slot/queue capacity, and ambiguous padding policy. `PendingEncoderTableSizeTransition` and HPACK transaction promotion remain engine-owned. Public output acknowledgement accepts only the suffix range named by one record-bound token; vectored adapter convenience cannot batch slots or completion hooks.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Distinguish Private, FramingCommitted, HpackCommitted, AbandonedWithConnection, and pending encoder-table transition; report redacted smallest/final table-size values, owed ACK count, actual field fragment cap, mandatory prefix, encoded block bytes, continuation count, preflight slot/entry demand versus ownership, padding overhead/data bytes, one-record token identity, and provisional-versus-published HPACK generation without field values. Never diagnose first exposure as HPACK table publication, collapsed size updates as collapsed SETTINGS ACKs, or one acknowledgement as crossing slots.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Add transitions for every supported actual local field fragment cap, mandatory-prefix and checked block-byte/ceiling-division/continuation boundaries, FramingCommitted versus HpackCommitted, capacity exhaustion before/after exposure, peer MAX_FRAME_SIZE and HEADER_TABLE_SIZE changes mid-block, smallest/final encoder updates plus exact per-frame ACKs, Private re-encode failure with ACK progress, provisional HPACK rollback/publication/abandonment, checked slot/padding arithmetic, and scalar/vectored cross-slot acknowledgement attempts. No generated acknowledgement may complete more than one frame.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Reject construction/rebinding of `HpackEncoderTransaction` or `PendingEncoderTableSizeTransition`, promotion from Private to FramingCommitted without exact actual-cap slot/entry ownership, caller promotion to HpackCommitted, bypass of block-byte/continuation/mandatory-prefix bounds, cross-record token acknowledgement, and any vectored API that can combine two frame slots under one token or lifetime.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Size each profile for `max_outbound_field_block_bytes` fragmented at its smallest supported actual `field_fragment_cap`, including enabled mandatory prefixes, checked ceiling division, per-frame nine-byte overhead, `max_outbound_continuations_per_block`, exact queue entries, pending encoder-table transition, and worst-case SETTINGS ACK obligations. Measure provisional HPACK transaction storage separately from frame slots and prove `9 + payload`/padding arithmetic at integer boundaries.
- Preserve the phase invariant: Adapters cannot alter protocol validity; TLS admission, EOF/alerts, deterministic resource ceilings, readiness, deadlines, storage, and release evidence remain explicit across targets.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7301, RFC 8446, RFC 9113 TLS prerequisites, platform ABI and target guarantees, the admitted Brynja contract, and all previously applicable HTTP requirements.
