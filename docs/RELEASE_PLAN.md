...
- Extend the HTTP/2 frame/state harness with every command ordering, partial
  ...
  half-close/close or fabricated completion.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Assign stable non-secret diagnostic codes to syntax, protocol scope/code, policy, capacity, cancellation, timeout, transport, and peer/local role; include bounded offsets/counters and generation identifiers, redact field/credential/compression/section contents, and emit each event once. Distinguish local `AssemblyInvalidationCapacity`, `PushedAssemblyProvenanceCapacity`, `StreamTrackingUnavailable`, and `PushRejectionTrackingUnavailable`. For reset and ordinary END_STREAM output, separately record command acceptance/local-send sealing, exposure, offered/acknowledged counts, outstanding token, frame completion, directional wire transition, remote completion/reset records, and immutable first-wire-closure cause. Never report offered-but-unacknowledged bytes as written, partial/failing output as local half-close/close, RemoteEndStream as a closure cause when it only produced HalfClosedRemote, or later local completion as replacing a peer-first close. Record terminal/section/workspace dispositions without raw contents and preserve redaction/caller-scrub rules.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Acceptance contract: Maintain stateful fuzz targets for HTTP/1, HPACK, HTTP/2, Structured Fields, and translation with deterministic minimized replay. The HTTP/2 oracle spans ResetOutput reservation/reason × exposure × offered/acknowledged prefix 0..=13 × ordinary HEADERS/DATA/trailers/empty-DATA END_STREAM frame length and acknowledgement offset × command sealing × token generation/validity × every inbound frame × prior wire state × remote/first-closure cause × terminal/section/workspace states × END/peer reset/GOAWAY/failure × caller-buffer and stream-generation reuse. It asserts immutable suffix progress, prior reset legality through byte 12, no ordinary local transition before full-frame acknowledgement, exact Open→HalfClosedLocal or HalfClosedRemote→Closed transition at completion, retained CONTINUATION/HPACK ownership, stable peer-first cause, partial-failure non-completion, state-neutral invalid tokens, no early reuse, and no stranded state.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
