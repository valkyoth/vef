...
def main() -> int:
    ...
    required_contract_text = (
        ...
        "separate bounded reserved-push-slot/work budget before reservation", "preflight tombstone and cutoff capacity before releasing its slot or convert that slot in place", "continuously represented as a generation-bound slot, tombstone, or connection-shutdown record", "Store policy disposition, current RFC wire state, local reset-output progress/reason/reservation, remote-closure cause, terminal-validation state, and active field-block ownership as independent generation-checked fields", "Normalize END_STREAM as a state-transition event separate from its carrying HEADERS or DATA event", "`HeadersStart { end_stream, end_headers }`", "CONTINUATION never changes wire state", "`ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByPeerReset, PolicyDormantAfterEndStream}`", "`ResetReason::{PolicyCancel, StreamError(StreamErrorCode)}`", "`TerminalValidation::{PendingFieldBlock, Valid, Malformed(StreamErrorCode)}`", "`RemoteClosureCause::{Reset, EndStream}`", "Any informational 1xx HEADERS carrying END_STREAM is explicitly terminal `Malformed(PROTOCOL_ERROR)` and requires stream RST_STREAM(PROTOCOL_ERROR)", "HEADERS with END_STREAM but without END_HEADERS records closed wire state immediately", "A valid terminal DATA marks `TerminalValidation::Valid` and releases a dormant policy-reset slot after credit", "another terminal message violation marks `Malformed(PROTOCOL_ERROR)` and re-arms that slot as a mandatory stream-error reset", "A zero-byte policy-CANCEL reservation can become dormant after remote END_STREAM without freeing its slot", "an uncommitted `StreamError(code)` replaces/re-arms a dormant or queued `PolicyCancel` in the same reserved slot", "partially serialized or committed RST_STREAM bytes are immutable", "HEADERS+END_STREAM without END_HEADERS sets terminal validation to `PendingFieldBlock`", "typed bounded `StreamTrackingUnavailable` connection shutdown", "validated promised request, trusted authority/push-policy provenance, and reserved slot unpublished and rollback-capable", "promised slot includes or pre-reserves its v0.117.0 in-place rejection-tombstone/cutoff representation", "At v0.145.0 the transaction's participants are the promised-stream slot/work and rejection tracking", "Rejected promised streams use the v0.118.0 orthogonal model", "total auditable `RejectedPushFrameDisposition` matrix", "normalized inbound event including END_STREAM/END_HEADERS", "header-section phase", "with no wildcard/default cell", "reset commit state/reason/reservation", "Every cell declares protocol legality and exact error code/scope, ordered wire transitions, HPACK/terminal-validation action, application visibility, stream/connection flow-credit action, reset dormancy/re-arm/release/serialization action, and connection-fatal override", "Implement RFC 9113's in-flight PUSH_PROMISE exception for an associated stream with a locally initiated reset", "accept and completely decode PUSH_PROMISE plus every CONTINUATION", "publish no promised request, and recreate no application, authority, push-policy, invalidation, or assembly provenance", "Missing recycled associated-request provenance selects safe promised-stream rejection", "Illegal promised IDs remain connection PROTOCOL_ERROR and malformed HPACK remains connection-fatal", "No v0.145.0-only pushed response has partial-retention or cross-request assembly authority",
