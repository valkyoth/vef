...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; blocks control serialization behind FramingCommitted and freezes exact generation-bound WINDOW_UPDATE records while preserving ACK/debt and field-block completion rules; Unlocks SETTINGS max-frame-size outbound integration. |
...
| `0.153.0` | Reserved control-output queues | Requires WINDOW_UPDATE churn defenses; owns exact WINDOW_UPDATE offsets 0..=13 and final-byte-only receive-credit commitment alongside SETTINGS ACK byte-nine commitment; Unlocks HTTP/2 conformance audit and pentest. |
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits receive-credit underflow/reclamation/frozen-output boundaries plus four-limit HPACK and every prior ACK/debt/block boundary; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.191.0` | Fixed-capacity caller-storage public API | Requires Authenticated origin authorization and HTTP/2 coalescing metadata; validates table physical capacity and seals ceiling/selection/ACK/debt plus receive-credit/update mutation authority; Unlocks Optional alloc-backed convenience API. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; separately reports HPACK limits/debt and receive advertised/reclaimed/in-flight credit, update target/generation/disposition/offset without contents; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; crosses HPACK limits/debt plus receive-credit ledgers, WINDOW_UPDATE offsets 0..=13, closure, coalescing, stale tokens, and failure while proving conservation/equivalence; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects forged HPACK authority and construction/rebinding/retargeting/early commitment of receive-credit and WINDOW_UPDATE state; Unlocks Long-running soak and exhaustion campaign. |
...
| `0.211.0` | Deterministic CPU, stack, code-size, and amplification budgets | Requires Aesynx kernel integration tests; measures HPACK state plus maximum stream/connection receive ledgers, private/frozen 13-byte update records, queue entries, checked arithmetic, and amplification; Unlocks 32-bit target campaign. |
