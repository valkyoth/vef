...
- Arbitrate a newly required local reset against v0.137.0 ordinary output before scheduling it. Supersede AcceptedPrivate ordinary output in place, release its unexposed DATA reservation, and queue the reset; if ordinary output is Frozen—even with zero acknowledged—retain its committed debit and finish the exact frame suffix before the reset begins. A mandatory RST_STREAM can preempt queued work but can never interleave inside a frozen ordinary frame. Completed ordinary output runs its hook once before reset serialization; connection failure transfers both records to bounded cleanup without inventing completion or refund.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Test increase/reduction activation at every outbound SETTINGS byte split,
  ...
  frozen-layout stability, rollback, ACK ordering, and absolute-boundary values.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Replay every RFC 9113 frame, stream-state, continuation, flow-control, SETTINGS, GOAWAY, push, priority, HPACK, malformed-field, flood, and shutdown vector for both roles; require exact connection/stream error code, state delta, publication trace, control output, work charge, and interop transcript before the HTTP/2 pentest exit. Replay every inbound frame at local RST_STREAM acknowledgement offsets 0..=13 and require pre-completion legality, full-acknowledgement-only local closure, immutable first-closure attribution, remote-first completion behavior, and partial-failure non-completion evidence. For HEADERS, DATA, trailers, and empty DATA carrying outbound END_STREAM, replay AcceptedPrivate/private-encoded/Frozen-zero/Frozen-partial/Complete against remote END_STREAM, peer/local reset, CONTINUATION, cancellation, and failure; require unexposed supersession, frozen whole-frame completion before reset, send sealing at acceptance, and directional transition only at complete frame acknowledgement. Exhaust atomic stream+connection DATA reservations across padding, concurrent last-credit contenders, WINDOW_UPDATE, negative SETTINGS reductions, unexposed resegmentation/revocation, frozen debit, and zero-length END_STREAM.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Acceptance contract: Maintain stateful fuzz targets for HTTP/1, HPACK, HTTP/2, Structured Fields, and translation with deterministic minimized replay. The HTTP/2 oracle spans ResetOutput × ordinary AcceptedPrivate/Frozen/Complete/Superseded output × exposure/acknowledgement × completion hook × HEADERS/DATA/trailers/empty-DATA layout × stream/connection available-reserved-committed credit × padding × WINDOW_UPDATE/SETTINGS/resegmentation × token generations × every inbound frame × wire/closure/terminal/section state × peer/local reset/GOAWAY/failure/reuse. It asserts unexposed supersession and paired refund, no frozen refund/mutation/interleaving, exact suffix and hook once, atomic no-oversubscription credit, full payload but no header charge, zero-length zero charge, negative-window blocking, directional legality, HPACK ownership, stable first cause, partial-failure non-completion, and no stranded state.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
