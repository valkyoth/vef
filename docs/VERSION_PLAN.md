...
| HTTP/2 DATA and command ownership | `0.130.0`, `0.136.0`–`0.138.0` | Apply DATA framing/credit before normalized END_STREAM closure; rejected DATA remains reserved/half-closed through reset prefix 12, gains post-reset connection-only credit only after actual remote or fully acknowledged local closure, and shares one generation-safe output lifecycle. |
...
| `0.123.0` | Atomic HPACK header-block integration | Requires HTTP/2 graceful GOAWAY and bounded shutdown sequencing; atomically seals/transfers TerminalFieldSectionLease while releasing only CompressionWorkspace, or releases/transfers all state on abort/fatal shortage; Unlocks SETTINGS header-table encoder and header-list policy coupling. |
...
| `0.137.0` | HTTP/2 outbound per-stream message command lifecycle | Requires HTTP/2 inbound DATA ownership, acknowledgement, and credit release; seals local sends at END_STREAM command acceptance but emits LocalEndStreamComplete only at full carrying-frame acknowledgement, preserving partial-failure state and fragmented HEADERS ownership; Unlocks HTTP/2 body cancellation, reset, and flow-credit lifecycle. |
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; exhausts offsets 0..=13 and makes full acknowledgement the sole local-close trigger, with remote-first no-op completion, exact suffixes, and partial-failure non-closure; Unlocks SETTINGS outstanding-ACK accounting. |
...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; schedules frozen records without regeneration, pins offered ranges/generations, and runs each END_STREAM completion hook once only after its carrying frame is fully acknowledged; Unlocks SETTINGS max-frame-size outbound integration. |
...
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits reset offsets 0..=13 plus every END_STREAM-carrying frame across acceptance/partial/full acknowledgement, CONTINUATION, peer/remote races, and failure; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; distinguishes local-send sealing, reset/END_STREAM frame completion, directional transition, remote records, and immutable first cause, never reporting partial failure as half-closed/closed; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts reset offsets and ordinary END_STREAM frame offsets across wire/closure/token/peer/CONTINUATION/failure/reuse races and proves full-ack-only transitions plus stable attribution; Unlocks Compile-fail state and lifetime tests. |
