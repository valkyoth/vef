...
| Never-untracked HTTP/2 rejection state | storage at `0.117.0`; END_STREAM/validation state at `0.118.0`; flag/block sequencing at `0.120.0`/`0.123.0`; push integration at `0.145.0`; assembly at `0.181.0`; diagnostics at `0.193.0` | Preflight bounded tracking and retain policy, wire, reset state/reason/reservation, terminal validation, closure cause, and active block independently. END_STREAM cannot release a dormant reset slot before validation. Tracking failure emits typed bounded shutdown without forgetting the ID. |
| Rejected-push frame and reset arbitration | model at `0.118.0`; error precedence at `0.121.0`; HPACK at `0.123.0`; malformed/status/message rules at `0.125.0`–`0.131.0`; credit at `0.136.0`; reset lifecycle at `0.138.0`; total push table at `0.145.0`; assembly at `0.181.0`; diagnostics/fuzz at `0.193.0`/`0.196.0` | Remote END_STREAM makes a zero-byte policy CANCEL dormant, not final: valid terminal validation releases it; malformed message re-arms the same slot as `StreamError(PROTOCOL_ERROR)`; fatal HPACK chooses bounded connection shutdown. Begun reset bytes are immutable, peer reset can supersede an unsent action, and no path emits two stream resets. |
...
| `0.117.0` | Generation-checked stream table and tombstones | Requires HTTP/2 stream-identifier rules; retains reset reason/reservation, terminal validation, closure, and field-block state independently, including dormant mandatory slots; Unlocks Exhaustive stream-state graph. |
| `0.118.0` | Exhaustive stream-state graph | Requires Generation-checked stream table and tombstones; adds `TerminalValidation::{PendingFieldBlock, Valid, Malformed}` beside normalized END_STREAM and surviving CONTINUATION ownership; Unlocks HTTP/2 activation preface, first-SETTINGS, and deadline sequencing. |
...
| `0.120.0` | HTTP/2 frame legality and fragmented-header-block sequencing | Requires HTTP/2 activation preface, first-SETTINGS, and deadline sequencing; separates policy/error reset reason, dormant-after-END_STREAM state, validation, closure, and active block ownership; Unlocks HTTP/2 error scope, typed deltas, and isolated stream mutation. |
| `0.121.0` | HTTP/2 error scope, typed deltas, and isolated stream mutation | Requires HTTP/2 frame legality and fragmented-header-block sequencing; orders connection-fatal shutdown over new stream action, stream error over uncommitted policy CANCEL, immutable begun bytes over later duplication, and peer reset over unsent action; Unlocks HTTP/2 graceful GOAWAY and bounded shutdown sequencing. |
...
| `0.123.0` | Atomic HPACK header-block integration | Requires HTTP/2 graceful GOAWAY and bounded shutdown sequencing; holds terminal validation/reset reservation through fragmented HPACK, then selects semantic validation or connection-fatal shutdown; Unlocks SETTINGS header-table encoder and header-list policy coupling. |
...
| `0.125.0` | Pseudo-field ordering and uniqueness | Requires SETTINGS header-table encoder and header-list policy coupling; maps terminal pseudo-field violations to retained stream PROTOCOL_ERROR even after END_STREAM; Unlocks Connection-specific field and TE validation. |
...
| `0.127.0` | HTTP/2 malformed initial-field-block publication barrier | Requires Connection-specific field and TE validation; re-arms dormant reset reservation for malformed terminal names/values/fields instead of accepting remote completion; Unlocks HTTP/2 request mapping. |
...
| `0.129.0` | HTTP/2 response mapping | Requires HTTP/2 request mapping; maps missing/malformed terminal `:status` to retained stream PROTOCOL_ERROR with no response publication; Unlocks HTTP/2 content-length, DATA, trailers, and END_STREAM reconciliation. |
| `0.130.0` | HTTP/2 content-length, DATA, trailers, and END_STREAM reconciliation | Requires HTTP/2 response mapping; validates Content-Length/body/message phase before valid release or malformed re-arm of the dormant slot; Unlocks Informational responses and trailers. |
| `0.131.0` | Informational responses and trailers | Requires HTTP/2 content-length, DATA, trailers, and END_STREAM reconciliation; explicitly maps 1xx+END_STREAM and malformed terminal trailers to stream PROTOCOL_ERROR after full synchronization; Unlocks Cookie field combination and Set-Cookie preservation. |
...
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; makes END_STREAM suppression conditional on terminal validation, re-arms malformed completion without allocation, preserves begun reset bytes, and prevents duplicate resets; Unlocks SETTINGS outstanding-ACK accounting. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.138.0 foundations; completes the total table across terminal validation, policy/error reset arbitration, END flags, phase, credit, HPACK outcome, and output offset; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.181.0` | Client correlation, cancellation, and retry tokens | Requires Partial-response media-type classification plus v0.145.0 push admission; atomically preflights pushed resources and inherits dormant-slot validation/re-arm and immutable-reset arbitration; Unlocks Streaming partial-response and retained-prefix validation. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports terminal validation, reset reason/dormancy/re-arm, connection override, closure cause, and actual wire emission independently and redacted; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts validation × reset reason/state/output offset × END flags × phase × HPACK/message/credit outcome and connection races; Unlocks Compile-fail state and lifetime tests. |
