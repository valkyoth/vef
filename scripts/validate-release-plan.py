...
def main() -> int:
    ...
    failures.extend(["unsafe SETTINGS ACK banking remains"] if "EarlyPeerAckPending" in detailed + compact else []); failures.extend(["unsafe symmetric bridge barrier remains"] if any(text in detailed + compact for text in ("HandshakeBarrier", "TransitionBarrier", "DownstreamCommitted", "UpstreamCommitted", "both barrier orders", "both completion orders")) else [])
    if failures:
