...
- Acceptance contract: For an untrusted downstream TCP client, a proxy client either waits for the CONNECT 2xx before forwarding payload or sends Connection: close; a proxy server rejecting CONNECT closes the transport and processes no later request regardless of Connection; specifically, HTTP/1 CONNECT 407 carries a valid Proxy-Authenticate challenge, mandates connection close, discards owned optimistic/tunnel bytes, logically invalidates attempt/credential capabilities and releases their references without promising caller-buffer zeroization, and permits authentication retry only on a fresh connection/generation. WebSocket request-following bytes before complete 101 transport commitment are a terminal handshake violation: consume/discard owned bytes exactly once, mint no transition lease, never return them to the HTTP parser, never enter Active, and never forward them after a later success. HTTP/1 CONNECT-UDP sends no optimistic payload. Ordinary CONNECT retains the separate RFC 9931 wait-or-Connection: close policy.
- Preserve the phase invariant: HTTP/1 has one octet-level inbound/outbound interpretation, bounded body ownership, exact transition handoff, typed dispositions, and no HTTP/0.9 fallback.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 1945, RFC 6455 opening-handshake requirements, RFC 9110, RFC 9111 preservation requirements, RFC 9112 and errata, the RFC 9298 non-applicability decision, and RFC 9931.
...
- Test wait-or-close, every rejected CONNECT close/no-reparse path, 407 challenge plus fresh-connection retry, owned-byte discard, capability/reference invalidation and caller-scrub boundary, HTTP/1 request head plus premature WebSocket bytes in one buffer, CONNECT-UDP prohibition, and all previously implemented relevant behavior with positive, negative, boundary, truncation, cancellation, capacity, and no-panic cases. Assert premature input mints no lease, creates no Active state, is never reparsed/forwarded, and has exactly one cleanup.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- For legal HTTP/1 success-following over-read, create one `PendingHttp1Transition { overread: OverreadLease, terminal: Option<PreActiveTerminalCause>, bridge_generation, leg, exchange_generation, transport_generation }`. The sealed owner retains exact bytes plus immutable first-terminal-cause attribution and distinguishes `PlainTcpEof`, `TlsCloseNotify`, `FatalTlsAlert`, `TransportReset`, and `Cancellation`. It disposes bytes and terminal ownership exactly once. WebSocket EOF/failure before downstream success exposure fails the handshake; after any success exposure it closes/aborts both legs without a replacement HTTP response. Ordinary CONNECT follows an explicit drain/close policy. TLS close_notify never fabricates TCP half-close authority.
...
- Never expose downstream 101/2xx before complete upstream inbound validation. First downstream success exposure freezes its exact bytes; only matching `CommittedDownstreamSuccess` enters `ActiveAfterDownstreamSuccessCommitted` and permits WebSocket bytes to cross. Activation transfers each legally minted input lease once without copying or releasing HTTP/2 credit. Application acknowledgement or policy discard remains the sole reclamation path and WINDOW_UPDATE owner. Failure from any earlier phase remains an HTTP-framed downstream failure and invokes the HTTP/1 `PendingHttp1Transition` or existing HTTP/2 non-2xx/reset/PendingConnect cleanup once. Failure after any downstream success exposure closes downstream and aborts/resets the established upstream side without a second HTTP response or double discard. Over-read/PendingConnect bytes remain owned by their original leg; legal success-following bytes cannot cross, be parsed as WebSocket, be released, restore credit, or survive generation teardown before Active, while forbidden request-following input can never enter these transferable owners.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Feed success HEADERS followed in the same input buffer by DATA, padded DATA, DATA plus END_STREAM, and RST_STREAM. Also feed downstream HTTP/1 request head plus premature WebSocket bytes and downstream RFC 8441 request plus DATA before 2xx. Cross zero, short, and full downstream 101/2xx acknowledgement combined with input; invalid acknowledgement; legal upstream HTTP/1 101 plus data; and the legal upstream HTTP/2 success-following cases. Assert premature input creates no transferable lease or Active state, is never reparsed/forwarded, restores no credit early, and receives exactly one cleanup.
- Include padding-only and mixed padded DATA, then cross every downstream-success output offset, stream/connection WINDOW_UPDATE threshold, GOAWAY, PendingConnect/bridge capacity boundary, and bridge cancellation. Cross HTTP/1 legal over-read with plain TCP EOF, TLS close_notify, fatal TLS alert, reset, and cancellation at every bridge phase and immediately before/after first downstream success exposure. Test WebSocket pre-exposure handshake failure, ordinary CONNECT drain/close and preserve-versus-reject policies, pending FIN publication immediately after Active, and reset/fatal failure immediately before versus after first success exposure. Prove one existing owner per protocol and no duplicate copy/store, exact range order and padding/semantic accounting, no early/double credit or discard, immutable first terminal cause, exactly-once HTTP/1 byte/terminal cleanup, no close_notify-derived TCP half-close, existing HTTP/2 cleanup once before Active, linear ranges/FIN transfer once at Active, generation teardown safety, partial downstream-success close/abort without a second response, and WebSocket bytes cross only after Active.
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus.
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
...
- Test Exact CONNECT, Upgrade, and tunnel byte-handoff ownership and all previously implemented relevant behavior with positive, negative, boundary, truncation, invalid-state, cancellation, capacity, and no-panic cases. Cross reservation shortages, outbound commit and inbound validation evidence for each protocol, every inbound semantic/correlation boundary, downstream request-following HTTP/1/HTTP/2 WebSocket input at zero/short/full/invalid success acknowledgement, legal upstream 101-plus-data and success-HEADERS-plus-DATA, HTTP/1 over-read plus EOF/reset/close_notify/fatal alert/cancellation at every bridge phase, HTTP/2 DATA timing/padding/partial acknowledgement/credit threshold/capacity, each legal phase and illegal skip, stale stream/bridge generations, RST_STREAM/GOAWAY/connection failure, and terminal events immediately before/after first downstream exposure. Prove premature input has no transferable lease/reparse/Active/forwarding/early-credit effect and one cleanup; upstream success validation precedes downstream success bytes; Active alone permits one transfer; terminal cause is immutable; close_notify grants no TCP half-close; no inbound HpackCommitted fabrication or duplicate owner/copy/credit/discard occurs; and failure selects exactly one terminal cleanup.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  capabilities, legal-success-following `BridgeInputLease` minting,
  `PendingHttp1Transition`, PendingConnect/ReceiveCredit/
  ...
  generation evidence; turn premature WebSocket input into a lease; copy/
  release/rebind pending bytes; publish a pending FIN; replace the first
  terminal cause; turn TLS close_notify into TCP half-close; skip/reorder
...
- Record early-final request generation, framing kind, disposition, bounded
  ...
  correlation stage, legal-success-following versus forbidden-request-following
  input classification and acknowledgement offset, HTTP/1
  ...
  padded/semantic DATA and private/advertised credit counts, distinct plain
  EOF/close_notify/fatal-alert/reset/cancellation cause, pending-FIN policy,
  immutable first terminal cause, lease-minted/terminal-discarded disposition,
  downstream-success exposure/acknowledgement, Active promotion, and exact
  pre-exposure HTTP failure or post-exposure close/reset/abort action without
  handshake/tunnel bytes or duplicate ownership.
...
- Fuzz CONNECT 2xx, Upgrade/WebSocket 101, and the v0.163.0/v0.187.0/v0.188.0
  ...
  exposure failure. Generate downstream request-following HTTP/1 WebSocket
  bytes and RFC 8441 DATA with zero/short/full/invalid success acknowledgement;
  legal upstream 101 plus data; and success HEADERS followed by DATA, padded
  DATA, DATA plus END_STREAM, or RST_STREAM. Cross HTTP/1 over-read with plain
  EOF, close_notify, fatal alert, reset, and cancellation at every phase; every
  downstream-success output offset; both WINDOW_UPDATE thresholds; GOAWAY;
  capacity; cancellation; and Active/credit races. Assert no cross-phase
  capability substitution, inbound HpackCommitted, premature transferable
  lease/reparse/Active/forwarding/early-credit, duplicate store/copy/credit/
  discard, close_notify-derived TCP half-close, early WebSocket FIN, lost
  CONNECT pending FIN, or mutable first terminal cause; Active alone transfers
  legal ranges/allowed FIN once, and terminal cleanup occurs once.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Reject caller construction, copying, rebinding, or direct mutation of
  ...
  caller minting of BridgeInputLease from premature request-following input,
  construction/copy/rebinding of PendingHttp1Transition, mutation/replacement
  of PreActiveTerminalCause, close_notify-to-TCP-half-close conversion,
...
- Independently size each configured early-final request's framing/progress/
  ...
  Size each bridge generation's transaction, four phase-capability records,
  legal/forbidden input disposition, and lease metadata; each HTTP/1 leg's one
  bounded over-read store plus PendingHttp1Transition bridge/leg/exchange/
  transport generations and first plain-EOF/close_notify/fatal-alert/reset/
  cancellation cause; and the existing HTTP/2 PendingConnect/
  AwaitingBridgeActivation store plus linear lease/range/generation/padding/
  ReceiveCredit/pending-FIN/first-terminal-cause metadata separately—never two
  generic bridge stores. Include upstream/
