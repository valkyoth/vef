...
- Define the sole successor-admission edge as `Http10ConnectionState::Reusable { permit: Http10ReusePermit } -> ActiveExchange { next_exchange_generation, remaining_after_admission }`. Its total result is `Http10SuccessorAdmissionOutcome::{Admitted, RetryableCapacity { permit }, CloseLocal { reason }}`. Before transition, preflight the next exchange slot, request/response correlation owner, parser/output storage, request-count/work budgets, current policy, ledger binding/count, injected idle deadline, and checked next generation. `RetryableCapacity` covers only local slot/storage capacity: it leaves the permit and ledger unchanged, remains `Reusable`, returns all input wholly unconsumed, accepts/exposes no command or byte, and emits no peer error. `CloseLocal` covers `ZeroReuseAllowance`, `RequestCountExhausted`, `WorkBudgetExhausted`, `PolicyRevoked`, `IdleDeadlineReached`, `DeadlineArithmeticOverflow`, `CorrelationInvariantFailure`, and `GenerationExhausted`; every reason is local policy/resource/invariant state, never peer misconduct. It revokes the permit, preserves the ledger count, returns all input wholly unconsumed, admits no command or byte, and moves the whole connection context to closing. Exact deadline equality (`now >= deadline`) is expired. Correlation failure is a local invariant/security event. Generation uses checked increment only; maximum/exhaustion closes the entire connection context before input/command acceptance, never numerically wraps or rotates identities while stale capabilities can exist.
- On `Admitted`, atomically consume the non-Copy permit, checked-decrement the connection/hop ledger exactly once, store its `remaining_after_admission` in both the ledger and `ActiveExchange`, and mint a fresh non-overlapping exchange generation. The next permit can only observe that carried count; peer signaling, a new exchange, or builder configuration cannot reset it. Repeated capacity failures never decrement it, and cancellation/failure after admission never refunds it. A client or proxy-upstream API cannot accept or expose the next request before this transition; an origin/server cannot consume the next request byte before it. The combined-call fast path exists only when acknowledgement causes the exact outbound response lifecycle transition into `MessageCommitted`: for fixed-length output this is the final body byte, for chunked output the terminating chunk plus all trailers, and for bodyless output final head acknowledgement may also complete the message. Full acknowledgement of a nonfinal head/body record does not qualify. At the `MessageCommitted` edge, same-call successor input may mint and consume the permit before parsing. Invalid acknowledgement remains state-neutral; zero, short, or full acknowledgement that does not reach `MessageCommitted` leaves successor input wholly unconsumed as `PrematureHttp10Successor`. If message commitment cannot mint a permit—zero allowance, peer/local signal failure, policy/budget loss, deadline overflow, or another terminal reason—successor input remains wholly unconsumed and the connection closes. After `Admitted`, any consumed input, accepted command, or exposed byte can only complete or abandon `ActiveExchange`; no failure rolls back to `Reusable`.
- Preserve the phase invariant: HTTP/1 has one octet-level inbound/outbound interpretation, bounded body ownership, exact transition handoff, typed dispositions, and no HTTP/0.9 fallback.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 1945, RFC 6455 opening-handshake requirements, RFC 9110, RFC 9111 preservation requirements, RFC 9112 and errata, the RFC 9298 non-applicability decision, and RFC 9931.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete v0.56.0 `Connection` corpus under HTTP/1.0 and HTTP/1.1; require identical lexical decisions and exact version bindings but version-specific outcomes. Exhaust every v0.74.0 matrix cell and two-sided negotiation order: missing/uncommitted local signal, missing/invalid/stale received signal, response admission before/after oldest-request correlation, duplicate/mismatched/no outstanding request, unrelated-signal revocation, exact correlated-signal survival and cleanup, local request/response commitment at every output offset, lifecycle incomplete/complete, policy/framing/budget loss at every output phase, newest-message invalidation, and forged/stale/cross-role/direction/hop/version/connection/exchange/head/generation `ValidatedHttp10KeepAlive`, `CommittedHttp10KeepAliveHead`, `CorrelatedHttp10KeepAliveRequest`, and `Http10ReusePermit`. Require the exact client, origin/server, and proxy-upstream pairings and per-hop non-forwarding. Test reuse maxima 0, 1, and N across complete exchanges; repeated capacity failures then success; every total outcome reason; exact `now == deadline`; checked deadline-add overflow; ledger mismatch; and request-count/work/policy/correlation failures. Require the exact permit/input/ledger/connection/error ownership above, one decrement only on admission, carried remaining count, no reset, and no cancellation refund. Test generation maximum-minus-one, maximum, and checked exhaustion while retaining stale tokens; require full-context close before input/command and no wrap/ABA. Cross same-call successor input at final head, every fixed-body byte, terminating chunk, every trailer byte, and bodyless response; only the acknowledgement that enters `MessageCommitted` may admit. Full acknowledgement without a mintable permit closes with input wholly unconsumed; nonfinal full, zero, short, and invalid acknowledgement remain premature or neutral as specified. Prove generations never overlap and no post-admission failure returns to `Reusable`. Cross rejected HTTP/1.0 CONNECT with every matrix/negotiation outcome and v0.73.0 error/action state; prove no reuse permit, tunnel, application publication, authority resolution, forwarding, successor parse, optimistic-input lease, HTTP/1.1 close proof, or Upgrade authority is created.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  `Http10ReusePermit`, total `Http10SuccessorAdmissionOutcome`, the sole
  ...
  the ledger except once on admission; classify a terminal preflight failure as
  peer misconduct; consume input/permit on retryable capacity; numerically wrap
  a generation; admit overlapping generations; parse/accept/expose a successor
  before admission; treat head/record commitment short of `MessageCommitted` as
  the fast path; restore `Reusable` after admission; or turn
...
- Record early-final request generation, framing kind, disposition, bounded
  ...
  commitment, pair/hop match, reuse-ledger before/after count, permit deadline,
  successor outcome/reason, permit/input ownership, exact message-completion
...
- Fuzz CONNECT 2xx, Upgrade/WebSocket 101, and the v0.163.0/v0.187.0/v0.188.0
  ...
  non-forwarding, `Reusable -> ActiveExchange` preflight/permit/budget/deadline/
  fresh-generation transitions for both role entry points; ledger maxima 0/1/N,
  every retryable/terminal reason, capacity retry, exact deadline/add-overflow,
  generation maximum/exhaustion with stale tokens, and every combined
  response `MessageCommitted` plus successor-input order across bodyless/fixed/
  chunked/trailered output,
...
- Reject caller construction, copying, rebinding, or direct mutation of
  ...
  failure with wrong permit/input/state/error ownership; stale/expired permit
  ...
  failure consuming permit/input or decrementing the ledger; head/nonfinal-
  record acknowledgement treated as MessageCommitted; fixed/chunked/trailer
  successor admitted before completion; cancellation refund; rollback from
...
- Independently size each configured early-final request's framing/progress/
  ...
  `Http10PersistenceDisposition`/`ValidatedHttp10KeepAlive`/
  `CommittedHttp10KeepAliveHead`/`CorrelatedHttp10KeepAliveRequest`/
  `Http10ReuseLedger`/`Http10ReusePermit`, newest-message revocation, per-hop
  signal pairing, response-correlation transfer owner, monotonic remaining
  count, permit deadline, total successor result/reason/ownership, exact
  MessageCommitted gate, checked next-generation exhaustion owner, output-phase
  persistence-loss action, and either-version intermediary stripping.
