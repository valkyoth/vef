...
| `0.137.0` | HTTP/2 outbound per-stream message command lifecycle | Requires inbound ownership plus both send ledgers; defines AcceptedPrivate/Frozen/Complete/Superseded output, exact dual DATA reservation→debit at exposure, END_STREAM hook at full acknowledgement, and unexposed reset supersession; Unlocks HTTP/2 body cancellation, reset, and flow-credit lifecycle. |
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; supersedes/recredits only AcceptedPrivate output, finishes Frozen whole-frame suffix before reset, and preserves reset offset/closure rules without refund; Unlocks SETTINGS outstanding-ACK accounting. |
...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; treats reservations as linear queue capabilities, commits them at exposure, completes Frozen frames before reset, and runs hooks once; Unlocks SETTINGS max-frame-size outbound integration. |
| `0.143.0` | SETTINGS max-frame-size outbound integration | Requires Bounded outbound scheduling; finalizes DATA segmentation/padding before reservation, releasing paired credit before replacing unexposed layouts and preserving Frozen layouts; Unlocks GOAWAY cutoff and retry classification. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.142.0 foundations; completes the rejected-push table across every frame and reset offset, wire/closure cause, token, semantic/HPACK lease, peer/terminal/failure race, caller reuse, and generation; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits reset/ordinary output disposition boundaries plus atomic padded DATA credit under contention, WINDOW_UPDATE, negative SETTINGS, resegmentation, cancellation, and failure; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; distinguishes ordinary disposition/hook and reset ordering plus bounded available/reserved/committed DATA credit, never reporting released reservation as debit or partial output as completion; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts ordinary/reset output states and dual-credit ledgers across exposure, padding, contention, settings/update/resegmentation, cancellation, failure, and generation races; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects OutputToken duplication/rebinding, buffer reuse while offered, frozen substitution, invalid acknowledgement, and early stream/tombstone reuse; Unlocks Long-running soak and exhaustion campaign. |
