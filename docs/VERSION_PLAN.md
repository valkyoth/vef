...
| Never-untracked HTTP/2 rejection state | storage at `0.117.0`; terminal/output state at `0.118.0`/`0.120.0`; block handoff at `0.123.0`; semantic owners at `0.125.0`–`0.131.0`; push at `0.145.0`; assembly at `0.181.0`; diagnostics at `0.193.0` | Retain policy, wire, reset frozen record/token/prefix, remote closure, immutable first-closure cause, terminal/section/workspace, and block independently. Sealed transitions prevent aliasing, early release, or partial-output closure; failure transfers cleanup without forgetting IDs or overstating progress. |
...
| `0.118.0` | Exhaustive stream-state graph | Requires Generation-checked stream table and tombstones; models local reset completion as a distinct event, selects first closure once, and forbids partial-output or duplicate closure alongside sealed TerminalFieldSectionLease ownership; Unlocks HTTP/2 activation preface, first-SETTINGS, and deadline sequencing. |
...
| `0.137.0` | HTTP/2 outbound per-stream message command lifecycle | Requires HTTP/2 inbound DATA ownership, acknowledgement, and credit release; applies private-encoding versus non-empty-exposure freeze and suffix-only frame continuation; Unlocks HTTP/2 body cancellation, reset, and flow-credit lifecycle. |
...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; schedules frozen mandatory records without regeneration and pins offered ranges/owning generations; Unlocks SETTINGS max-frame-size outbound integration. |
...
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; replays every inbound frame at reset offsets 0..=13 and audits full-ack-only local closure, remote-first attribution, and partial-failure non-completion; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; distinguishes output completion, local-close disposition, remote closure, and immutable first cause, never reporting partial failure as complete/closed or overwriting remote-first attribution; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts every frame × reset offset 0..=13 × wire/closure cause × token/peer/END/failure/buffer/generation race and proves exact legality, one close, and stable attribution; Unlocks Compile-fail state and lifetime tests. |
