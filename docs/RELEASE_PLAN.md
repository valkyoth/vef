...
- Define the sole successor-admission edge as `Http10ConnectionState::Reusable { permit: Http10ReusePermit } -> ActiveExchange { next_exchange_generation }`. Before the atomic transition, preflight the next exchange slot, request/response correlation owner, parser and output storage, and request-count/work/idle budgets; recheck the permit's injected idle deadline and remaining reuse count. On success consume the non-Copy permit once, decrement its reuse allowance once, and mint a fresh non-overlapping exchange generation. A client or proxy-upstream API cannot accept or expose the next request before this transition; an origin/server cannot consume the next request byte before it. Full final-response acknowledgement plus next-request input in one combined call first completes output, may mint the permit, then atomically admits the successor before parsing input. Invalid acknowledgement remains state-neutral; zero or short acknowledgement cannot complete the prior message, leaves successor input wholly unconsumed as `PrematureHttp10Successor`, and cannot mint or consume the permit. Preflight capacity failure returns input wholly unconsumed and preserves `Reusable` plus the permit; deadline expiry instead atomically revokes it and closes. After transition, any consumed input, accepted command, or exposed byte can only complete or abandon `ActiveExchange`; no failure rolls back to `Reusable`.
- Preserve the phase invariant: HTTP/1 has one octet-level inbound/outbound interpretation, bounded body ownership, exact transition handoff, typed dispositions, and no HTTP/0.9 fallback.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 1945, RFC 6455 opening-handshake requirements, RFC 9110, RFC 9111 preservation requirements, RFC 9112 and errata, the RFC 9298 non-applicability decision, and RFC 9931.
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  `CorrelatedHttp10KeepAliveRequest`, `Http10ReusePermit`, the sole
  `Reusable -> ActiveExchange` successor transition, output-phase persistence-
  loss action, and either-version stripping refinements;
  ...
  without atomic successor preflight, decrement it twice, admit overlapping
  generations, parse/accept/expose a successor before admission, restore
  `Reusable` after admission, or turn `ValidatedHttp10KeepAlive` into
...
- Record early-final request generation, framing kind, disposition, bounded
  ...
  commitment, pair/hop match, reuse-permit allowance/deadline, successor
  preflight/consumption, next exchange generation, output-phase persistence-
  loss action, and pipeline-disabled state;
...
- Fuzz CONNECT 2xx, Upgrade/WebSocket 101, and the v0.163.0/v0.187.0/v0.188.0
  ...
  fresh-generation transitions for both role entry points and every combined
  final-response acknowledgement plus successor-input order,
...
- Reject caller construction, copying, rebinding, or direct mutation of
  ...
  consumption without complete preflight; duplicate budget decrement; stale/
  expired permit admission; overlapping successor generation; client request
  acceptance/exposure or server input consumption before admission; capacity
  failure consuming permit/input; rollback from ActiveExchange to Reusable;
...
- Independently size each configured early-final request's framing/progress/
  ...
  `Http10PersistenceDisposition`/`ValidatedHttp10KeepAlive`/
  `CommittedHttp10KeepAliveHead`/`CorrelatedHttp10KeepAliveRequest`/
  `Http10ReusePermit`, newest-message revocation, per-hop signal pairing,
  response-correlation transfer owner, permit allowance/deadline, successor
  preflight state, next-generation owner, output-phase persistence-loss action,
  and either-version intermediary stripping.
