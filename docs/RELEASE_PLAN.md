...
- Acceptance contract: For an untrusted downstream TCP client, a proxy client either waits for the CONNECT 2xx before forwarding payload or sends Connection: close; a proxy server rejecting CONNECT closes the transport and processes no later request regardless of Connection; specifically, HTTP/1 CONNECT 407 carries a valid Proxy-Authenticate challenge, mandates connection close, discards owned optimistic/tunnel bytes, logically invalidates attempt/credential capabilities and releases their references without promising caller-buffer zeroization, and permits authentication retry only on a fresh connection/generation. Seal ordinary HTTP/1 optimistic payload as `PermittedOptimisticConnectInput` only when a generation-bound permit proves `Connection: close` was emitted directly or by configured close-after-exchange policy; success may transfer that owner once, while non-2xx/failure discards it once and permanently closes without reparsing. WebSocket request-following bytes before complete 101 transport commitment are `ForbiddenOptimisticWebSocketInput`: consume/discard owned bytes exactly once, mint no transition lease, never return them to the HTTP parser, never enter Active, and never forward them after a later success. `ForbiddenHttp1ConnectUdpInput` always terminates without a lease.
- Preserve the phase invariant: HTTP/1 has one octet-level inbound/outbound interpretation, bounded body ownership, exact transition handoff, typed dispositions, and no HTTP/0.9 fallback.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 1945, RFC 6455 opening-handshake requirements, RFC 9110, RFC 9111 preservation requirements, RFC 9112 and errata, the RFC 9298 non-applicability decision, and RFC 9931.
...
- Test wait-or-close, HTTP/1 CONNECT with and without proof that Connection: close was emitted directly or by configured policy followed by early bytes then 2xx and non-2xx, every rejected CONNECT close/no-reparse path, 407 challenge plus fresh-connection retry, one optimistic-owner transfer on success, exactly-once discard on failure, capability/reference invalidation and caller-scrub boundary, HTTP/1 request head plus premature WebSocket bytes in one buffer, CONNECT-UDP prohibition, and all previously implemented relevant behavior with positive, negative, boundary, truncation, cancellation, capacity, and no-panic cases. Assert forbidden input mints no lease, creates no Active state, is never reparsed/forwarded, and has exactly one cleanup.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Premature downstream RFC 8441 WebSocket DATA has one exact HTTP/2 disposition. First validate frame syntax and padding; any independent connection-fatal framing condition retains its normal scope. Otherwise charge the complete flow-controlled payload against both stream and connection windows, select stream-local `PrematureExtendedConnectData` and `RST_STREAM(PROTOCOL_ERROR)` with no HTTP response, discard semantic DATA only for that stream, and reclaim it only through the existing v0.136.0 `ReceiveCredit` ledger. Continue parsing later SETTINGS, PING, HEADERS, and DATA for unrelated streams in the same input buffer; never reset/mutate them, discard their bytes, or desynchronize HPACK.
- Never expose downstream 101/2xx before complete upstream inbound validation. First downstream success exposure freezes its exact bytes; only matching `CommittedDownstreamSuccess` enters `ActiveAfterDownstreamSuccessCommitted` and permits WebSocket bytes to cross. Activation transfers each provenance-authorized input lease once without copying or releasing HTTP/2 credit. Success transfers success-following and permitted optimistic ordinary-CONNECT ownership once; non-2xx/failure discards/closes once. Application acknowledgement or policy discard remains the sole reclamation path and WINDOW_UPDATE owner. Failure from any earlier phase normally remains an HTTP-framed downstream failure and invokes the HTTP/1 `PendingHttp1Transition` or existing HTTP/2 non-2xx/reset/PendingConnect cleanup once; premature RFC 8441 DATA uses its explicit stream reset instead. Failure after any downstream success exposure closes downstream and aborts/resets the established upstream side without a second HTTP response or double discard. Over-read/PendingConnect bytes remain owned by their original leg and provenance until Active or cleanup.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Feed success HEADERS followed in the same input buffer by DATA, padded DATA, DATA plus END_STREAM, and RST_STREAM. Cross HTTP/1 ordinary CONNECT with/without proof that Connection: close was emitted directly or by configured policy, early bytes, then 2xx/non-2xx; HTTP/2 ordinary CONNECT DATA before 2xx versus RFC 8441 WebSocket DATA before 2xx; downstream HTTP/1 premature WebSocket bytes; zero/short/full/invalid success acknowledgement; legal upstream HTTP/1 101 plus data; and legal HTTP/2 success-following cases. Prove only success-following or permitted optimistic provenance creates a transferable lease, with one transfer on success and one discard/close on failure; forbidden input creates no Active/reparse/forwarding/lease or duplicate credit owner.
- For premature RFC 8441 DATA, cross every valid/invalid padding and flow-window boundary, then interleave SETTINGS, PING, HEADERS, and DATA for unrelated streams in the same buffer. Require exact stream/connection charge before `RST_STREAM(PROTOCOL_ERROR)`, v0.136.0-only reclamation, no HTTP response, stream-local semantic discard, continued unrelated-frame processing, no unrelated mutation/loss/reset, and no HPACK desynchronization.
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  capabilities, sealed four-way `TransitionInputProvenance`,
  `OptimisticConnectPermit`, `BridgeInputLease` minting,
  ...
  generation evidence; forge optimistic permission; rebind ordinary CONNECT to
  WebSocket/CONNECT-UDP; turn forbidden input into a lease; copy/release/rebind
  ...
  premature DATA; skip/reorder phases; expose downstream success; cross bytes;
...
- Record early-final request generation, framing kind, disposition, bounded
  ...
  correlation stage, all four transition-input provenance classes, optimistic
  permit kind/binding, transfer/discard outcome, and acknowledgement offset,
  HTTP/1
...
- Fuzz CONNECT 2xx, Upgrade/WebSocket 101, and the v0.163.0/v0.187.0/v0.188.0
  ...
  exposure failure. Generate all four provenance classes, HTTP/1 ordinary
  CONNECT with/without close proof and early bytes through 2xx/non-2xx,
  ...
  SETTINGS/PING/HEADERS/DATA. Assert exact provenance gates, one transfer or
  discard, no ordinary-to-WebSocket rebinding, stream-local PROTOCOL_ERROR with
  no HTTP response, preserved unrelated frames/HPACK, no duplicate owner/
  credit, no close_notify-derived TCP half-close, and immutable first cause.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Reject caller construction, copying, rebinding, or direct mutation of
  ...
  caller construction/rebinding of TransitionInputProvenance or
  OptimisticConnectPermit, ordinary-CONNECT permission reused for WebSocket or
  CONNECT-UDP, forbidden-input BridgeInputLease minting, construction/copy/
...
- Independently size each configured early-final request's framing/progress/
  ...
  four-way input provenance, optimistic permit, transfer/discard disposition,
