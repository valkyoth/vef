...
- The sole work-authority edge is atomic `try_charge_admission_work(cost) -> Result<AdmissionWorkCharge, WorkBudgetExhausted>`. Preflight nonzero `cost`, both remaining counters, both consumed additions, generations, and both ledger invariants before mutation. Only if every check succeeds, one indivisible commit decrements both remaining counters and increments both consumed counters by the entire cost. Failure mutates neither ledger, performs no governed work, and closes before command inspection. The returned non-Copy `AdmissionWorkCharge { permit_generation, connection_generation, cost, permit_consumed_after, connection_consumed_after }` is linear diagnostic/accounting evidence consumed by the governed unit, never authorization. Rejection/capacity retains the modified permit budget; pure output-capacity failure charges only completed preflight work. Successful admission transfers the already-charged per-permit total into immutable Active diagnostics and never recommits or recharges attempt work.
- Before transition, transactionally assemble one internal non-Copy/non-Clone role-specific `Http10NextExchangeReservation::{Client { request: PendingHttp10SuccessorRequest, private_output, correlation, resources }, Server { parser, local_persistence_mode, correlation, resources }}`. Shared resources include the next-exchange record, parser/event/output storage leases, one tentative request/exchange-count debit, minimum parser-work reserve, checked deadline snapshot, and generation; the reservation never owns the deadline. The event lease is sized and reserved for the largest normal or cancellation terminal event so either can be encoded/replaced without later capacity acquisition. Initial connection/exchange construction uses an equivalent `Http10InitialExchangeReservation` and must reserve the identical maximum-size terminal-event lease before publishing the initial `ActiveExchange`, including configured-zero/default-close mode. Insufficient initial terminal capacity is a local construction failure that consumes no peer input and publishes no exchange. Every reservation is all-or-nothing; one-short capacity releases tentative ownership and preserves state apart from already atomically charged attempt work. Construction validates policy, exact unexpired permit deadline/binding, ledger count, and checked generation. Terminal admission reasons revoke permit/deadline, preserve ledger count, consume no input, and close locally without blaming the peer; exact deadline equality is expired and generation never wraps.
- On `Admitted`, atomically consume the reservation and permit; commit only the tentative request/exchange-count debit; install records, leases, and the parser-work reserve; transfer the already-consumed admission-attempt total into `ActiveExchange { next_exchange_generation, reuse_remaining_snapshot, local_persistence_mode, admission_attempt_work_consumed }` without recommit or recharge; decrement the reuse ledger once; install generation/mode; then publish. Capacity rollback retains the exact deadline and modified attempt budget. No structural fallibility remains after publication.
- `ActiveExchange` exclusively owns its committed exchange/correlation/evidence records and parser/event/output leases. Normal success atomically enters `Http10ConnectionState::Completing { transaction }`. The transaction has total internal phases `Http10CompletingPhase::{Resolving, DecisionHeld { decision }, Reclaiming { terminal_decision, reclamation_state }, PublicationPending { publication: PendingHttp10TerminalPublication }}`. Polling, timeouts, cancellation, and admission observe only `Completing`. `Resolving` keeps correlation/evidence and event capacity; `DecisionHeld` owns the unpublished decision; `Reclaiming` owns all unreclaimed resources plus generation-bound receipts for each completed release; `PublicationPending` owns the reserved slot, encoded event, terminal decision, receipts, and final generation. Each forward edge consumes its predecessor exactly once; no resolution or reclamation step reruns.
- Cancellation is total and capacity-infallible in every phase. In `Resolving`, it skips reuse resolution, selects cancellation-close, and begins exact-once reclamation. In `DecisionHeld`, it revokes any provisional permit, replaces the decision with cancellation-close, and enters reclamation. In `Reclaiming`, it revokes provisional reuse if still held, replaces the terminal decision, and continues only unreclaimed items from the receipts. In `PublicationPending`, it rewrites the held maximum-size event slot and next state in place. Every path constructs or retains one cancellation publication using the event lease already reserved by initial/successor construction, remains `Completing` through backpressure, and never reacquires terminal capacity. Only the final infallible consumption of `PendingHttp10TerminalPublication` atomically publishes one terminal event and constructs Reusable/closing state. Cancellation after that acts only on the published state.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Exhaust every role/evidence/lifecycle/framing/policy/generation combination and each close reason. For atomic `try_charge_admission_work`, cross unequal permit/connection remaining values, cost zero/one/greater-than-one, exact exhaustion, insufficient either side, consumed-add overflow, generation mismatch, and integer boundaries. Require both ledgers to preserve `configured_max == remaining + consumed`; failure mutates neither and performs no governed work; success mutates both once and returns one diagnostic-only charge record; admission transfers totals without a second charge.
- Inject cancellation/duplicate hooks in every `Resolving`, `DecisionHeld`, partial-`Reclaiming`, and `PublicationPending` position and during backpressure. Require phase-specific cancellation behavior, provisional-permit revocation, receipt-guided no-double-reclaim, retained maximum-size event slot, one cancellation publication, no capacity reacquisition, and only Completing observation until one infallible event/state publication. Test successor and initial `Http10InitialExchangeReservation` at one-slot-short/exact event capacity, configured maximum zero/default close, and cancellation in every completion phase; insufficient initial capacity publishes no exchange and consumes no input.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance extension: Audit v0.75.0 as one causal product: total cancellation across `Http10CompletingPhase::{Resolving, DecisionHeld, Reclaiming, PublicationPending}`; atomic all-or-nothing `try_charge_admission_work` preserving both ledger equations and returning diagnostic-only evidence; exact Active transfer without recommit/recharge; and identical maximum terminal-slot reservation for initial/configured-zero and successor exchanges. Treat partial charge, governed work after charge failure, double reclaim, phase without cancellation behavior, initial Active without a slot, or capacity reacquisition as release-blocking.
...
- HTTP/1.0 API extension: expose neither Completing phase transitions/cancellation actions, either work ledger, `AdmissionWorkCharge`, reclamation receipts, initial/successor event slots, nor Active work-transfer construction. Callers cannot split ledger mutation, use charge evidence as authority, omit initial terminal capacity, or recommit/recharge consumed work.
...
- HTTP/1.0 diagnostics extension: report exact Completing phase and cancellation edge; atomic charge cost, both ledger before/after totals and invariant failure; Active transferred total; initial/successor slot origin; reclamation receipts; and final generation. Never make `AdmissionWorkCharge` or phase state reusable authority.
...
- HTTP/1.0 fuzz extension: inject cancellation in every four-phase and partial-reclamation position; cross unequal ledger remaining amounts, zero/one/multi-unit cost, exact exhaustion, either-side shortage, overflow, and generation mismatch; and test initial configured-zero plus successor one-short/exact terminal slots. Assert both-or-neither mutation, no governed work on failure, no double reclaim/charge, and no slot omission/reacquisition.
...
- HTTP/1.0 compile-fail extension: reject direct construction/cross-generation movement of Completing phases, cancellation actions, either ledger, `AdmissionWorkCharge`, Active transferred-work field, initial/successor slot, and receipts. Reject split-ledger mutation, charge-record authorization, initial Active without a slot, and recommit/recharge APIs.
...
- HTTP/1.0 soak extension: cycle atomic multi-cost charges across many permits and every cancellation phase across partial reclamation/backpressure for initial configured-zero and successor exchanges. Require invariant preservation, no partial/double charge or reclaim, no slot/receipt leak, one terminal publication, and exact capacity restoration.
...
- HTTP/1.0 role extension: neither role may select a Completing cancellation edge, mutate one work ledger, retain a charge record as authority, construct Active work totals, omit the initial/successor terminal slot, or forge reclamation receipts. Pentest partial-charge faults, multi-cost overflow, partial-reclaim cancellation, initial-slot bypass, and slot theft.
...
- HTTP/1.0 sizing extension: independently bound all four Completing phases and cancellation edges, atomic dual-ledger preflight/mutation and charge records for multi-unit cost, Active consumed-work field, initial/successor maximum event slots, partial-reclamation receipts, and backpressure. Duplicate charge, decision, slot, receipt, or cleanup ownership is forbidden.
