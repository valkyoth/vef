...
- Before mutating either work ledger, charging validates the work-kind-to-cursor mapping and preflights `cursor_before + charged_units` into `cursor_limit`; invalid cursor selection or arithmetic selects typed local `AdmissionWorkCursorInvariantFailure`, poisons the attempt, inspects no command data, and creates no permit/window/receipt. Thus every possible window advancement and final receipt calculation is infallible. Atomic `try_charge_admission_work<'attempt, 'command>(&'attempt mut AdmissionAttempt<'command>, spec) -> Result<AdmissionWorkPermit<'attempt, 'command>, AdmissionWorkChargeError>` retains the prior charge errors and mutates both ledgers or neither only after that preflight. Private non-Copy/non-Clone `AdmissionWorkPermit<'attempt, 'command> { attempt: PhantomData<&'attempt mut AdmissionAttempt<'command>>, cursor_kind, cursor_before, cursor_limit, kind, charged_units, cost, permit_generation, connection_generation, permit_consumed_after, connection_consumed_after, attempt_generation }` structurally borrows the exact attempt; numeric generation remains defense-in-depth and diagnostics.
- Governed work uses only `AdmissionAttempt::with_charged_window(spec, |window: &mut ChargedWorkWindow<'_, 'command, T>| ...)`. The method owns the permit and window; the closure receives only a non-movable mutable borrow, so early `?`, explicit abort, ordinary return, dropping the reference, or `core::mem::forget` of that reference cannot drop/forget the owned guard or bypass finalization. Byte/entry windows expose only the bounded command prefix derived privately from `AdmissionAttempt.command`; semantic, framing, output, and reservation windows expose only `take_unit()`, which must succeed before each operation. The original unbounded command/view is unavailable to validation code while the scope runs.
- On every closure result, including early error or explicit abort, the method consumes its owned window, copies the internally maintained `processed_units` and `cursor_after`—which can only move within preflighted `cursor_before..=cursor_limit`—updates the attempt cursor, and emits one receipt before returning `AdmissionWindowOutcome::{Completed, Aborted}`. Private `finish()` accepts no caller count and performs no fallible arithmetic. If an impossible internal guard/window invariant is detected, `AdmissionAttemptState::Poisoned` burns the charge, advances no cursor, emits no receipt, forbids further work/admission, and selects typed local `AdmissionWorkWindowInvariantFailure`; no refund occurs.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz/Kani target now and retain its minimized corpus. Test attempt generation and every cursor at maximum-minus-one, maximum, and exhaustion; corrupted work-kind/cursor selection; and addition/subtraction boundaries. Require failure before inspection/ledger mutation, poisoning where specified, and no inconsistent receipt. For every work kind, charge zero/one/maximum units and consume every bounded prefix or `take_unit()` count `0..=n`. Prove `AdmissionAttempt` stores the sole engine command view with no raw getter/parallel unbounded validation access; lifetimes reject permit escape/wrong-attempt coexistence; and the scoped method retains guard ownership across normal return, early `?`, explicit abort, dropped borrowed references, and attempted `core::mem::forget`. Require exactly one Completed/Aborted receipt or typed poisoned close, internally derived progress, no above-charge work, and no refund. Use instrumented iterators and direct beyond-window attempts. On every reason-only retry, mutate every command byte and prove prior state cannot skip validation; new cursors start at zero and repeated work is charged again. Require cumulative ceilings and no quadratic/unpriced traversal.
...
- Acceptance extension: Audit v0.75.0 as one causal product: `AdmissionAttempt` owns the sole private engine command reference; cursor/arithmetic preflight precedes ledger charge; and `with_charged_window` owns/finalizes every guard across normal, early-error, and explicit-abort closure results. Treat PhantomData-only command ownership, parallel raw engine command access, movable/forgettable owned windows, missing receipt after early return, finish-time arithmetic failure, inconsistent receipt cursor fields, generation/cursor overflow after inspection or ledger mutation, work without bounded yield/take, or uncharged/quadratic traversal as release-blocking.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz/Kani target now and retain its minimized corpus. Replay the complete `Connection` corpus and HTTP/1.0 causal product. Cross attempt/cursor maxima and exhaustion, corrupted cursor-kind selection, scoped window normal/early-`?`/explicit-abort/drop/forget attempts, every bounded prefix/`take_unit()` count, early malformed input, retries, command mutation, and arithmetic boundaries. Require sole stored command view, no raw parallel access, preflight before charge, unchanged ledgers where applicable, exactly one receipt or typed poisoned close, infallible receipt construction with consistent cursor fields, no lifetime/window escape or residual authority, bounded linear work, and one reclamation/publication. Preserve all completion-binding, terminal-attribution, HTTP/1 lifecycle, transition, and rejection corpora.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- HTTP/1.0 API extension: expose neither generation constructors, attempt/raw command getters, terminal constructors, work kind/spec/units, private permit/window, nor cursors. `AdmissionAttempt` privately stores the sole engine `&Command`; public and validation-stage APIs cannot obtain a parallel unbounded view. Only scoped `with_charged_window` owns the guard; callers cannot move/drop/forget it, bypass finalization with early return, report progress, fabricate work/receipts, or turn diagnostics into authority.
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  method, sole private command-bearing attempt and checked generation, O(1)
  unit/cursor preflight, structurally borrowed permits, scoped engine-owned
  windows, non-authoritative cursor-audit receipts and poison state, total
...
- HTTP/1.0 diagnostics extension: report the exact non-authoritative receipt fields plus scoped outcome Completed/Aborted, and report attempt/window poison causes separately. Receipt cursor fields come only from preflight-bounded internal window state and are always consistent. Diagnostics retain no command/view/attempt borrow, cursor, permit, window, or work authority and cannot reveal command contents.
...
- HTTP/1.0 fuzz extension: cross sole-command-view enforcement, attempt/cursor exhaustion, corrupted cursor-kind mapping, scoped window normal/early-`?`/abort/drop/forget behavior, every bounded prefix/`take_unit()` count, stale/cross-attempt work, poison cleanup, receipt misuse, instrumented iterators, early malformed bytes, fragments, retries, and arithmetic errors. Assert preflight-before-charge, exactly one receipt or poison close, infallible consistent receipt fields, no missing finalization/under-reporting/residual authority, and no unpriced/quadratic work.
...
- HTTP/1.0 compile-fail extension: reject raw command extraction from `AdmissionAttempt`, parallel unbounded validation views, attempt/cursor escape, permit lifetime erasure/wrong-attempt coexistence, owned-window escape/move/drop/forget, closure extraction of the borrowed window, work fabrication/reuse, receipt escalation, caller-reported progress, cursor rewind, and byte/entry/semantic/reservation work without bounded yield or successful `take_unit()`.
...
- HTTP/1.0 soak extension: cycle attempt/cursor domains through saturation/exhaustion, mutate commands between retries, and run scoped windows through every normal/early-error/abort result, bounded prefix/unit split, maximum fragment, and early rejection until cumulative exhaustion. Require no parallel raw access, missing/double finalization, post-exhaustion ledger mutation, poison recovery, receipt inconsistency, stale authority, unused-unit refund, free retry/rescan, quadratic growth, or accounting drift.
...
- HTTP/1.0 role extension: neither role may obtain the stored command view, parallel unbounded work access, attempts, permits, windows, cursors, progress input, or receipt authority. Pentest raw-view leakage, closure/window escape via early `?`/drop/forget, missing/double finalization, preflight ordering, cursor overflow/corrupt kind, poison bypass, receipt inconsistency/forgery, missing `take_unit()`, stale work, retry bypass, and fragmentation amplification.
...
- HTTP/1.0 sizing extension: independently bound one private command-bearing attempt, generation/cursor domains, O(1) cursor-limit preflight, one scoped permit/window, bounded view or unit counter, one Completed/Aborted receipt, and poison state/cause. No parallel engine command view, owned window outside the scope, finish-time arithmetic, caller progress input, or inconsistent receipt exists; duplicate finalization, cursor, permit/window, receipt authority, or reuse-permit ownership is forbidden.
...
- Independently size each configured early-final request's framing/progress/
  ...
  sole private command-bearing attempt, checked generation/cursor domains,
  O(1) cursor-limit preflight, structurally borrowed permit, scoped owned
  window finalization, diagnostic cursor-audit receipt and poison outcome,
