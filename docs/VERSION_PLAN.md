...
| Never-untracked HTTP/2 rejection state | storage at `0.117.0`; terminal state/stages at `0.118.0`; flag/block handoff at `0.120.0`/`0.123.0`; semantic owners at `0.125.0`–`0.131.0`; push at `0.145.0`; assembly at `0.181.0`; diagnostics at `0.193.0` | Retain policy, wire, reset reservation, terminal state/stage, closure, and active block independently. Sealed monotonic transitions prevent intermediate release or impossible state combinations; tracking failure transfers cleanup without forgetting the ID. |
...
| `0.117.0` | Generation-checked stream table and tombstones | Requires HTTP/2 stream-identifier rules; retains reset reservation, terminal state/stage, closure, and field-block ownership independently, including dormant mandatory slots; Unlocks Exhaustive stream-state graph. |
| `0.118.0` | Exhaustive stream-state graph | Requires Generation-checked stream table and tombstones; defines NotTerminal, PendingFieldBlock(validate/deferred-abort), PendingSemantics(stage), Valid, Malformed, and AbortedByPeerReset with sealed monotonic/capacity-safe transitions; Unlocks HTTP/2 activation preface, first-SETTINGS, and deadline sequencing. |
...
| `0.123.0` | Atomic HPACK header-block integration | Requires HTTP/2 graceful GOAWAY and bounded shutdown sequencing; transfers PendingFieldBlock to PendingSemantics(PseudoFields), deferred peer-reset abort, or connection-owned fatal cleanup—never directly to Valid; Unlocks SETTINGS header-table encoder and header-list policy coupling. |
...
| `0.125.0` | Pseudo-field ordering and uniqueness | Requires SETTINGS header-table encoder and header-list policy coupling; owns PseudoFields and advances only to ConnectionFields or Malformed; Unlocks Connection-specific field and TE validation. |
...
| `0.131.0` | Informational responses and trailers | Requires HTTP/2 content-length, DATA, trailers, and END_STREAM reconciliation; owns final TrailerAndRoleRules, the sole HEADERS owner allowed to produce Valid/release after every prior stage; Unlocks Cookie field combination and Set-Cookie preservation. |
...
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; retains dormant reset through every stage, implements immediate/deferred peer-reset abort, GOAWAY preservation, and connection-owned cleanup transfer without duplicate release/re-arm; Unlocks SETTINGS outstanding-ACK accounting. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.138.0 foundations; completes the total table across every terminal stage, abort/cleanup owner, reset arbitration, END flags, phase, credit, HPACK, capacity, and output offset; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.181.0` | Client correlation, cancellation, and retry tokens | Requires Partial-response media-type classification plus v0.145.0 push admission; atomically preflights pushed resources and inherits staged validation, peer-reset abort, cleanup transfer, and immutable-reset arbitration; Unlocks Streaming partial-response and retained-prefix validation. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports bounded terminal stage, deferred/final peer-reset abort, reset dormancy/re-arm, cleanup owner, closure, and actual wire emission independently and redacted; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; pauses after every semantic stage and exhausts peer reset/GOAWAY/failure/capacity × reset offset × END flags × HPACK/message/credit outcomes; Unlocks Compile-fail state and lifetime tests. |
