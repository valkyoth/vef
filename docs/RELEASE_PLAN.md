...
- Define total `Http10ReuseResolutionOutcome::{Minted, CloseWithoutReuse { reason: Http10ReuseCloseReason }}` with reasons `ZeroConfiguredAllowance`, `LedgerExhausted`, `NegotiationUnavailable`, `FramingNotReusable`, `PolicyRevoked`, `DeadlineArithmeticOverflow`, and `CorrelationInvariantFailure`. Resolution runs after both message lifecycles are terminal and accepts their optional, generation-bound evidence; it is not preconditioned on successful negotiation. Apply one documented precedence: correlation integrity, policy, framing, configured-zero, exhausted ledger, missing exact two-sided negotiation, checked deadline arithmetic, then `Minted`. Only `Minted` requires and consumes both exact signals. It creates one sealed permit inside engine-owned `Reusable`; client/proxy-upstream resolution consumes correlated committed-request plus most-recent validated response evidence, while origin/server resolution consumes validated request plus committed response evidence. The permit binds the exact ledger generation and observed remaining count but does not decrement it. It exclusively owns `Http10IdleDeadline { connection_generation, permit_generation, expires_at }`; no reservation or ActiveExchange duplicates that deadline authority. Every close-without-reuse reason is reachable, creates no permit, returns same-call successor input wholly unconsumed, and closes without a peer protocol error; correlation invariant failure is a local security event. A proxy resolves evidence and ledger independently on each hop; received keep-alive evidence is never forwarded, transformed into local commitment, or reused across downstream/upstream connections.
...
- Define the sole successor-admission edge as `Http10ConnectionState::Reusable { permit: Http10ReusePermit } -> ActiveExchange { next_exchange_generation, reuse_remaining_snapshot, local_persistence_mode }`. Its total result is `Http10SuccessorAdmissionOutcome::{Admitted, RejectedLocalCommand { reason: Http10LocalCommandReason }, RetryableCapacity { reason: Http10CapacityReason }, CloseLocal { reason }}`. `RejectedLocalCommand` covers invalid method/target/ordered fields, illegal framing, semantic invalidity, and conflicting caller `Connection`; it retains no caller borrow, accepts/exposes no command byte, preserves the permit/deadline/ledger/output/structural state in `Reusable`, and permits a corrected command before the same absolute deadline. `RetryableCapacity` also carries only a freely constructible reason and preserves the same engine-owned authority. Every validation/preflight step consumes bounded monotonic `Http10AdmissionAttemptWork` as it is performed; this work is never refunded on rejection or capacity retry and is the only state allowed to change on either result. If insufficient work remains before inspection, return `CloseLocal::WorkBudgetExhausted` with the command uninspected. Pure close-injection/output-capacity failure is retryable after charging only work already performed.
- Before transition, transactionally assemble one internal non-Copy/non-Clone role-specific `Http10NextExchangeReservation::{Client { request: PendingHttp10SuccessorRequest, private_output, correlation, resources }, Server { parser, local_persistence_mode, correlation, resources }}`. Shared resources include the next-exchange record, parser/event/output storage leases, one request/exchange-count debit, the minimum parser-work reserve needed to reach safe backpressure, immutable checked permit-deadline snapshot/binding, and checked next generation; the reservation never owns the deadline. Construction validates current policy, `now < permit.deadline.expires_at`, exact deadline generation/value, and permit-to-ledger binding/count. It is all-or-nothing: every one-short storage, queue, count, or work failure releases incomplete ownership and leaves the exact permit/deadline, input, ledger, generation, and output unchanged apart from already charged attempt work. `CloseLocal` covers `RequestCountExhausted`, `WorkBudgetExhausted`, `PolicyRevoked`, `IdleDeadlineReached`, `PermitLedgerMismatch`, `CorrelationInvariantFailure`, and `GenerationExhausted`; every reason is local policy/resource/invariant state, revokes the permit/deadline, preserves the ledger count, returns input wholly unconsumed, and closes without blaming the peer. Exact deadline equality is expired and checked generation exhaustion never wraps.
...
- `ActiveExchange` exclusively owns its committed exchange/correlation records, negotiation evidence, and parser/event/output storage leases. Non-normal cancellation, parse failure, transport failure, or connection close uses generation-bound non-Copy cleanup exactly once. Normal success instead enters one internal linear `Http10NormalCompletionTransaction`: verify both message lifecycles terminal; resolve reuse while correlation and optional evidence remain owned; transfer consumed evidence into a provisional permit or terminal-close decision; release every non-transferred exchange/correlation/parser/event/output lease and unused parser-work reserve; then atomically publish exactly one terminal exchange event together with `Reusable` or closing state. No intermediate state is application-visible, and ordinary cleanup cannot release correlation/evidence ahead of resolution. If cancellation wins before publication, destroy/revoke any provisional permit and complete terminal cleanup without ever publishing `Reusable`; if commit wins, later cancellation acts only on the published state. Duplicate completion hooks are state-neutral. Ledger/count/admission-attempt/consumed-parser-work charges never refund, while all caller storage and unused reserve return exactly once; stale cleanup cannot affect a newer generation.
- The combined-call fast path exists only when acknowledgement causes the exact outbound HTTP/1.0 response lifecycle transition into `MessageCommitted`: for fixed-length output this is the final promised body byte, and for semantically bodyless output final head acknowledgement may also complete the message. Chunked/trailered output has no positive path and full acknowledgement of a nonfinal record does not qualify. The completion transaction permits exactly one resolution, terminal publication, or cancellation winner. Same-call successor input remains wholly unconsumed until the entire transaction publishes `Reusable`; only then may role-specific successor admission run. Invalid, zero, short, or nonfinal acknowledgement leaves it wholly unconsumed as `PrematureHttp10Successor`; `CloseWithoutReuse` also leaves it unconsumed. Across resolution, completion publication, immediate admission, cancellation, and repeated hooks there is at most one `Minted`, permit, ledger decrement, terminal event, and successor publication. After `Admitted`, any consumed input, accepted command, or exposed byte can only complete or abandon `ActiveExchange`; no failure rolls back to `Reusable`.
- Preserve the phase invariant: HTTP/1 has one octet-level inbound/outbound interpretation, bounded body ownership, exact transition handoff, typed dispositions, and no HTTP/0.9 fallback.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 1945, RFC 6455 opening-handshake requirements, RFC 9110, RFC 9111 preservation requirements, RFC 9112 and errata, the RFC 9298 non-applicability decision, and RFC 9931.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete v0.56.0 `Connection` corpus under HTTP/1.0 and HTTP/1.1; require identical lexical decisions and exact version bindings but version-specific outcomes. Exhaust every v0.74.0 matrix cell, evidence order, lifecycle boundary, role, hop, policy/framing/budget loss, newest-message invalidation, and forged/stale/cross-binding. Test reuse maxima 0, 1, and N and independently reach every ordered `Http10ReuseCloseReason`, including configured-zero versus exhausted-ledger and missing evidence without a false mint precondition. For client admission test malformed/overlong method and target, invalid ordered fields, illegal framing, semantic failure, conflicting `Connection`, and close-injection capacity; require sealed exact-command ownership only after validation. Prove `RejectedLocalCommand` and `RetryableCapacity` retain no caller borrow, expose no bytes, preserve permit/deadline/ledger/output/structural state, charge only bounded monotonic `Http10AdmissionAttemptWork`, and allow corrected/exact retry before the unchanged deadline; prove pre-inspection work exhaustion closes with an uninspected command. At one-short/exact capacity for every role-specific reservation component, require no partial ownership or publication and one atomic admission commit.
- Inject duplicate lifecycle hooks and cancellation before every `Http10NormalCompletionTransaction` step, at publication, and after publication. Require correlation/evidence to remain owned until resolution, provisional permits to be revoked when cancellation wins, all non-transferred leases and unused reserve to return once, irreversible accounting to stay charged, and exactly one terminal event plus state to become visible with no intermediate `Reusable`. Supply same-call successor input at every point; it remains wholly unconsumed until committed `Reusable`, then follows client validated-command or server parser admission. Test exact deadline equality, generation exhaustion, stale cleanup/expiry, bodyless/fixed positive paths, and Transfer-Encoding/chunked/trailer/close-delimited/nonfinal negative paths. Cross rejected HTTP/1.0 CONNECT with every outcome; it creates no permit, successor, tunnel, publication, forwarding, or HTTP/1.1 authority.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance extension: Audit v0.75.0 as one causal product: total `Http10ReuseResolutionOutcome` over optional evidence with every precedence-selected close reason reachable; sealed `PendingHttp10SuccessorRequest`; role-specific `Http10NextExchangeReservation`; reason-only `RejectedLocalCommand` and `RetryableCapacity` with monotonic `Http10AdmissionAttemptWork` under an unchanged deadline; and `Http10NormalCompletionTransaction` retaining correlation/evidence through atomic terminal event/state publication. Treat an unreachable outcome, retained caller borrow, accepted/exposed rejected command byte, free invalid retry, early evidence release, visible provisional permit, duplicate terminal event, or cancellation-created `Reusable` as release-blocking.
...
- HTTP/1.0 API extension: expose neither construction nor borrowing of `PendingHttp10SuccessorRequest`, either role-specific reservation, resolution evidence, the permit/deadline, completion transaction, provisional state, nor cleanup authority. A local-command rejection or capacity retry returns only a reason and progress accounting; it cannot retain a caller borrow, reset the absolute deadline, refund attempt work, or expose command bytes. Public polling observes only the atomically published terminal event/state pair.
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  lifecycle-bound exactly-once resolution state, `Http10ReuseResolutionOutcome`,
...
- HTTP/1.0 diagnostics extension: report the selected total reuse-resolution reason and precedence stage, local-command rejection class, charged/cumulative admission-attempt work, client/server reservation phase, and completion transaction winner/final generation. Never include command bytes, field values, correlation contents, or provisional permit state; emit one terminal event only after atomic publication.
...
- HTTP/1.0 fuzz extension: generate every reuse-resolution reason and overlapping precedence condition; malformed/overlong method/target/fields, illegal framing, semantic and conflicting-Connection commands; corrected retry and close-injection capacity failure under the same deadline; attempt-work exhaustion; both reservation variants; and cancellation/duplicate hooks at every completion-transaction step. Assert no retained borrow, early command byte, evidence-before-resolution cleanup, visible provisional state, duplicate terminal publication, free invalid retry, or cancellation-created reuse.
...
- HTTP/1.0 compile-fail extension: reject construction, copying, extraction, retention, or cross-generation rebinding of `PendingHttp10SuccessorRequest`, either reservation variant, optional resolution evidence, completion transaction, provisional permit, and cleanup authority. Also reject APIs that retain command borrows across reason-only rejection/capacity, reset deadline/attempt work, release correlation before resolution, or separately publish terminal event and connection state.
...
- HTTP/1.0 soak extension: cycle invalid-command/corrected-command retries, output-capacity recovery, every total resolution reason, and cancellation at every completion step across generation and capacity high-water marks. Require monotonic attempt work, unchanged absolute retry deadlines, one terminal event/state, no evidence/provisional-permit leak, exact reclaimable-capacity restoration, and no irreversible-accounting refund.
...
- HTTP/1.0 role extension: clients may admit only a sealed validated successor command, while servers admit parser capacity plus retained local persistence mode. Neither role may observe or retain reservation, evidence, deadline, provisional completion, or cleanup ownership. Pentest reason-only rejection/capacity retry and atomic completion publication for borrow retention, command smuggling, deadline extension, free-work amplification, double event, and reuse-after-cancellation.
...
- HTTP/1.0 sizing extension: independently bound immutable `configured_max` plus mutable ledger state, every resolution branch, validated command metadata/head plan, both reservation variants, per-attempt and cumulative invalid-retry work, completion transaction/provisional authority, and atomic terminal event/state publication. Include adversarial invalid-command retry amplification and cancellation at every completion step; duplicate evidence, permit, deadline, cleanup, or storage ownership is forbidden.
...
- Independently size each configured early-final request's framing/progress/
  ...
  `Http10PersistenceDisposition`/`ValidatedHttp10KeepAlive`/
  `CommittedHttp10KeepAliveHead`/`CorrelatedHttp10KeepAliveRequest`/
  `Http10ReuseLedger`/`Http10ReuseResolutionOutcome`/`Http10ReusePermit`, newest-
