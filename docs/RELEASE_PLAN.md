...
- Track `OutboundFieldBlock::{Private { stream, generation, hpack: HpackEncoderTransaction, initial, continuations }, FramingCommitted { stream, generation, hpack: HpackEncoderTransaction, remaining_continuations }, HpackCommitted, AbandonedWithConnection}` at connection scope. Before any initial HEADERS exposure, fully encode the bounded field block and atomically reserve/materialize every exact HEADERS/CONTINUATION slot plus queue entry needed to reach END_HEADERS. Segment its fragment payloads no larger than 16,384, the minimum legal peer MAX_FRAME_SIZE, so a valid reduction during the block cannot require resegmentation; any preflight shortage returns local backpressure with zero exposure. The whole Private block may then be superseded and all slots plus provisional HPACK mutations released together. First non-empty initial-frame exposure moves only to FramingCommitted; every remaining CONTINUATION is non-supersedable and its resources are already owned. No later field block is encoded against the provisional transaction. Full acknowledgement of the frame carrying END_HEADERS atomically publishes `HpackCommitted` and releases the block resources in order. No RST_STREAM, GOAWAY, PING ACK, SETTINGS ACK, WINDOW_UPDATE, or other-stream output can interleave. Fatal transport failure may abandon the transaction/block only with the entire connection and emits no interleaved GOAWAY. An END_STREAM completion hook on the initial HEADERS still runs when that frame is fully acknowledged, independently of final HPACK commit. v0.145.0 applies the same lifecycle to PUSH_PROMISE.
...
- Extend the HTTP/2 frame/state harness with every command ordering, partial
  ...
  needed by worst-case 16,384-byte fragmentation before initial exposure and
...
- Test increase/reduction activation at every outbound SETTINGS byte split,
  ...
  field-block preflight at 16,384 followed by peer reduction, checked 9-byte
...
- Outbound PUSH_PROMISE reuses the v0.137.0 connection-scoped `OutboundFieldBlock`: atomically materialize/reserve every worst-case-fragmented PUSH_PROMISE/CONTINUATION slot and queue entry plus its provisional HPACK transaction before first exposure. Its whole Private block may roll back, but first exposure selects FramingCommitted and guarantees drain through END_HEADERS against reset, GOAWAY, control replies, other streams, application cancellation, peer MAX_FRAME_SIZE reduction, and later local exhaustion. Promised-/associated-stream policy changes cannot supersede an individual continuation; final-frame acknowledgement publishes HpackCommitted, while only whole-connection abandonment discards an exposed transaction.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Public constructors reject overflow in `checked_add(9, max_outbound_frame_payload)`, insufficient slot/queue capacity for configured worst-case field blocks, and ambiguous padding policy. Public output acknowledgement accepts only the suffix range named by one record-bound token; vectored adapter convenience cannot batch slots or completion hooks.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Distinguish Private, FramingCommitted, HpackCommitted, and AbandonedWithConnection; report preflight slot/entry demand versus ownership, padding overhead/data bytes, one-record token identity, and provisional-versus-published HPACK generation without field values. Never diagnose first exposure as HPACK table publication or one acknowledgement as crossing slots.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Reject construction/rebinding of `HpackEncoderTransaction`, promotion from Private to FramingCommitted without the complete reserved slot/entry set, caller promotion to HpackCommitted, cross-record token acknowledgement, and any vectored API that can combine two frame slots under one token or lifetime.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Size each profile for its maximum encoded field block fragmented at 16,384 plus exact queue entries, or document a lower local accepted-field ceiling that makes the proof hold. Measure provisional HPACK transaction storage separately from frame slots and prove `9 + payload`/padding arithmetic at integer boundaries.
- Preserve the phase invariant: Adapters cannot alter protocol validity; TLS admission, EOF/alerts, deterministic resource ceilings, readiness, deadlines, storage, and release evidence remain explicit across targets.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7301, RFC 8446, RFC 9113 TLS prerequisites, platform ABI and target guarantees, the admitted Brynja contract, and all previously applicable HTTP requirements.
