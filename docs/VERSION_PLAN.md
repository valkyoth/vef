...
| Outbound field-block completion and HPACK commit | provisional transaction semantics at `0.98.0`; encoder SETTINGS arbitration at `0.124.0`; HEADERS preflight/ownership at `0.137.0`; reset/scheduler at `0.138.0`/`0.142.0`; PUSH_PROMISE at `0.145.0`; reserved controls at `0.153.0`; audit/diagnostics/fuzz/compile-fail at `0.154.0`/`0.193.0`/`0.196.0`/`0.197.0`; sizing at `0.211.0` | Before exposure, reserve exact slots/entries using the minimum peer/local/slot cap and checked block/continuation bounds. Private rollback discards provisional HPACK; FramingCommitted drains; final acknowledgement publishes HpackCommitted; pending table-size transition/ACK ownership blocks later encoding. |
| HEADER_TABLE_SIZE transaction arbitration | contract at `0.124.0`; scheduler/reserved ACK integration at `0.142.0`/`0.153.0`; audit/diagnostics/fuzz/compile-fail at `0.154.0`/`0.193.0`/`0.196.0`/`0.197.0`; resource replay at `0.211.0` | No-active applies immediately; Private rolls back then ACKs ahead of re-encode; FramingCommitted drains and publishes first. Retain smallest/final maxima for one or two next-block HPACK updates, acknowledge every SETTINGS frame once, and encode no later block until resolution. |
...
| `0.124.0` | SETTINGS header-table encoder and header-list policy coupling | Requires Atomic HPACK header-block integration; serializes HEADER_TABLE_SIZE with no-active/Private/FramingCommitted transactions, exact ACK obligations, smallest/final next-block updates, rollback/re-encode, and later-block exclusion; Unlocks Pseudo-field ordering and uniqueness. |
...
| `0.135.0` | SETTINGS initial-window active-stream integration and atomic rollback | Requires Connection flow control; revokes paired unexposed reservations before atomic deltas/re-reservation while preserving frozen debit and negative windows; Unlocks HTTP/2 inbound DATA ownership, acknowledgement, and credit release. |
...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; exposes only exact-cap-preflight blocks, serializes HPACK/table-size transitions, prioritizes Private-setting ACK before re-encode, and drains FramingCommitted before pending ACK; Unlocks SETTINGS max-frame-size outbound integration. |
| `0.143.0` | SETTINGS max-frame-size outbound integration | Requires Bounded outbound scheduling; preserves actual-local-cap-preflight blocks across peer reductions and computes DATA from checked peer/local/credit/slot budget minus padding overhead without truncation; Unlocks GOAWAY cutoff and retry classification. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.142.0 foundations; requires field cap room for the four-byte PUSH_PROMISE prefix and exact remaining-block continuation preflight under the shared HPACK lifecycle; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.153.0` | Reserved control-output queues | Requires WINDOW_UPDATE churn defenses; reserves one ACK per SETTINGS frame while collapsing only HPACK smallest/final updates, releases Private ACK before re-encode, and defers behind FramingCommitted; Unlocks HTTP/2 conformance audit and pentest. |
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits table-size transaction/ACK ordering, actual local fragment cap and mandatory/block/continuation bounds, HPACK publication, one-slot tokens, arithmetic, END_STREAM, and DATA credit; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.191.0` | Fixed-capacity caller-storage public API | Requires Authenticated origin authorization and HTTP/2 coalescing metadata; validates actual field cap, mandatory prefixes, block/continuation ceilings, transition/ACK reserves, checked slot/padding policy, and one-record acknowledgement without allocator; Unlocks Optional alloc-backed convenience API. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; distinguishes HPACK/setting transition states and ACKs, actual cap/prefix/block/continuation demand, one-record tokens, arithmetic, arena capacity, and DATA credit without content; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts HPACK/setting transitions and ACKs, all field caps/prefix/block/continuation bounds, re-encode failure, peer reductions, tokens, arena lifetimes, and credit; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects pending-transition construction, transaction promotion, bound/preflight bypass, cross-slot acknowledgement, staged-byte reuse, and early recycling; Unlocks Long-running soak and exhaustion campaign. |
...
| `0.211.0` | Deterministic CPU, stack, code-size, and amplification budgets | Requires Aesynx kernel integration tests; sizes maximum blocks at the smallest supported actual local field cap with prefix/continuation overhead, pending transition/ACK reserves, HPACK storage, slot arithmetic, and independent exhaustion; Unlocks 32-bit target campaign. |
