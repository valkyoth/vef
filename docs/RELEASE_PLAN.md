...
- Governed work uses `AdmissionAttempt::with_charged_window<'attempt, R, E, F>(&'attempt mut self, spec: AdmissionWorkChargeSpec, f: F) -> Result<AdmissionWindowOutcome<R, E>, AdmissionWindowTerminalError>` with `F: for<'scope> FnOnce(&'scope mut ChargedWorkWindow<'attempt, 'command, T>) -> Result<R, E>`. The distinct lifetimes are binding: `'attempt` is the structural window-to-attempt borrow, `'command` is the attempt-to-command borrow, and `'scope` is only the temporary closure reborrow. The higher-ranked `'scope` cannot appear in `R` or `E`; it must never be conflated with `'attempt`. The method owns the permit/window while the closure receives only a non-movable temporary mutable reborrow; early `?`, explicit `Err`, ordinary `Ok`, dropping the reference, or `core::mem::forget` of that reference cannot move/drop/forget the owned guard or bypass finalization. Byte/entry windows expose only the bounded command prefix derived privately from `AdmissionAttempt.command`; semantic, framing, output, and reservation windows expose only `take_unit()`, which must succeed before each operation. The original unbounded command/view is unavailable to validation code while the scope runs.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz/Kani target now and retain its minimized corpus. Test attempt generation/cursors at maximum-minus-one, maximum, and exhaustion; corrupted cursor selection; and arithmetic boundaries. For every work kind, charge zero/one/maximum units and consume every bounded prefix or `take_unit()` count. Prove the sole engine command view, preflight ordering, and structural permit lifetime. Add positive compile tests returning owned `R` and owned `E`. Add negative compile tests attempting to return/store the window, a bounded slice, an iterator item reference, or any other value borrowing `'scope`; none may escape through `R` or `E`. Cross `Ok(R)`, early `?`/`Err(E)`, explicit abort, dropped/forgotten borrowed references, and internal poison; require the exact inseparable Completed/Aborted value-or-error plus one receipt, or separate typed poison close with none. Verify production profiles are panic-abort. If unwind tests exist, inject panic at every window progress point and require guard cleanup to poison/burn/no-receipt before the outer harness observes it; never simulate Completed/Aborted. Retain command-mutation retry, instrumented iterator, beyond-window, cumulative-ceiling, and no-quadratic tests.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz/Kani target now and retain its minimized corpus. Replay the complete HTTP/1 causal product. Cross owned Ok/Err positive compile cases, distinct `'attempt`/`'scope` typing, higher-ranked borrow-escape compile failures, exact Completed/Aborted coupling, separate poison, production panic-abort configuration, and optional unwind injection at every window progress point. Require one inseparable result/receipt on closure return; no receipt and permanent poison after caught test unwind; no `'scope`-derived window/slice/iterator-item/other borrow in `R`/`E`; and no unwind recovery in production. Retain attempt/cursor maxima, corrupt kind, drop/forget, bounded prefix/unit, malformed input, retry, arithmetic, completion-binding, terminal-attribution, and lifecycle corpora.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Expose early-final policy preference without exposing progress or reuse
  ...
  unit/cursor preflight, structurally borrowed permits, higher-ranked scoped
  windows, inseparable generic result/receipt outcomes, separate poison state,
  abort-only production panic policy and non-authoritative diagnostics, total
...
- HTTP/1.0 diagnostics extension: report `Completed` with value metadata and receipt or `Aborted` with error metadata and receipt as one event; report terminal poison separately with no receipt. Never diagnose unwind as ordinary completion/abort. Receipt fields remain consistent and non-authoritative; diagnostics retain no `'scope`-derived window/slice/item borrow, command contents, or work authority.
...
- HTTP/1.0 fuzz extension: cross owned Ok/Err payloads, distinct `'attempt`/`'scope` typing, exact outcome/receipt coupling, higher-ranked borrow-escape attempts, scoped normal/early-`?`/abort/drop/forget behavior, poison, and optional unwind injection at every progress point. Assert production panic-abort, no unwind-to-success mapping, one receipt on closure return or none on poison, no `R`/`E` value borrowing `'scope`, and no missing finalization. Retain command-view, cursor, bounded-work, stale-work, fragmentation, retry, and arithmetic fuzzing.
...
- HTTP/1.0 compile-fail extension: positively compile owned `R` and owned `E`; reject conflating or weakening the structural `'attempt` lifetime and temporary higher-ranked `'scope` reborrow, and reject returning/storing the window, a bounded slice, an iterator item reference, or any other `'scope`-borrowed value in `R` or `E`; also reject separating a Completed/Aborted payload from its receipt; constructing poison as ordinary abort; raw command extraction; attempt/permit lifetime escape; owned-window move/drop/forget; work fabrication; receipt escalation; caller progress; cursor rewind; and work without bounded yield/`take_unit()`.
...
- HTTP/1.0 soak extension: cycle generic Completed/Aborted outcomes, poison, and optional unwind injection across attempt/cursor saturation, every bounded split, mutation/retry, and early rejection. Require inseparable result/receipt lifetime, no window-borrow escape, no missing/double receipt, no unwind success simulation, permanent poison/no receipt after caught test unwind, no post-exhaustion mutation, stale authority, refund, quadratic growth, or accounting drift.
...
- HTTP/1.0 role extension: neither role may choose, conflate, or weaken callback `'attempt`/`'scope` lifetimes or panic mode, borrow any `'scope`-derived value through `R`/`E`, separate result from receipt, map poison/unwind to ordinary outcome, or obtain command/work authority. Pentest higher-ranked escape, lifetime conflation, window/slice/iterator-item escape, receipt separation, poison-as-abort, early `?`/drop/forget, optional caught unwind, missing/double finalization, preflight/cursor faults, stale work, retry bypass, and fragmentation amplification.
...
- HTTP/1.0 sizing extension: independently bound one private attempt, its structural `'attempt` permit/window lifetime, temporary higher-ranked `'scope` reborrow, generic Completed-or-Aborted payload plus exactly one receipt, separate poison cause/state, and optional test-unwind cleanup metadata. The lifetime split is type-level and adds no runtime authority or duplicate window owner. Production is panic-abort. No `'scope`-derived result borrow, outcome/receipt separation, unwind success outcome, owned guard escape, finish arithmetic, inconsistent receipt, duplicate finalization, or receipt authority exists.
...
- Independently size each configured early-final request's framing/progress/
  ...
  O(1) cursor-limit preflight, structurally borrowed permit, higher-ranked
  scoped window finalization, inseparable generic result/receipt, separate
  poison terminal error, and abort-only production panic behavior,
