...
- Define a sealed non-Copy/non-Clone `StoredRepresentationSlice<'a>` created only by completing engine-controlled writes into caller-provided fixed storage. It inseparably owns an immutable lease over the exact arena/storage generation, byte interval, and `TransferDecodedContentEncoded` representation-byte domain—after HTTP transfer decoding but before Content-Encoding decoding, parsing, transcoding, or any other transformation. Every `ValidatedPartialSegment` and `ValidatedIncomplete200Prefix` contains this capability; safe APIs cannot acknowledge/recycle, mutate, evict, decode in place, change storage generation, or substitute newer backing bytes while the lease lives. Cancellation or rejection releases the lease without granting content authority; unsafe caller mutation violates the documented memory-safety precondition and can never be treated as validated input.
- When a GET receives a complete, semantically valid 200 head but content terminates prematurely, produce `ValidatedIncomplete200Prefix` with original request/correlation binding, stored-slice lease, target and selected-representation identity, content coding, expected and received length, prefix interval, representation metadata, validator evidence, and a typed incomplete-message disposition. Eligible termination is premature HTTP/1 EOF before Content-Length, EOF before the terminal chunk with otherwise valid chunk syntax, or HTTP/2 RST_STREAM/connection failure/cancellation before END_STREAM after a fully validated head. Clean HTTP/2 END_STREAM with Content-Length disagreement is a malformed response and stream PROTOCOL_ERROR with no accepted prefix; invalid fields/pseudo-fields, connection-fatal HPACK/framing failure, ambiguous HTTP/1 framing, or malformed chunk syntax likewise yields no prefix capability. An accepted incomplete response is never complete: HTTP/1 is non-reusable, while HTTP/2 preserves the connection only when connection state remains synchronized. Unsupported coding, unknown length, overflow, or unsafe prefix storage produces a discard disposition.
- Classify only a successfully completed, framing- and semantics-valid correlated 200 as `FullRepresentationFallback`; v0.181.2 invalidates contexts sharing its `AssemblyReplacementKey`, independent of validator equality, while a truncated 200 follows the incomplete-prefix/discard path. Classify multipart/byteranges as opaque `NeedsMultipartConsumer` without boundary/part parsing or recombination authority. A proxy preserves and forwards a well-formed unknown range unit even though a VEF client returns UnsupportedRangeUnit/NoRecombine; framing-valid semantic violations remain synchronized and recipient-policy controlled.
...
- Test exact/subset/outside requested ranges, complete-length changes, missing/duplicate/malformed/unknown Content-Range, single versus multipart Content-Type, exact/short/long/truncated 206 bodies, strong/weak/missing/mismatched validators all yielding structurally valid standalone segments where otherwise valid, `StandaloneOnly`/`NoRecombine`, and absence of combination/header/publication authority. Prove stored slices bind exact immutable bytes, intervals, arena/storage generations, and the transfer-decoded/content-encoded domain; reject mutation, eviction, acknowledgement/reuse, in-place decoding/transformation, newer-generation substitution, and stale backing metadata. Exhaust complete 200 fallback; eligible HTTP/1 EOF/missing-terminal-chunk and HTTP/2 reset/failure/cancellation; clean HTTP/2 END_STREAM length mismatch as malformed stream PROTOCOL_ERROR with no prefix; invalid field/pseudo-field, HPACK/framing, ambiguous framing, and malformed-chunk rejection; expected/received prefix accounting, unknown/overflowing length, transport reuse, discard, proxy unknown units, cancellation/backpressure, every split/limit, and arbitrary-input no-panic behavior.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
...
- Acceptance contract: Add a caller-created but non-forgeable, generation-safe, fixed-capacity `PartialAssemblyContext` for one `AssemblyReplacementKey` comprising effective target, representation-selection/Vary identity, content coding and representation-byte domain, and client storage/navigation generation; its old strong validator and known complete length remain combination constraints but are not part of replacement identity. Each v0.181.1 segment and stored-slice lease remains permanently bound to its original validated request/correlation/storage generations; the context may admit segments from different Range requests only after exact context matching and refines them to `CombinablePartialSegment`, or refines a matching incomplete prefix to `CombinableIncomplete200Prefix`. Retries, redirects, cancellation, or caller input cannot rebind an admitted value, and stale/context-crossed values fail without mutation.
- Add a non-forgeable fixed-capacity `PartialCombinationPlan` that consumes only admitted combinable values with their immutable input leases. Before comparing overlap bytes or issuing work, preflight every input lease plus one distinct mutable `CombinationOutputLease`, all intervals, the bounded copy schedule, and output capacity. VEF 1.0 rejects any input/output byte alias and keeps every input immutably leased until commit or cancellation, so output writes cannot overwrite unread input. Mutation, eviction, transformation, acknowledgement/reuse, domain or generation change invalidates the plan. Execution writes only through the verified schedule; success atomically publishes and consumes the output lease, while cancellation/failure marks any partial output invalid and unpublished and releases inputs without modifying them.
...
- Any successfully completed, framing- and semantics-valid 200 with the same `AssemblyReplacementKey` atomically invalidates the superseded context, segments, leases, and plans regardless of whether its new validator is equal, changed, weak, or absent; a different Vary/selection identity, content coding/domain, or client storage/navigation generation cannot cross-invalidate. A truncated 200 enters only through validated incomplete-prefix refinement or discard. No combined head, range, body write, or publication becomes observable before the entire plan, immutable input bytes, output lease, and synthesized metadata validate.
...
- Exhaust intervals, adjacency, every overlap/gap and equal/conflicting overlap byte arrangement while holding immutable leases; mutate/recycle/evict/decode/transform backing storage adversarially; reject input/output alias, stale storage/domain/generation, bad copy schedules, output exhaustion, and partial-output publication under cancellation. Positively combine distinct Range request generations/connections while rejecting rebound, retried-as-new, reused, standalone-only, multipart, and unknown-unit values. Test both refinements and incomplete-prefix paths; replacement-key invalidation with changed/weak/missing validators, no invalidation across Vary/coding/domain/storage generations, and truncated-200 retention; mint head ordinals at atomic head publication, delay/reorder bodies, retry, manipulate Date, mismatch source generations, and rotate on exhaustion. Exhaust header replacement, full/prefix/continuous/disjoint unions, corrected lengths, backpressure, and model-based byte/interval/header comparison with no panic or hidden allocation.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
...
- Acceptance contract: Provide compile-fail cases proving borrowed input/body/events cannot outlive storage, acknowledged chunks cannot be reused, stale stream/exchange generations cannot issue commands, role-specific builders cannot construct forbidden transitions, and fixed-capacity APIs cannot smuggle alloc/std ownership into core crates; additionally prove callers cannot construct/clone/copy/reuse/rebind civil-time evidence, CurrentRepresentationEvidence, WouldBe200Snapshot, RequestContentPermit, MethodExecutionPermit, ValidatedConditionalRequest, PartialResponseDisposition, StoredRepresentationSlice, ValidatedPartialSegment, ValidatedIncomplete200Prefix, CombinablePartialSegment, CombinableIncomplete200Prefix, AssemblyReplacementKey, PartialAssemblyContext, ReceiptOrderSource, ResponseHeadReceiptOrdinal, CombinationOutputLease, PartialCombinationPlan, or `ResponseEmissionPermit`; separate any capability from its frozen data; publish content/execute methods before authorization; pass raw request/response data beside a token through either engine/facade; alias input/output leases; mutate, decode, acknowledge, recycle, or evict leased bytes; mutably alias validated buffers; use validated data for the wrong protocol/role/generation/domain; issue body/trailer commands outside its response generation; forge/rebind trailer permissions; or fabricate v0.180.2/v0.180.3 conditional/range outcomes.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
