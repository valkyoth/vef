...
on v0.157.3 (Proxy-authentication hop ownership and 407 lifecycle) and must be independently trustworthy before v0.158.0 (Max-Forwards TRACE and OPTIONS intermediary semantics) begins.
...
reproducible evidence; v0.157.4 (Injected civil time and HTTP-date policy) still passes; no behavior assigned to v0.159.0 (HTTP/1 TE request-field and trailers forwarding semantics) is
claimed; the active resource profile passes; and no critical/high finding is
open.
...
on v0.180.3 (Bounded byte ranges and single-range response planning) and must be independently trustworthy before v0.180.5 (Dependency-free media type grammar and partial-content classification) begins.
...
The outbound conditional/range request contract and all previously implemented relevant behavior have reproducible evidence; v0.180.3 (Bounded byte ranges and single-range response planning) still passes; no behavior assigned to v0.180.5 (Dependency-free media type grammar and partial-content classification) is claimed; the active resource profile passes; and no critical/high finding is open.

`0.180.4 implementation stop reached. Run pentest for this exact commit.`

### v0.180.5 — Dependency-free media type grammar and partial-content classification

Status: planned

#### Goal

Deliver **Dependency-free media type grammar and partial-content classification** as the sole primary capability in this stop. It builds
...
- Acceptance contract: Add a separate dependency-free, no_std `vef-media-type` crate depending only on `vef-core`; implement bounded incremental parsing of media type, subtype, ordered parameters, tokens, quoted strings, quoted-pair escapes, and permitted whitespace with independent raw-byte, field-count, parameter-count, decoded-byte, and work limits. Preserve raw values while comparing type/subtype and parameter names case-insensitively; reject invalid token/quote/escape syntax and duplicate parameter names without substring, prefix, or suffix classification.
- Provide a sealed `PartialContentTypeClassification::{Absent, SinglePart, MultipartByteRanges, Other}` only after the entire ordered Content-Type field set validates. More than one Content-Type field or a malformed present value is a framing-synchronized received semantic violation and yields no classification capability. Recognize `multipart/byteranges` by exact case-insensitive type/subtype comparison regardless of parameter order/casing; require a valid boundary parameter for multipart classification, preserve its quoted/unquoted value under limits, and never treat misleading names such as multipart/byteranges-x or x-multipart/byteranges as multipart.
- Starting here `vef-conditions` depends on `vef-media-type` and accepts only its sealed classification in v0.181.1. Multipart classification can never grant top-level Content-Range, stored-segment, or combination authority; absent/other valid Content-Type can enter the single-part path only when every independent 206 and Content-Range check passes. Malformed/duplicate values remain synchronized under recipient policy but produce no validated partial head.
- Update paragraph-addressable RFC 9110 media-type, Content-Type, 206, multipart/byteranges, and security requirements plus threat model, API docs, release notes, traceability, resource measurements, and conformance corpora.

#### Verification

- Exhaust type/subtype casing, permitted whitespace, token/quoted parameter values, escapes, boundary quoting and empty/maximum values, parameter order, duplicate parameters, absent/single/duplicate fields, malformed quotes/escapes, every misleading exact-match prefix/suffix, multipart with top-level Content-Range, all independent limits, every input split/truncation, round trips, arbitrary-input no-panic behavior, and no hidden allocation. Prove only exact sealed multipart classification reaches `NeedsMultipartConsumer` and no malformed/multipart input can reach single-range or combination paths.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.

#### Exit criteria

The media-type grammar and partial-content classification contract and all previously implemented relevant behavior have reproducible evidence; v0.180.4 (Outbound conditional and range request validation) still passes; no behavior assigned to v0.181.0 (Client correlation, cancellation, and retry tokens) is claimed; the active resource profile passes; and no critical/high finding is open.
...
on v0.180.5 (Dependency-free media type grammar and partial-content classification) and must be independently trustworthy before v0.181.1 (Streaming partial-response and retained-prefix validation) begins.

#### Deliverables

- Acceptance contract: Assign one generation-safe request token retaining the exact v0.180.4 `ValidatedConditionalRequest`, requested range, target/representation identity, and validator evidence; correlate informational/final responses, body/trailers, cancellation, reset, GOAWAY, and connection loss; classify possibly-unprocessed separately from replay safety, expose the correlation to v0.181.1 without caller rebinding, and publish no duplicate terminal event.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
reproducible evidence; v0.180.5 (Dependency-free media type grammar and partial-content classification) still passes; no behavior assigned to v0.181.1 (Streaming partial-response and retained-prefix validation) is
claimed; the active resource profile passes; and no critical/high finding is
open.
...
- Acceptance contract: Add a non-forgeable `PartialResponseDisposition` bound permanently to the v0.180.4 validated request and v0.181.0 request/correlation generation. Consume only v0.180.5 sealed Content-Type classification: `MultipartByteRanges` yields opaque `NeedsMultipartConsumer` and forbids top-level Content-Range/segment/combination authority; malformed or duplicate Content-Type yields a synchronized semantic violation and no partial capability; Absent/SinglePart/Other can continue only through every independent single-part check. Publish `ValidatedPartialResponseHead` only after exactly one understood bytes Content-Range, requested-range correlation, head semantics, and limits pass; it starts generation-bound inclusive body accounting. Missing, weak, or mismatching validators do not invalidate standalone streaming and produce `StandaloneOnly`/`NoRecombine`.
- Before consuming body bytes require one sealed `PartialDeliveryMode::{StreamOnly, RetainOnly, StreamAndRetain}`. `StreamOnly` publishes borrowed acknowledgement-capable `PartialBodyChunk<'a>` events without storage. `RetainOnly` writes/account bytes without application chunk publication. `StreamAndRetain` copies and accounts each chunk into retained storage before publishing its borrowed event, and acknowledgement cannot recycle transport input until that retention write commits. Issue `StandalonePartialComplete` only after exact length and the entire message, including permitted trailers, complete; malformed, out-of-request, unknown-unit, short, or long responses never produce completion proof.
...
- At atomic validated-head publication, the correlation engine parses bounded Vary field-name lists and mints sealed `VariantSelectionKey` from the exact correlated original request, response generation, opaque caller representation principal, cache/privacy partition, and navigation generation. Deduplicate Vary names case-insensitively; `*` anywhere yields permanent `NoRecombine`; preserve absent versus present-empty request fields; combine multiple request field lines only for definitions explicitly marked combinable; apply only sealed field-specific semantic normalizers already implemented by VEF, otherwise retain a conservative ordered raw comparison. Callers supply the opaque principal but cannot construct or replace the engine-derived Vary component. Received trailer Vary deterministically yields `NoRecombine` and never changes selection, Content-Range, byte domain, or the head key.
...
- Classify only a successfully completed, framing- and semantics-valid correlated 200 as `FullRepresentationFallback`; v0.181.2 invalidates contexts sharing its `AssemblyReplacementKey`, independent of validator equality, while a truncated 200 follows the incomplete-prefix/discard path. A proxy preserves and forwards a well-formed unknown range unit even though a VEF client returns UnsupportedRangeUnit/NoRecombine; framing-valid semantic violations remain synchronized and recipient-policy controlled.
...
- Test sealed Content-Type absent/single/other/multipart/malformed/duplicate classifications and prove multipart never reaches top-level Content-Range. Exhaust all three delivery modes under every chunk split, storage-before-publication order, acknowledgement/recycle attempt, backpressure, cancellation, and capacity boundary. Prove 206 retention preflights before body, local capacity is not peer error, RetainOnly failure cancels, StreamAndRetain incomplete-200 exhaustion permanently continues StreamOnly/NoRetainedPrefix, and cancellation exposes a prefix only when all received bytes were retained. Test zero-retention ranges larger than every profile, terminal proof, optional safe leases, and DMA fencing. Exhaust head/trailer ETag finalization. For VariantSelectionKey test bounded/case-insensitive Vary deduplication, `*` anywhere, exact original-request binding, absent versus empty, combinable versus non-combinable repeated fields, sealed normalizers versus conservative comparison, caller principal non-forgeability, trailer Vary NoRecombine, stale/cross-generation keys, every split/limit, and no panic/allocation.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
...
- Acceptance contract: Add a caller-created but non-forgeable, generation-safe, fixed-capacity `PartialAssemblyContext` for one semantic `AssemblyReplacementKey`: effective target, the engine-minted v0.181.1 `VariantSelectionKey`, content coding and representation-byte domain, and its already-bound representation principal, cache/privacy partition, and navigation generation. Callers cannot supply a normalized Vary identity or replace any component of the selection key. Arena identity and `StorageLeaseGeneration` are excluded from this semantic key and remain physical validity evidence only inside stored-slice capabilities. `Vary: *` is never eligible for cross-request assembly. The old strong validator and complete length remain combination constraints, not replacement identity. Each `StoredPartialSegment` remains bound to its original request/correlation/storage generations; exact context matching refines stored inputs from different Range requests into `CombinablePartialSegment` or `CombinableIncomplete200Prefix`, while retries or caller input cannot rebind them.
- Add a non-forgeable fixed-capacity `PartialCombinationPlan` that consumes only admitted combinable values with immutable input leases. Before any copy, sort validated intervals under bounded work and perform one interval sweep under a dedicated `OverlapComparisonBudget`: overlapping equal octets deduplicate, but the first unequal octet returns `ConflictingPartialContent`, publishes no output, and never chooses newest/head-receipt order as byte authority. Budget exhaustion is a typed local `WorkLimit`, not a peer violation, and likewise produces no output. Obtain output storage only by safe non-overlapping slice splitting from the exclusively owned arena or from a completely separate sealed arena; then preflight its mutable `CombinationOutputLease`, every input lease, interval, bounded copy schedule, overlap comparisons, and capacity. No public unsafe/trust constructor or caller storage implementation can assert non-aliasing. Inputs remain immutably leased until commit/cancel; execution writes only through the verified schedule, success atomically publishes output, and cancellation/failure invalidates partial output and releases unchanged inputs. Physical lease rotation/reissue never changes semantic replacement identity.
- On `ConflictingPartialContent`, atomically quarantine the entire assembly context, its validator association, segments, leases, and plans before returning the error. A quarantined key accepts no further partial input and can be cleared only by a successfully completed same-key 200, a fresh navigation/selection key, or explicit same-key revalidation that proves a new current representation generation; retries, receipt order, storage rotation, or another partial response cannot clear it. Quarantine and work-limit paths expose no synthesized headers or bytes.
...
- Any successfully completed, framing- and semantics-valid 200 with the same semantic `AssemblyReplacementKey` atomically invalidates the superseded context, segments, leases, and plans regardless of validator or destination arena generation; a different Vary/selection identity, representation principal/privacy partition, navigation generation, content coding, or byte domain cannot cross-invalidate. A truncated 200 enters only through retained-prefix refinement or discard. Trailer-finalized combinability retains the original head ordinal, and no combined output becomes observable before every immutable byte, validator, output lease, and synthesized metadata check passes.
...
- Exhaust safe split/separate-arena output acquisition, compile-fail alias/mutation/recycle/decode attempts, stale physical lease generations, bad copy schedules, output exhaustion, cancellation, and DMA/device adapter fencing. Combine distinct request/connection/storage generations under one engine-minted selection key; reject caller-normalized/rebound keys, standalone-only, multipart, unknown-unit, principal/partition/navigation/Vary/coding/domain mismatch, and every `Vary: *`; prove arena migration neither preserves stale leases nor blocks same-key full-200 invalidation. Test head/trailer strong-validator finalization and conflicts without ordinal change; mint ordinals at head publication, delay/reorder bodies, retry, manipulate Date, and rotate on exhaustion. Exhaust disjoint, adjacent, fully/partly/nested overlapping intervals: equal overlap deduplicates; unequal overlap returns `ConflictingPartialContent`, emits no bytes/headers, quarantines the entire context, rejects subsequent partials, and clears only through an allowed complete-200/navigation/revalidation path. Exhaust `OverlapComparisonBudget` immediately before/within/after comparisons, prove typed local WorkLimit and no quadratic path with sorted-interval model tests, then verify header replacement, full/prefix/continuous/disjoint unions, corrected lengths, backpressure, and model comparison without panic or hidden allocation.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
...
- Acceptance contract: Provide compile-fail cases proving borrowed input/body/events cannot outlive storage, acknowledged chunks cannot be reused, stale stream/exchange generations cannot issue commands, role-specific builders cannot construct forbidden transitions, and fixed-capacity APIs cannot smuggle alloc/std ownership into core crates; additionally prove callers cannot construct/clone/copy/reuse/rebind civil-time evidence, CurrentRepresentationEvidence, WouldBe200Snapshot, RequestContentPermit, MethodExecutionPermit, ValidatedConditionalRequest, PartialContentTypeClassification, PartialResponseDisposition, ValidatedPartialResponseHead, PartialDeliveryMode, PartialBodyChunk, StandalonePartialComplete, VariantSelectionKey, StorageLeaseGeneration, StoredRepresentationSlice, StoredPartialSegment, ValidatedIncomplete200Prefix, CombinablePartialSegment, CombinableIncomplete200Prefix, AssemblyReplacementKey, PartialAssemblyContext, ReceiptOrderSource, ResponseHeadReceiptOrdinal, OverlapComparisonBudget, CombinationOutputLease, PartialCombinationPlan, ConflictingPartialContent, or `ResponseEmissionPermit`; separate any capability from its frozen data; publish content/execute methods before authorization; forge or substitute media-type/Vary/variant classifications; forge storage via traits/trust constructors; alias input/output leases; mutate, decode, acknowledge, recycle, or evict leased bytes; mutably alias validated buffers; use validated data for the wrong protocol/role/generation/domain/principal; change head decisions from trailers; issue body/trailer commands outside its response generation; forge/rebind trailer permissions; or fabricate conditional/range outcomes.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
