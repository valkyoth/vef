...
def main() -> int:
    ...
    required_contract_text += ("`OutboundSettingsTransaction::{ReservedPrivate { generation,", "`9 + 6 * entry_count`", "future outstanding-ACK FIFO slot", "`EarlyPeerAckPending` indication", "Track `locally_requested`", "sole eligible frame until its full commitment", "Full-frame acknowledgement atomically applies the prevalidated commit", "Peer ACK consumes the oldest CommittedAwaitingAck record exactly once", "Fatal GOAWAY before exposure selects AbandonedBeforeExposure", "commit plan is the sole wire-commit authority", "outbound non-ACK SETTINGS", "no allocator or fallible capacity step", "Audit `OutboundSettingsTransaction`")
