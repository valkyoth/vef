...
def main() -> int:
    ...
    required_contract_text += ("sole bounded, version-neutral `Connection` field parser", "return sealed `ValidatedConnectionOptions` bound to the exact ordered field set, exact HTTP version", "No later milestone may reparse or independently normalize `Connection`", "At this stop activate only the HTTP/1.1 persistence interpretation", "`ReceivedValidatedCloseOption` can be refined only from the exact v0.56.0 `ValidatedConnectionOptions`", "do not parse, split, case-fold, trim, or normalize `Connection` again", "sealed `ValidatedHttp10KeepAlive` refinement of the exact HTTP/1.0-bound v0.56.0 `ValidatedConnectionOptions`", "This refinement authorizes only the HTTP/1.0 keep-alive lifecycle", "cannot construct `ReceivedValidatedCloseOption`, authorize optimistic CONNECT, bind HTTP/1.1 Upgrade", "Consume the exact ordered nominations in v0.56.0 `ValidatedConnectionOptions` for either HTTP/1.0 or HTTP/1.1", "typed `UnsupportedHttp10ConnectDisposition`", "local client builder returns zero-output `UnsupportedVersionMethod`", "receiving server or proxy selects one bounded semantically valid 501 Not Implemented response plus mandatory transport close", "never resolves or forwards the authority", "discard them exactly once", "Partial 501 output followed by EOF/cancellation/transport failure closes", "existing deterministic zero-partial-output close action", "every bounded 501 output acknowledgement offset", "attempt application publication, authority resolution, forwarding, successor parsing, and tunnel creation", "one atomic runtime transition that consumes the exact `CommittedDownstreamSuccess`", "pre-reserved permit slot", "clear `PrematureInputLatch` snapshot", "matching `DownstreamSuccessFrozen` bridge generation", "duplicate/stale acknowledgements, repeated completion hooks, cancellation races, and generation reuse", "At final commitment inject duplicate and stale acknowledgements", "at most one permit and success event"); failures.extend(f"missing concrete security contract: {contract_text}" for contract_text in required_contract_text if contract_text not in detailed)
