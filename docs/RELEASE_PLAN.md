...
- Test client-originated PUSH_PROMISE connection failure; every method/content/trailer/field-section case; TLS, cleartext, proxy, stale/cross-connection/cross-generation and foreign authority evidence without v0.190.0; sender/receiver associated-state perspectives; promised-ID, GOAWAY, reserved-slot/work and tombstone/cutoff exhaustion, reservation at MAX_CONCURRENT_STREAMS zero, open-time concurrency rejection, cancellation/atomic rollback/in-place rejection conversion/slot reclamation, duplicate promise and post-rejection frames, mandatory-output backpressure, unpublished admission provenance, promised-versus-associated isolation, absence of partial/assembly authority, non-cacheable metadata, never-idle tracking, exactly-once release, and all earlier behavior.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Extend the v0.145.0 receiving-client PUSH_PROMISE transaction after the complete promised field block and all CONTINUATION input have decoded, HPACK is synchronized, and promised-request semantics, authority, safe/cacheable method, and caller push policy pass, but before promised-request publication. Atomically preflight/commit the existing promised-stream slot, matching `AssemblyInvalidationHandle`, independent pushed-provenance storage, and mandatory rejection-tombstone/cutoff tracking. If handle or provenance capacity is unavailable, convert the already tracked provisional promised slot in place to `RejectingPromised` without first releasing it; publish no promised request or assembly correlation, leave the associated stream and unrelated contexts unchanged, and schedule stream-local `RST_STREAM(CANCEL)` for the promised ID as permitted by RFC 9113 Section 6.6. The ID is never momentarily untracked. Keep the tombstone through mandatory-output backpressure until local CANCEL commits and retained peer stream-ID/cutoff evidence can classify all later traffic; peer RST_STREAM before local CANCEL, duplicate promise, and later HEADERS/CONTINUATION/DATA are handled as known rejected/closed traffic, with field blocks synchronized and DATA receive credit accounted. If mandatory rejection/cutoff tracking itself cannot be represented, retain the provisional slot and initiate typed bounded `PushRejectionTrackingUnavailable` connection shutdown—never release/reacquire or forget the promised ID. Every provisional slot, handle, provenance lease, tombstone, and shutdown record releases exactly once at its defined terminal horizon.
