...
| Outbound field-block completion and HPACK commit | provisional transaction semantics at `0.98.0`; encoder SETTINGS arbitration at `0.124.0`; HEADERS preflight/ownership at `0.137.0`; reset/scheduler at `0.138.0`/`0.142.0`; PUSH_PROMISE at `0.145.0`; reserved controls at `0.153.0`; audit/diagnostics/fuzz/compile-fail at `0.154.0`/`0.193.0`/`0.196.0`/`0.197.0`; sizing at `0.211.0` | Before exposure, reserve exact slots/entries using the minimum peer/local/slot cap and checked block/continuation bounds. Private rollback discards provisional HPACK; FramingCommitted drains; final acknowledgement publishes HpackCommitted; a pending table-size transition and its owning frame transaction block later encoding. |
| Frame-wide inbound SETTINGS transaction | skeleton at `0.108.0`; HPACK/window/core/admission/frame-size/push participants at `0.124.0`/`0.135.0`/`0.139.0`/`0.141.0`/`0.143.0`/`0.145.0`; amplification/reserved output at `0.149.0`/`0.153.0`; audit/diagnostics/fuzz/compile-fail at `0.154.0`/`0.193.0`/`0.196.0`/`0.197.0`; sizing at `0.211.0` | Fully validate, reserve one transaction/ACK, then apply ordered entries through generation-bound participants. ACK once after all effective and in receive order; fatal failure cancels ACK and connection. Duplicates/mixed/unknown settings never create subsystem ACKs. |
| HEADER_TABLE_SIZE transaction arbitration | participant contract at `0.124.0`; scheduler/shared ACK at `0.142.0`/`0.153.0`; audit/diagnostics/fuzz/compile-fail at `0.154.0`/`0.193.0`/`0.196.0`/`0.197.0`; resource replay at `0.211.0` | Pending transition owns smallest/final maxima and SETTINGS-owner references, never ACKs. No-active applies; Private rolls back then re-encodes independently; FramingCommitted drains/publishes first. No later field block encodes until resolution. |
...
| `0.108.0` | SETTINGS syntax, role, directional values, and ACK rules | Requires SETTINGS frame codec; fully prevalidates ordered entries and reserves one generation-bound frame transaction/ACK before mutation, with participant and fatal-cancellation skeleton; Unlocks PING frame codec. |
...
| `0.124.0` | SETTINGS header-table encoder and header-list policy coupling | Requires Atomic HPACK header-block integration; makes HEADER_TABLE_SIZE a shared SETTINGS participant whose transition owns smallest/final values and owner references but no ACK; covers rollback/re-encode and later-block exclusion; Unlocks Pseudo-field ordering and uniqueness. |
...
| `0.139.0` | SETTINGS outstanding-ACK accounting | Requires HTTP/2 body cancellation, reset, and flow-credit lifecycle; integrates frame-wide ordered participants, all-effective/FIFO single-ACK eligibility, combined-entry atomicity, and fatal cancellation across enabled components; Unlocks Bounded stream admission. |
...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; exposes only exact-cap-preflight blocks, serializes HPACK/table-size transitions, completes Private HPACK participation before independent re-encode, and schedules only all-effective FIFO frame ACKs after FramingCommitted; Unlocks SETTINGS max-frame-size outbound integration. |
...
| `0.153.0` | Reserved control-output queues | Requires WINDOW_UPDATE churn defenses; places one ACK slot in each validated frame transaction, releases only after all participants and FIFO ordering, cancels on fatal failure, and defers serialization behind FramingCommitted; Unlocks HTTP/2 conformance audit and pentest. |
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits mixed-entry shared transaction validation/participants/single-ACK/fatal cancellation plus table-size, field-block, token, arithmetic, END_STREAM, and credit contracts; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.191.0` | Fixed-capacity caller-storage public API | Requires Authenticated origin authorization and HTTP/2 coalescing metadata; keeps SETTINGS transaction/ACK/participant authority sealed while validating their reserves plus field/slot/padding and one-record APIs without allocator; Unlocks Optional alloc-backed convenience API. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; reports redacted frame-generation/participant/FIFO/single-ACK disposition plus HPACK transition, field layout, token, arena, and credit without misattributed ACK ownership; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts combined SETTINGS validation/participants/FIFO ACK/fatal paths, HPACK transition, field bounds, re-encode failure, tokens, arena, and credit; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects transaction/ACK construction or extraction, participant rebinding/early completion/reordering plus prior promotion/preflight/token/reuse violations; Unlocks Long-running soak and exhaustion campaign. |
...
| `0.211.0` | Deterministic CPU, stack, code-size, and amplification budgets | Requires Aesynx kernel integration tests; sizes simultaneous frame transactions, ordered entries, participants, one ACK each, FIFO/fatal cleanup separately from field/HPACK/arena bounds; Unlocks 32-bit target campaign. |
