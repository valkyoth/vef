...
def main() -> int:
    ...
    required_contract_text += ("can be minted only for legal success-following input", "upstream HTTP/1 101 plus over-read WebSocket bytes in the same read", "downstream RFC 8441 DATA before complete 2xx transport commitment", "Discard their owned bytes exactly once without HTTP reparsing or early credit restoration", "never enter Active, and never forward them after later success", "complete 101/2xx acknowledgement runs its success hook before input", "Valid zero or short acknowledgement followed by WebSocket input", "invalid acknowledgement remains state-neutral and leaves input wholly unconsumed", "`PendingHttp1Transition { overread: OverreadLease, terminal: Option<PreActiveTerminalCause>, bridge_generation, leg, exchange_generation, transport_generation }`", "`PlainTcpEof`, `TlsCloseNotify`, `FatalTlsAlert`, `TransportReset`, and `Cancellation`", "disposes bytes and terminal ownership exactly once", "TLS close_notify never fabricates TCP half-close authority", "HTTP/1 request head plus premature WebSocket bytes in one buffer", "Assert premature input creates no transferable lease or Active state", "Cross HTTP/1 legal over-read with plain TCP EOF, TLS close_notify, fatal TLS alert, reset, and cancellation at every bridge phase", "terminal events immediately before/after first downstream exposure")
