...
on v0.180.4 (Outbound conditional and range request validation) and must be independently trustworthy before v0.181.1 (Streaming partial-response and retained-prefix validation) begins.
...
reproducible evidence; v0.180.4 (Outbound conditional and range request validation) still passes; no behavior assigned to v0.181.1 (Streaming partial-response and retained-prefix validation) is
claimed; the active resource profile passes; and no critical/high finding is
open.
...
- Acceptance contract: Add a non-forgeable `PartialResponseDisposition` bound permanently to the v0.180.4 validated request and v0.181.0 request/correlation generation. A single-part 206 publishes `ValidatedPartialResponseHead` only after Content-Type, exactly one understood bytes Content-Range, requested-range correlation, head semantics, and limits pass; it starts generation-bound inclusive body accounting. Stream borrowed `PartialBodyChunk<'a>` events under the ordinary acknowledgement/backpressure contract without retention or allocation, and issue `StandalonePartialComplete` only after exact content length and the entire message, including permitted trailers, complete. Missing, weak, or mismatching validators do not invalidate otherwise valid standalone streaming and produce `StandaloneOnly`/`NoRecombine`; malformed, unknown-unit, out-of-request, or length-mismatched responses never produce terminal completion proof.
- Retention is explicit and optional. Before body delivery, a safe API accepts only an exclusive `&mut [u8]` or a sealed fixed-arena slot, gives engine-controlled writes exclusive access, and converts the completed written region into an immutable `&[u8]`-backed non-Copy/non-Clone `StoredRepresentationSlice<'a>` with exact `StorageLeaseGeneration`, interval, and `TransferDecodedContentEncoded` byte domain. There is no public trust-me constructor, caller-implementable storage trait, or safe path to aliased storage; safe code cannot mutate, acknowledge/recycle, evict, decode in place, or change generation while leased. `StoredPartialSegment` is optional and contains the lease inseparably; only it can refine at v0.181.2 into `CombinablePartialSegment`. External DMA/device adapters must quiesce and fence memory before yielding the exclusive slice and cannot expose it while device mutation is possible; reported post-lease device mutation is a semantic-integrity fault that invalidates the storage/connection generation, never a Rust memory-safety precondition imposed on safe VEF callers.
...
- Test head publication, every borrowed chunk split/acknowledgement/backpressure path, exact/short/long bodies, zero-retention streaming of ranges larger than every storage profile, terminal standalone proof, and absent/weak/mismatching validators. Test opt-in exclusive-slice/sealed-arena retention, exact immutable bytes/domain/generation, and compile-fail inability to mutate, recycle, decode in place, implement a forging storage trait, or obtain overlapping output; exercise safely split/separate arenas and adapter DMA quiesce/fence evidence. Exhaust ETag in head, trailer, neither, duplicate-equal, and conflicting forms; no stored finalization before trailers; immutable head ordinal and Content-Range/domain across trailers. Exhaust retained/non-retained incomplete responses, eligible transport termination, malformed HTTP/1/HTTP/2 exclusion, complete fallback, limits, cancellation, and arbitrary-input no-panic behavior.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
...
- Exhaust safe split/separate-arena output acquisition, compile-fail alias/mutation/recycle/decode attempts, stale physical lease generations, bad copy schedules, output exhaustion, cancellation, and DMA/device adapter fencing. Combine distinct request/connection/storage generations under one semantic key; reject rebound, standalone-only, multipart, unknown-unit, principal/partition/navigation/Vary/coding/domain mismatch, and every `Vary: *`; prove arena migration neither preserves stale leases nor blocks same-key full-200 invalidation. Test head/trailer strong-validator finalization and conflicts without ordinal change; mint ordinals at head publication, delay/reorder bodies, retry, manipulate Date, and rotate on exhaustion. Exhaust overlap bytes, header replacement, full/prefix/continuous/disjoint unions, corrected lengths, backpressure, and model comparison without panic or hidden allocation.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy, audit, SBOM, CI, and CodeQL default-setup gates.
