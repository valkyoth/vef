...
| Linear, bounded full-200 fallback invalidation | reserve at `0.181.0`; evidence at `0.181.1`; consume at `0.181.2`; stabilize at `0.191.0`; compile-fail at `0.197.0` | Before request output, reserve a non-Copy/non-Clone exact-correlation handle from isolated per-shard capacity. Exhaustion returns local AssemblyInvalidationCapacity with zero bytes/no correlation and applies Sans-I/O backpressure; it never rotates an arena or continues untracked. Hold the handle through informational/final processing and terminal backpressure, release exactly once at a terminal disposition, and independently reserve for retries. Keyed replacement stays exact; unkeyed replacement invalidates its namespace and widens only across coding/domain children. Arena rotation requires detected semantic/storage corruption or explicit caller policy, with assembly disable clearing the shard before untracked admission. |
...
| `0.181.0` | Client correlation, cancellation, and retry tokens | Requires Partial-response media-type classification integration; reserves one linear shard-bound invalidation handle before output or returns zero-byte AssemblyInvalidationCapacity backpressure; Unlocks Streaming partial-response and retained-prefix validation. |
| `0.181.1` | Streaming partial-response and retained-prefix validation | Requires the exact correlation and its held invalidation handle; consumes it into keyed/unkeyed fallback evidence and stores normalized identity canonically; Unlocks Cross-request partial assembly and header synthesis. |
| `0.181.2` | Cross-request partial assembly and header synthesis | Requires Streaming partial-response and retained-prefix validation; consumes fallback evidence with namespace-bounded invalidation and borrow-aware reclamation, while rotation is corruption/caller-policy only; Unlocks Retry safety, idempotency, and body-replayability contract. |
