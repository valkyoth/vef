...
- Store policy disposition, current RFC wire state, local reset-output progress/reason/reservation, remote-closure cause, terminal-validation state, and active field-block ownership as independent generation-checked fields; policy rejection, remote closure, or field-block completion alone never releases another dimension. A zero-byte policy-CANCEL reservation can become dormant after remote END_STREAM without freeing its slot, so later terminal validation can release it or re-arm the same bounded slot as a mandatory stream-error reset without allocation. A locally reset stream's tombstone retains required state through the closed-stream classification horizon, providing bounded storage for the v0.145.0 in-flight-push exception without retaining or recreating application authority.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- HEADERS+END_STREAM without END_HEADERS sets terminal validation to `PendingFieldBlock`. Successful HPACK completion advances to semantic validation; malformed HPACK commits connection-fatal shutdown, while a successfully decoded malformed message records `Malformed(StreamErrorCode)` and arbitrates the retained reset slot without changing wire closure or valid compression-table mutations.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- When END_STREAM already closed the wire stream, a completed field block with a forbidden/duplicate/unknown/misordered pseudo-field records terminal `Malformed(PROTOCOL_ERROR)` and consumes the retained mandatory reset reservation; it is never treated as valid completion merely because the peer ended first.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- If the initial block carried END_STREAM, every malformed-message result still becomes terminal `Malformed(PROTOCOL_ERROR)` after full HPACK/field validation and re-arms the dormant reset slot as `StreamError(PROTOCOL_ERROR)` unless immutable reset bytes or a connection-fatal decision already won; remote completion is not a reason to suppress the required stream error.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Missing, duplicate, malformed, or context-invalid `:status` on terminal HEADERS records stream `Malformed(PROTOCOL_ERROR)` after HPACK synchronization and re-arms the retained mandatory reset reservation; no response or terminal-valid event is published.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Classify initial, informational, final, and trailer HEADERS independently from END_STREAM/END_HEADERS. A HEADERS END_STREAM event can close the wire stream before its fragmented initial/final/trailer block completes, but the block owner, terminal validation, dormant reset reservation, and HPACK obligation remain. Any informational 1xx HEADERS carrying END_STREAM is explicitly terminal `Malformed(PROTOCOL_ERROR)` and requires stream RST_STREAM(PROTOCOL_ERROR); a valid final/trailer block marks terminal valid, while forbidden trailer pseudo-fields or other malformed terminal fields re-arm the same stream-error reset. Trailer HEADERS require END_STREAM and close exactly once.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Test Informational responses and trailers and all previously implemented relevant behavior with positive, negative, boundary, truncation, invalid-state, cancellation, capacity, and no-panic cases. Cross initial, informational, final, and trailer HEADERS with END_STREAM/END_HEADERS present and absent, valid/missing/malformed status, pseudo-fields, names, values, and HPACK success/failure; prove 1xx+END_STREAM and malformed terminal blocks yield stream PROTOCOL_ERROR, valid terminal blocks release the dormant slot, connection-fatal HPACK wins, two-step wire transition and CONTINUATION drain remain unchanged, and exactly one terminal/error action commits.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Emit at most one RST_STREAM, account discarded DATA credit, tolerate bounded in-flight frames, discard queued output after reset, resolve END_STREAM/GOAWAY/shutdown races, and recycle only after connection effects. Track reset output as `ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByPeerReset, PolicyDormantAfterEndStream}`, reason as `ResetReason::{PolicyCancel, StreamError(StreamErrorCode)}`, and terminal validation independently. Peer RST_STREAM can supersede a zero-byte action; remote END_STREAM instead moves only an uncommitted policy CANCEL to `PolicyDormantAfterEndStream` and retains its mandatory slot until validation. `Valid` releases the dormant slot; `Malformed(code)` atomically re-arms it as queued `StreamError(code)` without a capacity check. If policy CANCEL serialization already began, finish that immutable frame once and emit no second reset after later malformed validation; if a stream-error reset is queued/partial/committed, never downgrade or duplicate it. Connection-fatal HPACK/framing failure commits bounded connection shutdown and no new stream reset, while any partially serialized frame obeys output framing. Field-block ownership and wire closure remain independent through the terminal decision.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone. Race valid/malformed terminal HEADERS/DATA, peer RST_STREAM, remote END_STREAM, HPACK failure, connection failure, GOAWAY, cancellation, and every policy/error reset output offset. Cross END_HEADERS fragmentation and terminal-validation states. Require zero CANCEL when valid remote completion wins before byte zero; exactly one re-armed RST_STREAM(PROTOCOL_ERROR) when later stream-malformed validation wins; exactly one immutable CANCEL when its serialization already began; no second reset after a committed stream-error reset; bounded connection shutdown on fatal HPACK; no duplicate/restart/partial resume; and no stranded CONTINUATION. A connection failure after partial output records only the committed prefix and never reports a complete wire frame.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Rejected promised streams use the v0.118.0 orthogonal model: `PolicyDisposition::Rejecting` does not replace `WireStreamState`, reset state/reason/reservation, `RemoteClosureCause`, `TerminalValidation`, or active field-block ownership. `HeadersStart { end_stream: false, .. }` moves reserved(remote) to half-closed(local); with `end_stream: true` it then immediately closes, while `end_headers: false` retains CONTINUATION/HPACK and puts terminal validation pending. Remote END_STREAM before byte zero makes a queued policy CANCEL dormant, not permanently superseded. After full field/DATA validation, valid completion releases the slot; malformed message re-arms it as `StreamError(PROTOCOL_ERROR)`; connection-fatal HPACK commits shutdown. A partially serialized CANCEL remains immutable and precludes a second reset. After local reset commit, late HEADERS/CONTINUATION still synchronize HPACK, and tolerated late DATA uses connection credit only with no stream WINDOW_UPDATE.
- Define a total auditable `RejectedPushFrameDisposition` matrix keyed by policy disposition, RFC wire state, reset commit state/reason/reservation, remote-closure cause, terminal-validation state, normalized inbound event including END_STREAM/END_HEADERS, header-section phase, and active field-block ownership, with no wildcard/default cell. Every cell declares protocol legality and exact error code/scope, ordered wire transitions, HPACK/terminal-validation action, application visibility, stream/connection flow-credit action, reset dormancy/re-arm/release/serialization action, and connection-fatal override; generated/property tests enumerate the same key space so implementation and oracle cannot silently omit a combination.
...
- Test client-originated PUSH_PROMISE connection failure; every method/content/trailer/field-section case; TLS, cleartext, proxy, stale/cross-connection/cross-generation and foreign authority evidence without v0.190.0; sender/receiver associated-state perspectives; promised-ID, GOAWAY, reserved-slot/work and tombstone/cutoff exhaustion, reservation at MAX_CONCURRENT_STREAMS zero, open-time concurrency rejection, cancellation/atomic rollback/in-place rejection conversion/slot reclamation, and all earlier behavior. Exhaustively cross wire state, reset state/reason/output offset, terminal validation, Reset/EndStream closure cause, normalized events, END_STREAM/END_HEADERS, initial/informational/final/trailer phase, valid/malformed status/fields/Content-Length/body phase, HPACK success/failure, payload/padding credit, and connection races. Prove valid completion releases a dormant policy reset, malformed completion re-arms exactly one PROTOCOL_ERROR reset, begun CANCEL remains the sole immutable frame, connection-fatal HPACK wins boundedly, ordered transitions/credit/HPACK remain unchanged, and no block/reset slot is stranded or duplicated. Reset the associated stream immediately before/during/after fragmented PUSH_PROMISE, recycle buffers, and vary GOAWAY, invalid IDs, capacity, and mandatory-control backpressure; prove rejection-only handling never republishes authority and every record releases once after its validation/classification horizon.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Maintain stateful fuzz targets for HTTP/1 framing/transitions, HPACK tables/blocks, HTTP/2 frames/streams/flow/flood, Structured Fields, and translation; seed RFC/adversarial corpora, assert progress/capacity/no-panic/state-isolation oracles, minimize crashes, and replay every regression deterministically at the milestone gate. The HTTP/2 oracle spans rejection × wire state × reset state/reason/output offset × terminal validation × closure cause, associated-reset/PUSH_PROMISE ordering, HEADERS/DATA/CONTINUATION with END flags, message phase, status/field/Content-Length/body validity, payload/padding credit, HPACK success/failure, GOAWAY, connection failure, and capacity. It asserts error-action precedence, exact wire/HPACK/credit state, dormant-slot release/re-arm, bounded connection override, no stranded block, zero/one stream reset, continuous tracking, and no authority resurrection.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
