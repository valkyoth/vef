...
- Expose early-final policy preference without exposing progress or reuse
  ...
  `Http10PermitMintOutcome`, `Http10ReusePermit`, total reason-only-capacity
  `Http10SuccessorAdmissionOutcome`, the sole
  `Reusable -> ActiveExchange` successor transition, checked generation-
  exhaustion owner, output-phase persistence-loss action, and either-version
  stripping refinements;
  ...
  without atomic successor preflight; reset, increment, refund, or decrement
  the ledger except once on admission; consult or modify the ActiveExchange
  snapshot as authorization; conflate mint and admission failures; classify a
...
- Record early-final request generation, framing kind, disposition, bounded
  ...
  after count, immutable snapshot, permit deadline, successor outcome/reason,
  permit/input ownership, exact bodyless/fixed message-completion
  boundary, next-generation checked-exhaustion result, output-phase
  persistence-loss action, and pipeline-disabled state;
...
- Fuzz CONNECT 2xx, Upgrade/WebSocket 101, and the v0.163.0/v0.187.0/v0.188.0
  ...
  mint/admission reason, reason-only capacity retry with engine-retained permit,
  exact deadline/add-overflow in its correct phase,
...
- Reject caller construction, copying, rebinding, or direct mutation of
  ...
  duplicate decrement; zero/count/work/policy/deadline/correlation/generation
  ...
  failure consuming permit/input or decrementing the ledger; snapshot modified
  or consulted as authorization; head/nonfinal-record acknowledgement treated
...
- Independently size each configured early-final request's framing/progress/
  ...
  mutable remaining count, immutable ActiveExchange snapshot, permit deadline,
  separate mint and successor results/reasons/ownership, exact bodyless/fixed
  MessageCommitted gate plus negative framing state, checked next-generation
  exhaustion owner, output-phase persistence-loss action, and either-version
