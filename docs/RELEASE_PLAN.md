...
- Acceptance contract: Encode each field block under one bounded provisional `HpackEncoderTransaction`; table-size updates start the block, lookup/indexing/sensitive overrides are deterministic, and complete block-size/storage preflight occurs before output. Here `HpackCommitted` means publication of the whole transaction after its owner confirms complete field-block acknowledgement, not the later HTTP/2 `FramingCommitted` state created by first frame exposure. Private cancellation/supersession rolls back all provisional mutations, no later field block is encoded against them, NeedOutput/retry cannot double-insert, and connection abandonment discards the transaction with its compression context. v0.137.0 binds owner confirmation to full acknowledgement of the frame carrying END_HEADERS.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HPACK fuzz/model target now, including output-commit atomicity for encoder work.
- Prove no published table mutation before whole-block `HpackCommitted`, no double insert after NeedOutput, exact private rollback, serialized transaction ownership that blocks later field-block encoding, correct cancellation/partial-output/abandonment state, legal table-size-update ordering, bounded preflight, and bounded indexing cost.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Validate SETTINGS syntax, role/direction, known values, unknown identifiers, duplicate ordering, empty ACK payloads, and pending versus acknowledged local values; store peer advertisements without touching stream, window, encoder, admission, or scheduler state that does not yet exist. For each complete non-ACK frame, first validate every ordered entry with zero component mutation, then atomically reserve exactly one connection-owned `InboundSettingsTransaction { frame_generation, ordered_entries: SettingsEntryLease, ack: ReservedSettingsAck, pending_participants: SettingsParticipantSet, disposition: SettingsDisposition }`. The single ACK belongs to the complete frame regardless of entry count, duplicates, unknowns, or later participating subsystems. Process its entries in wire order without intervening frame dispatch; enabled components attach generation-bound participant obligations to the same transaction, and ACK eligibility requires every participant effective. Maintain transactions and ACK eligibility in received-frame FIFO order. A connection-fatal validation result creates no transaction or ACK and selects the exact connection error; a later participant failure selects AbortedConnection, cancels that transaction's ACK, prevents every component from emitting it independently, and commits the exact error/GOAWAY path. ACK-slot/transaction-capacity exhaustion before mutation selects typed bounded connection shutdown with no partial setting effect.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Acceptance contract: Arbitrate each valid peer SETTINGS_HEADER_TABLE_SIZE with `HpackEncoderTransaction` through a bounded connection-owned `PendingEncoderTableSizeTransition::{None, Pending { smallest_seen, final_value, owners: SettingsTransactionSet }}`. It never owns, counts, queues, or serializes SETTINGS ACKs; each generation-bound owner is the v0.108.0 frame-wide `InboundSettingsTransaction`, whose HPACK participant becomes Effective when this transition applies. With no active encoder transaction, atomically apply the received limit, complete the HPACK participant for each owner, and require the next field block to start with the necessary encoder size update. With Private unexposed HPACK, roll back provisional encoder mutations and complete field-block slots, apply the transition and complete its owner participants, then re-encode under the new limit; local re-encoding/storage failure is ordinary backpressure outside the settings transaction and cannot delay an ACK once all other frame participants are effective. With FramingCommitted, retain owner references while the pre-reserved block drains without interleaving; after final END_HEADERS-frame acknowledgement, publish the existing HpackCommitted transaction, apply the pending transition, complete each HPACK participant, and require the next block's first representations to carry the update; frame-wide ACK serialization remains FIFO and scheduler-owned. Across duplicate values, multiple entries, or multiple owning SETTINGS transactions while unresolved, retain the smallest observed maximum and final maximum in receive order; emit one HPACK size update when equal or the RFC-required smallest-then-final pair when different. A connection-fatal HPACK result reports failure to the owning transaction, which cancels its single ACK and selects connection shutdown. No later field block may encode while either a provisional transaction or pending encoder-size transition is unresolved. Treat peer MAX_HEADER_LIST_SIZE as outbound guidance while preserving independent hard inbound decode limits.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone.
- Cross no-active, Private, and every FramingCommitted initial/CONTINUATION/final acknowledgement prefix with one/multiple combined SETTINGS frames, duplicate HEADER_TABLE_SIZE entries, decreasing/increasing sequences, local re-encode capacity failure, another participant pending/failing, reset, cancellation, GOAWAY, and connection failure. Prove exact owner references without ACK ownership, Private rollback, old-transaction publication before pending transition, HPACK participant completion at the right boundary, frame ACK eligibility independent of re-encode success but dependent on every other participant, smallest/final update order at next-block start, no later stale/provisional encoding, and no double update/ACK.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Track `OutboundFieldBlock::{Private { stream, generation, hpack: HpackEncoderTransaction, initial, continuations }, FramingCommitted { stream, generation, hpack: HpackEncoderTransaction, remaining_continuations }, HpackCommitted, AbandonedWithConnection}` at connection scope. Define `field_fragment_cap = min(16_384, ResourceProfile::max_outbound_frame_payload, checked_sub(slot_capacity, 9))`; reject zero/overflow and require it to fit the enabled initial frame's mandatory payload prefix (four promised-stream-ID octets for PUSH_PROMISE and five priority octets only when that HEADERS form is enabled). Check `ResourceProfile::max_outbound_field_block_bytes` before encoding. Compute initial fragment capacity after its prefix, then compute `continuation_count = checked_ceil_div(remaining_encoded_block_bytes, field_fragment_cap)` and require it not exceed `ResourceProfile::max_outbound_continuations_per_block`. Before any initial HEADERS/PUSH_PROMISE exposure, fully encode the bounded block and atomically reserve/materialize the exact initial slot, every CONTINUATION slot, total checked arena bytes, and `checked_add(1, continuation_count)` queue entries. This actual local cap is no larger than the minimum legal peer MAX_FRAME_SIZE, so a valid peer reduction cannot require resegmentation; shortage or oversized locally generated fields returns typed local `OutboundFieldBlockCapacity`/validation failure with zero exposure. The whole Private block may then be superseded and all slots plus provisional HPACK mutations released together. First non-empty initial-frame exposure moves only to FramingCommitted; every remaining CONTINUATION is non-supersedable and its resources are already owned. No later field block is encoded against the provisional transaction or a pending encoder-table transition. Full acknowledgement of the frame carrying END_HEADERS atomically publishes `HpackCommitted` and releases block resources in order. No RST_STREAM, GOAWAY, PING ACK, SETTINGS ACK, WINDOW_UPDATE, or other-stream output can interleave. Fatal transport failure may abandon the transaction/block only with the entire connection and emits no interleaved GOAWAY. An END_STREAM completion hook on initial HEADERS still runs when that frame is fully acknowledged, independently of final HPACK commit. v0.145.0 applies the same lifecycle to PUSH_PROMISE.
...
- Arbitrate a newly required local reset against v0.137.0 ordinary output and `OutboundFieldBlock` before scheduling it. Supersede an unexposed Private field block or unrelated AcceptedPrivate frame in place, release its complete reserved slot/entry set, roll back provisional HPACK work, and release unexposed DATA reservations before queueing reset. Once the initial HEADERS/PUSH_PROMISE is exposed, every remaining CONTINUATION is protected by the FramingCommitted block obligation even while AcceptedPrivate; complete the pre-reserved block through END_HEADERS before reset serialization, then publish HpackCommitted only on final-frame acknowledgement. Outside a framing-committed block, Frozen ordinary output—even with zero acknowledged—retains its slot/debit and finishes its exact frame before reset. Completed ordinary output runs its hook once; connection failure may abandon committed output/HPACK only by abandoning the connection and cannot inject GOAWAY or invent completion/refund.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Acceptance contract: Track locally emitted SETTINGS frames in a bounded FIFO only after each complete frame's bytes commit; apply each received ACK to the oldest outstanding entry, reject unsolicited ACK with connection PROTOCOL_ERROR, support multiple outstanding entries, and generate timeout actions through injected monotonic time without hidden timers. Activate the v0.108.0 connection-wide `InboundSettingsTransaction` for received non-ACK frames: fully validate ordered entries; reserve its sole ACK before mutation; then attach HPACK, stream-window, MAX_FRAME_SIZE, admission/MAX_CONCURRENT_STREAMS, ENABLE_PUSH, known extension, and other enabled participant obligations to that frame generation. Apply entries in wire order without intervening frame processing, but make the ACK eligible only after every participant reports Effective. Several ready/pending transactions retain receive order for ACK serialization. Unknown settings attach no participant but remain covered by the same single ACK; duplicate entries never create another ACK. Any connection-fatal participant atomically marks AbortedConnection, cancels every unexposed pending frame ACK transaction, prevents early ACK from another subsystem, and selects one error/GOAWAY action. Local post-effect work such as re-encoding a rolled-back Private field block is not a participant and cannot delay an otherwise-ready ACK.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone.
- Build combined SETTINGS frames spanning HEADER_TABLE_SIZE, INITIAL_WINDOW_SIZE, MAX_FRAME_SIZE, MAX_CONCURRENT_STREAMS, ENABLE_PUSH, duplicate and unknown entries. Cross every entry order, Private/FramingCommitted HPACK state, negative-window/atomic rollback, DATA resegmentation, admission/push state, participant failure, ACK capacity, multiple queued transactions, cancellation, GOAWAY, and generation reuse. Prove one frame-wide ACK, all-participant effectiveness, FIFO ACK order, no mutation before validation/reservation, no partial ACK on fatal failure, and independence from local re-encode pressure.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Serialize encoder work and SETTINGS_HEADER_TABLE_SIZE transitions: no new field-block encoding is runnable while a Private/FramingCommitted transaction or `PendingEncoderTableSizeTransition` is unresolved. A setting supersedes Private work and completes only the HPACK participant before ordinary re-encoding; behind FramingCommitted, retain the owning SETTINGS transaction while completing the block, then apply the transition and report HPACK Effective. Schedule only the single frame-wide ACK after every participant is effective and in transaction FIFO order.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Acceptance contract: Reserve independent fixed capacity for required RST_STREAM, SETTINGS ACK, PING ACK, WINDOW_UPDATE, and GOAWAY commands plus their partially committed bytes; application output cannot consume it, cancellation cannot discard it, and valid required replies cannot be silently dropped; when the relevant reserved queue or generation-safe slot is exhausted, coalesce where RFC-safe and otherwise commit exactly one terminal bounded shutdown action with no duplicate control frame. Reserve exactly one ACK slot inside each validated `InboundSettingsTransaction`, never inside HPACK/window/admission/push/frame-size participants; retain transaction FIFO order and make that slot eligible only after all participants are Effective. Duplicate/mixed entries still share the slot even when HEADER_TABLE_SIZE collapses to smallest/final encoder updates. Participant failure cancels the transaction slot before exposure and selects connection shutdown; no other subsystem can race an ACK. Reservation guarantees eventual eligibility, not illegal interleaving: FramingCommitted defers eligible ACK serialization until final acknowledgement/HpackCommitted, while Private rollback can complete the HPACK participant and let the frame ACK proceed ahead of independently backpressured re-encoding. If transport failure prevents completion, abandon the connection and all reserved commands without attempting an interleaved GOAWAY.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Distinguish Private, FramingCommitted, HpackCommitted, AbandonedWithConnection, pending encoder-table transition, and frame-wide SETTINGS transaction disposition; report frame generation, ordered-entry count, participant kinds/pending count, one ACK-slot state, FIFO ordinal, redacted smallest/final table-size values, actual field fragment cap, mandatory prefix, encoded block bytes, continuation count, preflight demand, padding/data bytes, one-record token identity, and provisional/published HPACK generation without settings contents. Never attribute ACK ownership to a participant, diagnose first exposure as HPACK publication, collapse frame ACKs with size updates, or report an ACK after participant failure.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Add transitions for every combined-entry `InboundSettingsTransaction`: full validation/reservation, wire-order HPACK/window/frame-size/admission/push/extension participation, duplicate/unknown entries, participant pending/failure, transaction/ACK capacity, FIFO across frames, FramingCommitted delay, Private re-encode independence, smallest/final encoder updates, and exactly one ACK or connection-fatal cancellation. Also cover every local field cap/bound, HPACK state, checked arithmetic, and scalar/vectored cross-slot acknowledgement. No participant may emit an ACK and no generated acknowledgement may complete more than one output frame.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
...
- Reject construction/rebinding of `HpackEncoderTransaction`, `InboundSettingsTransaction`, `ReservedSettingsAck`, or `PendingEncoderTableSizeTransition`; participant duplication/rebinding, cross-generation completion, ACK extraction/emission, readiness before all participants, transaction reordering, and reuse after abort/complete; promotion from Private without exact preflight, caller HpackCommitted, bound bypass, cross-record acknowledgement, and vectored multi-slot lifetimes.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
