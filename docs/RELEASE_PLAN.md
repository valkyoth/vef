...
- Store policy disposition, current RFC wire state, local reset-output progress, remote-closure cause, and active field-block ownership as independent generation-checked fields; policy rejection or field-block completion alone never closes a wire stream. A locally reset stream's tombstone retains them through the closed-stream classification horizon, providing bounded storage for the v0.145.0 in-flight-push exception without retaining or recreating application authority.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Acceptance contract: Apply the idle/reserved/open/half-closed/closed/unknown stream matrix, allow only one active fragmented header block per connection, reject illegal interleaving, and handle stream-ID exhaustion deterministically. Represent rejection policy separately from `WireStreamState::{ReservedRemote, HalfClosedLocal, Closed}`, local reset output as `ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByRemoteClosure}`, remote closure as `RemoteClosureCause::{Reset, EndStream}`, and field-block ownership as its own dimension. Normalize `HeadersStart { end_stream, end_headers }`, `Data { end_stream }`, `Continuation { end_headers }`, and `PeerReset` before table lookup; process the carrying frame event and then its END_STREAM event in order. Rejection while reserved(remote) still permits HEADERS, rejects DATA as connection PROTOCOL_ERROR, and treats reuse of a non-idle promised ID as connection PROTOCOL_ERROR; remote END_STREAM or reset closes wire state without pretending the queued local reset committed.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone. Cover the Cartesian product of policy disposition, RFC wire state, reset progress, remote-closure cause, normalized frame event, END_STREAM, END_HEADERS, field-block ownership, and peer/local reset order; prove rejection never substitutes for a wire transition, closure never strands CONTINUATION, and flags never change required connection-versus-stream error scope.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Process each DATA payload's padding, flow-controlled length, semantic length, framing, and policy disposition before applying its normalized END_STREAM closure event. This ordered disposition lets v0.136.0 discard/credit a rejected half-closed(local) DATA+END_STREAM exactly once and then close, without losing terminal Content-Length or flow-control evidence.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Classify initial, informational, final, and trailer HEADERS independently from END_STREAM/END_HEADERS. A HEADERS END_STREAM event can close the wire stream before its fragmented initial/final/trailer block completes, but the block owner and HPACK obligation remain; trailer HEADERS require END_STREAM and close exactly once, while informational-response rules retain their existing exact semantic/error disposition.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Test Informational responses and trailers and all previously implemented relevant behavior with positive, negative, boundary, truncation, invalid-state, cancellation, capacity, and no-panic cases. Cross initial, informational, final, and trailer HEADERS with END_STREAM/END_HEADERS present and absent; prove the exact semantic/error outcome, two-step wire transition, mandatory CONTINUATION drain, and single terminal disposition.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Emit at most one RST_STREAM, account discarded DATA credit, tolerate bounded in-flight frames, discard queued output after reset, resolve END_STREAM/GOAWAY/shutdown races, and recycle only after connection effects. Track reset intent separately from RFC wire state with `ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByRemoteClosure}` and retain `RemoteClosureCause::{Reset, EndStream}` separately. A queued reset is withdrawable only while zero frame octets have committed: peer RST_STREAM or a normalized remote END_STREAM event closes the stream and supersedes the command with its exact cause, while connection failure drops it with zero emitted CANCEL frames. The first committed reset octet makes that exact frame non-withdrawable; if the connection remains writable, finish only its remaining octets once even if reset/END_STREAM concurrently closes the subjective inbound state, never enqueue a duplicate, and never resume it after connection failure. Field-block ownership survives either closure until END_HEADERS or connection failure. Commit retains the tombstone through late-frame classification.
- Preserve the phase invariant: HPACK encoder/decoder state tracks committed wire bytes; HTTP/2 activates, validates, publishes, mutates settings/state, cancels, and shuts down only through ordered bounded lifecycles.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 7541, RFC 9110, RFC 9113 including verified and held errata dispositions, RFC 9218 where introduced, and the HTTP/2 security considerations.
...
- Create or extend the matching HTTP/2 frame/state Kani or stateful fuzz harness at this milestone. Race peer RST_STREAM, HEADERS/DATA remote END_STREAM, connection failure, GOAWAY, cancellation, and every output split before byte zero, after every partial byte count, and after commit; require exactly one complete CANCEL when local serialization reaches commit, zero CANCEL bytes when remote closure or connection failure wins before serialization, and no duplicate, restart, or partial resume after supersession/failure. Cross END_HEADERS present/absent and prove no CONTINUATION obligation is stranded. A connection failure after partial output records only the committed prefix and never reports a complete wire frame.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Rejected promised streams use the v0.118.0 orthogonal model: `PolicyDisposition::Rejecting` does not replace `WireStreamState`, `ResetCommitState`, `RemoteClosureCause`, or active field-block ownership. `HeadersStart { end_stream: false, .. }` moves reserved(remote) to half-closed(local); with `end_stream: true` it then immediately closes, while `end_headers: false` leaves the CONTINUATION/HPACK obligation active. Rejected half-closed(local) `Data { end_stream: true }` and trailer HEADERS+END_STREAM discard/credit or synchronize their carrying frame before applying closure. Remote END_STREAM or reset before the first local CANCEL octet supersedes the queued reset with its cause; after serialization begins, finish that one frame if the connection survives. After local reset commit, late HEADERS/CONTINUATION still synchronize HPACK, and tolerated late DATA uses connection credit only with no stream WINDOW_UPDATE. No path generically treats all post-rejection frames as closed traffic.
- Define a total auditable `RejectedPushFrameDisposition` matrix keyed by policy disposition, RFC wire state, reset commit state, remote-closure cause, normalized inbound event including END_STREAM/END_HEADERS, header-section phase, and active field-block ownership, with no wildcard/default cell. Every cell declares protocol legality and exact error code/scope, ordered wire transitions, HPACK action, application visibility, stream/connection flow-credit action, remote-closure cause, and reset queue/serialization action; generated/property tests enumerate the same key space so implementation and oracle cannot silently omit a combination.
...
- Test client-originated PUSH_PROMISE connection failure; every method/content/trailer/field-section case; TLS, cleartext, proxy, stale/cross-connection/cross-generation and foreign authority evidence without v0.190.0; sender/receiver associated-state perspectives; promised-ID, GOAWAY, reserved-slot/work and tombstone/cutoff exhaustion, reservation at MAX_CONCURRENT_STREAMS zero, open-time concurrency rejection, cancellation/atomic rollback/in-place rejection conversion/slot reclamation, and all earlier behavior. Exhaustively cross rejection policy with reserved(remote)/half-closed(local)/closed wire states, queued/partially-serialized/committed/remote-closure-superseded reset status, Reset/EndStream closure cause, normalized HEADERS/DATA/CONTINUATION events, END_STREAM/END_HEADERS present/absent, initial/informational/final/trailer phase, payload/padding credit, and every CANCEL output boundary. Prove ordered one/two-step transitions, mandatory HPACK completion after wire closure, reserved(remote) DATA and duplicate-ID connection errors, exact flow credit, zero CANCEL when remote closure wins before byte zero, one complete CANCEL when local serialization commits, and no stranded block or duplicate reset. Reset the associated stream immediately before/during/after fragmented PUSH_PROMISE, recycle its request buffers, mutate HPACK, and vary connection failure, GOAWAY, invalid promised IDs, promised-slot exhaustion, and mandatory-control backpressure; prove rejection-only handling never republishes authority and every record releases once after its field-block/classification horizon.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Extend the v0.145.0 receiving-client PUSH_PROMISE transaction after the complete promised field block and all CONTINUATION input have decoded, HPACK is synchronized, and promised-request semantics, authority, safe/cacheable method, and caller push policy pass, but before promised-request publication. Atomically preflight/commit the existing promised-stream slot, matching `AssemblyInvalidationHandle`, independent pushed-provenance storage, and mandatory rejection-tombstone/cutoff tracking. If handle or provenance capacity is unavailable, retain the provisional slot, set only `PolicyDisposition::Rejecting`, leave its RFC wire state reserved(remote), set local CANCEL queued, and publish no promised request or assembly correlation. The ID is never momentarily untracked. Apply the exact v0.145.0 normalized-event matrix: HEADERS without END_STREAM moves to half-closed(local); HEADERS with END_STREAM then closes, while a missing END_HEADERS retains mandatory CONTINUATION/HPACK ownership; rejected half-closed(local) DATA+END_STREAM and trailer HEADERS+END_STREAM finish credit/synchronization before closure; reserved(remote) DATA and duplicate promised ID are connection PROTOCOL_ERROR; remote reset or END_STREAM supersedes only a zero-byte queued CANCEL with separate closure cause; partially serialized CANCEL completes once if the connection survives; and post-commit late DATA restores connection credit only. If mandatory rejection/cutoff tracking itself cannot be represented, retain the provisional slot and initiate typed bounded `PushRejectionTrackingUnavailable` connection shutdown—never release/reacquire or forget the promised ID. Every provisional slot, handle, provenance lease, reset command, field-block owner, tombstone, and shutdown record releases exactly once at its defined terminal horizon.
...
- Test Client correlation, cancellation, and retry tokens and all previously implemented relevant behavior with positive, negative, boundary, truncation, invalid-state, cancellation, capacity, and no-panic cases. Exhaust handles with concurrent HTTP/2 requests, accepted pushes, and stalled streams at every per-shard/per-connection limit; prove local `AssemblyInvalidationCapacity` emits no request bytes, creates no tracked correlation, applies resumable backpressure, never rotates/clears an existing arena, and cannot consume another shard's reserve. For push floods independently exhaust promised-slot/work, invalidation-handle, pushed-provenance, and rejection-tombstone/cutoff capacity; vary every transaction acquisition/failure interleaving, wire/reset/closure-cause combination, HEADERS/DATA END_STREAM, HEADERS END_HEADERS, initial/informational/final/trailer phase, peer RST_STREAM, duplicate promise, HPACK mutation, payload/padding credit, every CANCEL output boundary, GOAWAY/cutoff advancement, and typed shutdown. Prove continuous non-idle tracking, exact ordered RFC transitions/error scope, full field-block synchronization after closure, no promised publication/partial authority, exact rollback/release, exactly one complete CANCEL when local serialization commits, zero CANCEL bytes when remote closure or connection failure wins before serialization, and no duplicate/resumed reset or associated-stream/unrelated-context mutation. For accepted pushes close/reset/terminally deliver the associated stream immediately, recycle and overwrite its buffers, mutate principal/navigation/policy inputs, and complete multiple promised responses independently; prove their immutable arena-bound provenance and handles remain exact and each releases once. Exercise accepted-push reserved/open/informational/body/trailer states, pushed full 200 and 206 correlation handoff, cancellation at every phase, reset, GOAWAY, connection failure, terminal-event backpressure, retry disposition/new-handle/provenance reservation, duplicate release, stale/rebound/Copy/Clone attempts, and explicit whole-shard assembly disable/clear ordering; prove every admitted capability is released exactly once and distinct principal/tenant shard identities cannot share an arena.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance contract: Maintain stateful fuzz targets for HTTP/1 framing/transitions, HPACK tables/blocks, HTTP/2 frames/streams/flow/flood, Structured Fields, and translation; seed RFC/adversarial corpora, assert progress/capacity/no-panic/state-isolation oracles, minimize crashes, and replay every regression deterministically at the milestone gate. The HTTP/2 oracle spans rejection disposition × wire state × reset serialization progress × remote-closure cause, associated-reset/PUSH_PROMISE ordering, normalized HEADERS/DATA/CONTINUATION events, END_STREAM/END_HEADERS, header-section phase, payload/padding credit, every output split, HPACK mutation, identifier reuse, GOAWAY, and capacity exhaustion; it asserts exact ordered transitions/error scope, no stranded block, zero/one reset emission, continuous tracking, and no authority resurrection.
- Preserve the phase invariant: Role APIs expose validated authorized messages; translation emits nothing before the complete destination head/framing decision passes; retry and transition ownership are explicit.
- Update paragraph-addressable requirements, role/applicability decisions,
  SHOULD dispositions, deviations, and verified/held errata for
  RFC 3986, RFC 8441, RFC 9110 including Via, Max-Forwards, and TE, RFC 9111, RFC 9112, RFC 9113, RFC 9218, RFC 9651, and RFC 9931; RFC 7239 Forwarded transformation remains out of scope.
