...
| Never-untracked HTTP/2 rejection state | storage at `0.117.0`; terminal/output state at `0.118.0`/`0.120.0`; block handoff at `0.123.0`; semantic owners at `0.125.0`–`0.131.0`; push at `0.145.0`; assembly at `0.181.0`; diagnostics at `0.193.0` | Retain policy, wire, reset frozen record/token/prefix, terminal/section/workspace, closure, and block independently. Sealed transitions prevent aliasing or early release; failure transfers cleanup without forgetting IDs or overstating written bytes. |
| Rejected-push terminal validation and reset arbitration | model at `0.118.0`/`0.120.0`; error/abort precedence at `0.121.0`; semantic pipeline at `0.123.0`–`0.131.0`; credit at `0.136.0`; reset lifecycle/scheduling at `0.138.0`/`0.142.0`; total table at `0.145.0`; assembly at `0.181.0`; diagnostics/fuzz at `0.193.0`/`0.196.0` | Policy CANCEL can become dormant/re-arm only before exposure. First non-empty offer freezes the exact reset; token acknowledgement controls suffix progress, not identity. Peer/semantic/GOAWAY races cannot replace frozen bytes or emit a second reset. |
...
| HTTP/2 DATA and command ownership | `0.130.0`, `0.136.0`–`0.138.0` | Apply DATA framing/credit before its normalized END_STREAM closure; rejected DATA distinguishes reserved(remote), half-closed(local), and post-reset connection-only credit; outbound commands and partial reset output use one generation-safe lifecycle. |
...
| `0.117.0` | Generation-checked stream table and tombstones | Requires HTTP/2 stream-identifier rules; retains reset frozen record/token/prefix, terminal/HPACK/section, closure, and block ownership independently and prevents early reuse; Unlocks Exhaustive stream-state graph. |
| `0.118.0` | Exhaustive stream-state graph | Requires Generation-checked stream table and tombstones; makes PendingSemantics own exactly one sealed non-Copy/non-Clone TerminalFieldSectionLease and forbids alias/substitution/early release; Unlocks HTTP/2 activation preface, first-SETTINGS, and deadline sequencing. |
...
| `0.120.0` | HTTP/2 frame legality and fragmented-header-block sequencing | Requires HTTP/2 activation preface, first-SETTINGS, and deadline sequencing; defines ResetOutput Reserved/Frozen/Complete/Superseded with immutable 13-byte frame and offered/acknowledged cursors; Unlocks HTTP/2 error scope, typed deltas, and isolated stream mutation. |
| `0.121.0` | HTTP/2 error scope, typed deltas, and isolated stream mutation | Requires HTTP/2 frame legality and fragmented-header-block sequencing; permits reason replacement/peer suppression only before exposure and gives frozen output precedence without duplicate reset; Unlocks HTTP/2 graceful GOAWAY and bounded shutdown sequencing. |
...
| `0.136.0` | HTTP/2 inbound DATA ownership, acknowledgement, and credit release | Requires SETTINGS initial-window active-stream integration and atomic rollback; credits rejected half-closed(local) DATA+END_STREAM payload/padding exactly once before closure and keeps post-reset updates connection-only; Unlocks HTTP/2 outbound per-stream message command lifecycle. |
...
| `0.138.0` | HTTP/2 body cancellation, reset, and flow-credit lifecycle | Requires HTTP/2 outbound per-stream message command lifecycle; implements frozen reset token/ack cursor, pre-exposure-only arbitration, exact suffix, invalid-token neutrality, and acknowledged-only failure cleanup; Unlocks SETTINGS outstanding-ACK accounting. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.142.0 foundations; completes the table across ResetOutput/token/prefix, semantic/HPACK leases, peer/terminal/failure races, caller reuse, and generations; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; distinguishes exposure/offered/acknowledged/token/completion and never reports unacknowledged bytes or failed partial output as complete; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts reset exposure × prefix/token validity × peer/semantic/GOAWAY/failure × buffer/generation reuse and proves immutable bytes plus exact progress; Unlocks Compile-fail state and lifetime tests. |
