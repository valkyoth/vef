...
- Governed work uses `AdmissionAttempt::with_charged_window<R, E, F>(spec, f) -> Result<AdmissionWindowOutcome<R, E>, AdmissionWindowTerminalError>` with `F: for<'window> FnOnce(&'window mut ChargedWorkWindow<'window, 'command, T>) -> Result<R, E>`. The higher-ranked lifetime prevents `R` or `E` from borrowing the window, and the method owns the permit/window while the closure receives only a non-movable mutable borrow; early `?`, explicit `Err`, ordinary `Ok`, dropping the reference, or `core::mem::forget` of that reference cannot move/drop/forget the owned guard or bypass finalization. Byte/entry windows expose only the bounded command prefix derived privately from `AdmissionAttempt.command`; semantic, framing, output, and reservation windows expose only `take_unit()`, which must succeed before each operation. The original unbounded command/view is unavailable to validation code while the scope runs.
...
- Acceptance extension: Audit v0.75.0 as one causal product: exact generic `AdmissionWindowOutcome<R,E>` couples value/error to receipt; a separate terminal error owns poison; and the callback's higher-ranked window lifetime cannot escape through `R` or `E`. Treat outcome/receipt separation, poison encoded as an ordinary abort, closure-returned window/slice/iterator borrows, production unwind recovery, unwind represented as Completed/Aborted, missing poison on test unwind, movable/forgettable owned windows, finish-time arithmetic, parallel raw access, or uncharged/quadratic traversal as release-blocking.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz/Kani target now and retain its minimized corpus. Replay the complete HTTP/1 causal product. Cross generic Ok/Err values, higher-ranked borrow-escape compile failures, exact Completed/Aborted coupling, separate poison, production panic-abort configuration, and optional unwind injection at every window progress point. Require one inseparable result/receipt on closure return; no receipt and permanent poison after caught test unwind; no window-derived borrow in `R`/`E`; and no unwind recovery in production. Retain attempt/cursor maxima, corrupt kind, drop/forget, bounded prefix/unit, malformed input, retry, arithmetic, completion-binding, terminal-attribution, and lifecycle corpora.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- HTTP/1.0 diagnostics extension: report `Completed` with value metadata and receipt or `Aborted` with error metadata and receipt as one event; report terminal poison separately with no receipt. Never diagnose unwind as ordinary completion/abort. Receipt fields remain consistent and non-authoritative; diagnostics retain no window-derived borrow, command contents, or work authority.
...
- HTTP/1.0 fuzz extension: cross generic Ok/Err payloads, exact outcome/receipt coupling, higher-ranked borrow-escape attempts, scoped normal/early-`?`/abort/drop/forget behavior, poison, and optional unwind injection at every progress point. Assert production panic-abort, no unwind-to-success mapping, one receipt on closure return or none on poison, no `R`/`E` window borrow, and no missing finalization. Retain command-view, cursor, bounded-work, stale-work, fragmentation, retry, and arithmetic fuzzing.
...
- HTTP/1.0 role extension: neither role may choose callback lifetime/panic mode, borrow a window through `R`/`E`, separate result from receipt, map poison/unwind to ordinary outcome, or obtain command/work authority. Pentest higher-ranked escape, receipt separation, poison-as-abort, early `?`/drop/forget, optional caught unwind, missing/double finalization, preflight/cursor faults, stale work, retry bypass, and fragmentation amplification.
...
- HTTP/1.0 sizing extension: independently bound one private attempt, cursor preflight, higher-ranked scope/permit/window, generic Completed-or-Aborted payload plus exactly one receipt, separate poison cause/state, and optional test-unwind cleanup metadata. Production is panic-abort. No window-derived result borrow, outcome/receipt separation, unwind success outcome, owned guard escape, finish arithmetic, inconsistent receipt, duplicate finalization, or receipt authority exists.
