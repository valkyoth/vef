...
| Outbound field-block completion and HPACK commit | provisional transaction semantics at `0.98.0`; HEADERS preflight/ownership at `0.137.0`; reset arbitration at `0.138.0`; scheduler obligation at `0.142.0`; PUSH_PROMISE reuse at `0.145.0`; reserved controls at `0.153.0`; audit/diagnostics/fuzz/compile-fail at `0.154.0`/`0.193.0`/`0.196.0`/`0.197.0`; sizing replay at `0.211.0` | Before exposure, fully encode and reserve every slot/entry using fragments no larger than 16,384. Private rollback discards provisional HPACK; first exposure selects FramingCommitted, final END_HEADERS-frame acknowledgement alone publishes HpackCommitted, and only connection abandonment can stop an exposed block. |
...
| `0.137.0` | HTTP/2 outbound per-stream message command lifecycle | Requires inbound ownership plus both send ledgers; pre-materializes every 16,384-fragmented field-block slot/entry, separates Private/FramingCommitted/HpackCommitted, binds one token to one slot, and checks `9 + DATA payload` plus padding arithmetic; Unlocks HTTP/2 body cancellation, reset, and flow-credit lifecycle. |
...
| `0.142.0` | Bounded outbound scheduling | Requires SETTINGS max-concurrent-streams admission integration; exposes a field block only after complete slot/entry preflight and treats FramingCommitted as the highest obligation through final acknowledgement; Unlocks SETTINGS max-frame-size outbound integration. |
| `0.143.0` | SETTINGS max-frame-size outbound integration | Requires Bounded outbound scheduling; preserves 16,384-preflight field blocks across peer reductions and computes DATA from checked peer/local/credit/slot budget minus padding overhead without truncation; Unlocks GOAWAY cutoff and retry classification. |
...
| `0.145.0` | Server-push lifecycle | Requires GOAWAY cutoff plus v0.117.0–v0.142.0 foundations; applies complete preflight and Private/FramingCommitted/HpackCommitted ownership to outbound PUSH_PROMISE and completes rejected-push races; Unlocks ALPN and cleartext prior-knowledge selection. |
...
| `0.153.0` | Reserved control-output queues | Requires WINDOW_UPDATE churn defenses; keeps required control capacity reserved but serialization blocked behind FramingCommitted through final END_HEADERS acknowledgement/HpackCommitted; Unlocks HTTP/2 conformance audit and pentest. |
| `0.154.0` | HTTP/2 conformance audit and pentest | Requires Reserved control-output queues; audits field-block preflight/HPACK publication under every exhaustion/reduction/prefix, one-slot tokens, checked padding/slot arithmetic, independent END_STREAM completion, and atomic DATA credit; Unlocks Protocol-neutral HTTP translation representation. |
...
| `0.191.0` | Fixed-capacity caller-storage public API | Requires Authenticated origin authorization and HTTP/2 coalescing metadata; validates checked slot/padding policy and worst-case field-block capacity, exposes one-record token acknowledgement, and requires no allocator or peer-sized storage; Unlocks Optional alloc-backed convenience API. |
...
| `0.193.0` | Stable diagnostics and security events | Requires Optional alloc-backed convenience API; distinguishes Private/FramingCommitted/HpackCommitted, preflight ownership, one-record tokens, padding/data arithmetic, arena capacity, and DATA credit without content or false publication; Unlocks Feature and dependency-policy surface. |
...
| `0.196.0` | Adversarial and stateful fuzz campaign | Requires Multi-implementation interoperability; exhausts field-block preflight/HPACK transactions, peer reductions, one-slot tokens, checked padding/slot arithmetic, arena lifetimes, and dual-credit ledgers; Unlocks Compile-fail state and lifetime tests. |
| `0.197.0` | Compile-fail state and lifetime tests | Requires Adversarial and stateful fuzz campaign; rejects transaction promotion, incomplete preflight exposure, cross-slot/vectored token acknowledgement, staged-byte reuse, and early record recycling; Unlocks Long-running soak and exhaustion campaign. |
...
| `0.211.0` | Deterministic CPU, stack, code-size, and amplification budgets | Requires Aesynx kernel integration tests; sizes worst-case 16,384-fragmented blocks and provisional HPACK storage separately, then replays checked slot/padding bounds and independent byte/entry exhaustion; Unlocks 32-bit target campaign. |
