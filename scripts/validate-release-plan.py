...
def main() -> int:
    ...
    required_contract_text = (
        ...
        "separate bounded reserved-push-slot/work budget before reservation", "validated promised request, trusted authority/push-policy provenance, and reserved slot unpublished and rollback-capable", "at v0.145.0 its participants are the promised-stream slot/work reservation", "No v0.145.0-only pushed response has partial-retention or cross-request assembly authority",
        ...
        "A full union yields complete 200 with corrected Content-Length", "`ConflictingPartialContent`", "`RequestedOverlapBudget`", "`ActiveOverlapBudget`", "before sorting, output acquisition, or comparison", "cannot reset, clone, enlarge, or be replaced", "A 304, successful revalidation retaining the conflicting validator", "A new navigation or variant-selection identity creates a separate context", "`FullRepresentationFallback::{Keyed, Unkeyed}`", "For a locally initiated request when assembly is enabled, before serializing any request byte or creating its correlation", "engine-only fixed per-shard reserve that Vary fields", "typed local `AssemblyInvalidationCapacity`, emits zero request bytes, creates no correlation, never rotates or clears an existing arena", "ordinary Sans-I/O backpressure until capacity is released", "cannot continue as NoRecombine", "mandatory caller-supplied principal/tenant shard identity", "after the complete promised field block and all CONTINUATION input have decoded", "derive principal, privacy partition, tenant shard, and navigation namespace only from the associated locally initiated request", "Atomically reserve both the existing promised-stream slot and the matching `AssemblyInvalidationHandle`", "finish the entire field-block sequence", "bounded mandatory-control rejection tombstone", "stream-local `RST_STREAM(CANCEL)`", "synchronization-only HPACK path", "discard DATA with normal receive-credit accounting", "rollback releases each acquired slot/handle reservation exactly once", "linear non-Copy/non-Clone capability bound to exactly one ordinary or promised-request correlation", "terminal-event backpressure", "Release it exactly once only after final completion", "A retry cannot duplicate or rebind the old handle", "Fixed per-shard and per-connection assembly-admission limits", "one shard's exhaustion cannot borrow or consume another shard's reserve", "concurrent HTTP/2 requests, accepted pushes, and stalled streams", "push floods distinguish promised-slot/work exhaustion from invalidation-handle exhaustion", "response HEADERS/CONTINUATION/DATA arriving before RST commit", "duplicate release", "distinct principal/tenant shard identities cannot share an arena", "`ConservativeInvalidationScope::Reserved { handle: AssemblyInvalidationHandle", "`ConservativeInvalidationScope::AssemblyArenaGeneration(ArenaRotationCause)`", "`ArenaRotationCause::{DetectedSemanticCorruption, DetectedStorageCorruption, ExplicitCallerPolicy}`", "internal invariant breach or trusted-storage integrity failure", "malformed/semantic/conflicting peer input cannot mint a corruption cause", "before local request output or promised-request publication", "ordinary and pushed full-200 terminal evidence", "`AssemblyInvalidationCapacity`, `LeaseHeld`", "The invalidation handle remains non-Copy/non-Clone, exact ordinary-or-promised-correlation-bound", "ordinary-to-pushed or cross-associated-request rebinding", "promised publication without atomic slot/handle reservation", "`SemanticallyInvalid`", "`PhysicallyReclaimable`", "typed local `LeaseHeld`/capacity", "cannot rotate the arena or evict outside the reserved namespace", "no slot reuse or new mutable alias until every lease drops/acknowledges",
