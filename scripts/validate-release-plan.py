...
def main() -> int:
    ...
    superseded_http10_concrete = ("Maintain one engine-owned `Http10ReuseLedger { connection_generation, hop, remaining }`", "initialized once from the configured maximum and monotonically non-increasing", "A proxy constructs evidence and ledger independently on each hop", "Exact deadline equality (`now >= deadline`) is expired", "Generation uses checked increment only", "maximum/exhaustion closes the entire connection context", "Full acknowledgement of a nonfinal head/body record does not qualify", "zero, short, or full acknowledgement that does not reach `MessageCommitted` leaves successor input wholly unconsumed as `PrematureHttp10Successor`", "Test reuse maxima 0, 1, and N across complete exchanges", "exact `now == deadline`"); failures = [failure for failure in failures if not failure.startswith(("missing revised HTTP/1.0 security contract:", "missing HTTP/1.0 final-allowance/atomic-admission contract:", "missing HTTP/1.0 role/deadline/cleanup contract:")) and failure not in tuple(f"missing concrete security contract: {text}" for text in superseded_http10_concrete)]; required_http10_total_contracts = ("`Http10ReuseLedger { connection_generation, hop, configured_max, remaining }`", "Immutable `configured_max` distinguishes a disabled extension from an exhausted allowance", "`Http10ReuseResolutionOutcome::{Minted, CloseWithoutReuse { reason: Http10ReuseCloseReason }}`", "Resolution runs after both message lifecycles are terminal and accepts their optional, generation-bound evidence", "it is not preconditioned on successful negotiation", "correlation integrity, policy, framing, configured-zero, exhausted ledger, missing exact two-sided negotiation, checked deadline arithmetic, then `Minted`", "Only `Minted` requires and consumes both exact signals", "Every close-without-reuse reason is reachable", "`PendingHttp10SuccessorRequest`", "Validation rejects malformed syntax, illegal framing, semantic conflicts", "caller-supplied `Connection` values that conflict with mandatory close", "`Http10SuccessorAdmissionOutcome::{Admitted, RejectedLocalCommand", "`RejectedLocalCommand` covers invalid method/target/ordered fields, illegal framing, semantic invalidity", "it retains no caller borrow, accepts/exposes no command byte", "permits a corrected command before the same absolute deadline", "`Http10AdmissionAttemptWork`", "this work is never refunded on rejection or capacity retry", "If insufficient work remains before inspection", "Pure close-injection/output-capacity failure is retryable", "`Http10NextExchangeReservation::{Client { request: PendingHttp10SuccessorRequest, private_output, correlation, resources }, Server { parser, local_persistence_mode, correlation, resources }}`", "leaves the exact permit/deadline, input, ledger, generation, and output unchanged apart from already charged attempt work", "`Http10NormalCompletionTransaction`", "resolve reuse while correlation and optional evidence remain owned", "atomically publish exactly one terminal exchange event together with `Reusable` or closing state", "No intermediate state is application-visible", "ordinary cleanup cannot release correlation/evidence ahead of resolution", "If cancellation wins before publication, destroy/revoke any provisional permit", "Duplicate completion hooks are state-neutral", "Same-call successor input remains wholly unconsumed until the entire transaction publishes `Reusable`", "every ordered `Http10ReuseCloseReason`", "malformed/overlong method and target", "conflicting `Connection`, and close-injection capacity", "cancellation before every `Http10NormalCompletionTransaction` step", "exactly one terminal event plus state"); failures.extend(f"missing total HTTP/1.0 reuse/admission/completion contract: {text}" for text in required_http10_total_contracts if text not in detailed)
    if failures:
