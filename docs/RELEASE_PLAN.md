...
- `Http10TerminalDecision { reuse_resolution, terminal_interrupt: Option<Http10CompletionFirstCause>, planned_final_state: Http10PlannedFinalState }` preserves the reuse record and optional interrupt as independent dimensions. `Http10PlannedFinalState::{PublishReuse, PublishClose}` is derived, never caller-selected: PublishReuse is legal only for an unrevoked Reuse record with no interrupt; Close, SkippedByInterrupt, ReuseRevokedByInterrupt, or any accepted interrupt forces PublishClose. A Close(NegotiationUnavailable) followed by TransportFailure retains both facts. An interrupt after successful resolution records ReuseRevokedByInterrupt rather than erasing success. Only final publication constructs the actual connection state.
...
- Define sealed granular `AdmissionWorkKind::{CommandBase, MethodByte, TargetByte, FieldEntry, FieldByte, FramingStep, SemanticRule, CloseInjectionByte, OutputPreflightByte, ReservationComponent}` and engine-only `AdmissionWorkChargeSpec { kind, units }`. The fixed schedule maps each kind to checked `(base_cost, per_unit_cost)` and derives `cost = base_cost + per_unit_cost * units`; callers can construct neither kind nor spec and select neither units nor cost. The engine derives bounded units solely from newly processed progress: zero-length stages pay their base kind, byte/entry kinds charge only actual bounded units, one-byte fragmentation retains the cursor, and any retry/rescan is charged again against the non-resetting ledgers. No scan, field traversal, output byte, reservation component, or repeated retry may occur outside a charge.
- Atomic `try_charge_admission_work(spec) -> Result<AdmissionWorkCharge, AdmissionWorkChargeError>` retains errors `PermitBudgetExhausted`, `ConnectionBudgetExhausted`, `InvalidZeroCost`, `GenerationMismatch`, `LedgerInvariantFailure`, and `ArithmeticOverflow` with existing precedence/classification. It checks unit bounds and multiplication/addition before both ledgers, then mutates both or neither. Every error performs no governed work. `AdmissionWorkCharge { kind, units, cost, permit_generation, connection_generation, permit_consumed_after, connection_consumed_after }` is diagnostic only. Successful admission transfers totals without recharge.
...
- `ActiveExchange` exclusively owns its records/evidence/leases. Entering Completing checked-allocates a fresh non-wrapping `completion_generation`; `Http10CompletingPhase::{Resolving, DecisionHeld { terminal_decision }, Reclaiming { terminal_decision, reclamation_state }, PublicationPending { publication }}` binds every phase to `Http10CompletionBinding { connection_generation, exchange_generation, completion_generation }`. Define `Http10CompletionInterrupt { binding, cause: Http10CompletionInterruptCause }`, where causes are `IdleDeadlineReached { permit_generation, deadline_generation, observed_now }`, `PolicyRevoked { policy_generation }`, `TransportFailure { transport_generation, cause }`, `LocalClose { close_command_generation }`, and `Cancellation { cancellation_generation }`. Every cause must match the common binding before cause-specific evidence is inspected or latched. Stale/wrong-binding LocalClose, Cancellation, PolicyRevoked, deadline, and transport notifications are state-neutral during every phase, after final publication, and after successor admission.
- The first exact valid interrupt is immutably latched in `Http10CompletionFirstCause`; same-call ties retain precedence TransportFailure, PolicyRevoked, IdleDeadlineReached, LocalClose, Cancellation. Exact deadline equality expires. In Resolving, an accepted interrupt records SkippedByInterrupt. In DecisionHeld/Reclaiming, Close reasons remain unchanged while the interrupt is added; Reuse is destroyed and becomes ReuseRevokedByInterrupt. PublicationPending rewrites the event/planned state while preserving both records. Every phase remains capacity-infallible, never reruns resolution/reclamation, and cannot publish expired/revoked reuse.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Test every work kind at zero, one, and maximum units; maximum method/target/field bytes and field count; one-byte fragmentation; malformed/capacity retry; and all checked multiplication/addition/error boundaries. Prove cursors charge only newly processed units, every rescan/retry charges again, cumulative ceilings prevent amplification, and no quadratic/unpriced traversal escapes.
- Cross every interrupt with exact and stale common connection/exchange/completion bindings, every cause-specific generation, each phase, after publication, and after successor admission. Cross normal Close reasons plus later interrupts, Reuse plus interrupt revocation, and interrupt-before-resolution; require independent resolution/terminal records, immutable first cause, correct planned close, destroyed permit authority, and no information collapse. Retain all initial-role ownership tests.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- Acceptance extension: Audit v0.75.0 as one causal product: exact common connection/exchange/completion binding plus cause evidence for every interrupt; independent reuse-resolution record, optional first interrupt, and derived final plan; and granular sealed unit specs with checked base-plus-per-unit pricing/progress cursors. Treat stale LocalClose/Cancellation/Policy replay, attribution collapse, revoked identity retaining authority, caller-selected units/cost, cursor rewind, free retry/rescan, or quadratic traversal as release-blocking.
...
- Create or extend the relevant stateful HTTP/1/intermediary fuzz target now and retain its minimized corpus. Replay the complete `Connection` corpus and HTTP/1.0 causal product. Cross common/cause interrupt bindings in every phase and after publication/successor; every resolution record plus later interrupt; and all granular work kinds at zero/one/max units, maximum fields/bytes, one-byte fragmentation, retries/rescans, and arithmetic boundaries. Require stale neutrality, independent attribution, revoked permit destruction, derived final state, monotonic cursors, charged rescans, bounded linear work, and one reclamation/publication. Preserve all earlier HTTP/1 lifecycle, transition, and rejection corpora.
- No test may require a later-version capability; previously established resource ceilings remain release-blocking.
- Prove failures do not publish partial state, mutate unrelated state, exceed
  active work/output limits, or require hidden allocation.
- Run Rust `1.90.0`–`1.97.1`, `no_std`, target, docs/package, dependency policy,
...
- HTTP/1.0 API extension: expose neither common/cause interrupt binding, completion generation, resolution/interrupt/planned-state constructors, revoked reuse identity, work kind/spec/units, nor progress cursors. Callers cannot replay stale close/cancel/policy, collapse attribution, select work quantity/cost, rewind progress, or request an uncharged rescan.
...
- HTTP/1.0 diagnostics extension: report common and cause-specific binding comparison, resolution record, optional first interrupt, planned final state, work kind/units/base/per-unit/total, progress cursor, retry/rescan charge, and typed error. Revoked reuse identity and diagnostics never grant authority.
...
- HTTP/1.0 fuzz extension: cross exact/stale common bindings and every cause generation in all phases, after publication and successor admission; combine Close/Reuse/Skipped resolution with interrupts; and fuzz zero/one/max units, max fields/bytes, one-byte fragments, retries/rescans, and arithmetic errors. Assert independent attribution, revoked permit destruction, cursor monotonicity, and no unpriced/quadratic work.
...
- HTTP/1.0 compile-fail extension: reject common/cause binding creation/rebinding, resolution/interrupt/final-plan conflation, revoked reuse identity conversion to permit, work-kind/spec/unit/cursor construction, numeric charge entry, cursor rewind, and traversal without a linear charge.
...
- HTTP/1.0 soak extension: cycle stale interrupt replays across many completion/exchange generations and every resolution/interrupt pair; run maximum fragmented methods/targets/fields/output and repeated retries/rescans until cumulative exhaustion. Require no ABA, attribution loss, revoked authority, free rescan, quadratic growth, or accounting drift.
...
- HTTP/1.0 role extension: neither role may forge common/cause binding, collapse resolution with interrupt, use revoked identity, derive units/cost, or control cursors. Pentest stale LocalClose/Cancellation/Policy replay, attribution erasure, permit resurrection, zero-unit underpricing, cursor rewind, retry bypass, and fragmentation amplification.
...
- HTTP/1.0 sizing extension: independently bound common/cause bindings, completion-generation domain, resolution/interrupt/planned-state records, revoked identity, granular kinds/spec units/base/per-unit calculation, progress cursors, fragmentation, and retry rescans. Duplicate binding, identity, spec, cursor, charge, or permit ownership is forbidden.
