...
def main() -> int:
    ...
    required_contract_text = (
        ...
        "separate bounded reserved-push-slot/work budget before reservation", "preflight tombstone and cutoff capacity before releasing its slot or convert that slot in place", "continuously represented as a generation-bound slot, tombstone, or connection-shutdown record", "Store policy disposition, current RFC wire state, local reset-output progress/reason/reservation, remote-closure cause, terminal-validation state/stage, and active field-block ownership as independent generation-checked fields", "Normalize END_STREAM as a state-transition event separate from its carrying HEADERS or DATA event", "`HeadersStart { end_stream, end_headers }`", "CONTINUATION never changes wire state", "`ResetCommitState::{Queued, PartiallySerialized, Committed, SupersededByPeerReset, PolicyDormantAfterEndStream}`", "`ResetReason::{PolicyCancel, StreamError(StreamErrorCode)}`", "`TerminalValidation::{NotTerminal, PendingFieldBlock(PendingBlockDisposition), PendingSemantics(SemanticStage), Valid, Malformed(StreamErrorCode), AbortedByPeerReset}`", "`PendingBlockDisposition::{ValidateSemantics, AbortAfterHpackByPeerReset}`", "`SemanticStage::{PseudoFields, ConnectionFields, FieldAndContext, RequestMapping, ResponseMapping, ContentAndPhase, TrailerAndRoleRules}`", "Permit only sealed/private transition constructors", "HEADERS+END_STREAM without END_HEADERS sets `PendingFieldBlock(ValidateSemantics)`", "Successful HPACK completion transfers ownership exactly once to `PendingSemantics(SemanticStage::PseudoFields)`, never directly to `Valid`", "Each stage uses pre-reserved bounded work/storage", "Peer reset during fragmentation changes only the disposition to `AbortAfterHpackByPeerReset`", "Connection-fatal teardown is connection-owned rather than a stream enum variant", "`RemoteClosureCause::{Reset, EndStream}`", "Any informational 1xx HEADERS carrying END_STREAM is terminal `Malformed(PROTOCOL_ERROR)` and requires stream RST_STREAM(PROTOCOL_ERROR)", "HEADERS with END_STREAM but without END_HEADERS records closed wire state immediately", "A valid terminal DATA for which this is the final applicable owner marks `TerminalValidation::Valid` and releases a dormant policy-reset slot only after credit", "another terminal message violation marks `Malformed(PROTOCOL_ERROR)` and re-arms that slot", "A zero-byte policy-CANCEL reservation can become dormant after remote END_STREAM without freeing its slot", "an uncommitted `StreamError(code)` replaces/re-arms a dormant or queued `PolicyCancel` in the same reserved slot", "partially serialized or committed RST_STREAM bytes are immutable", "resumable local capacity pressure leaves the same pending stage and dormant reservation intact", "typed bounded `StreamTrackingUnavailable` connection shutdown", "validated promised request, trusted authority/push-policy provenance, and reserved slot unpublished and rollback-capable", "promised slot includes or pre-reserves its v0.117.0 in-place rejection-tombstone/cutoff representation", "At v0.145.0 the transaction's participants are the promised-stream slot/work and rejection tracking", "Rejected promised streams use the v0.118.0 orthogonal model", "total auditable `RejectedPushFrameDisposition` matrix", "normalized inbound event including END_STREAM/END_HEADERS", "header-section phase", "terminal-validation state and `SemanticStage`", "with no wildcard/default cell", "reset commit state/reason/reservation", "Every cell declares protocol legality and exact error code/scope, ordered wire transitions, HPACK/semantic-stage action, application visibility, stream/connection flow-credit action, reset dormancy/re-arm/release/serialization action, peer-reset abort/deferred-abort action, and connection-fatal cleanup owner", "Implement RFC 9113's in-flight PUSH_PROMISE exception for an associated stream with a locally initiated reset", "accept and completely decode PUSH_PROMISE plus every CONTINUATION", "publish no promised request, and recreate no application, authority, push-policy, invalidation, or assembly provenance", "Missing recycled associated-request provenance selects safe promised-stream rejection", "Illegal promised IDs remain connection PROTOCOL_ERROR and malformed HPACK remains connection-fatal", "No v0.145.0-only pushed response has partial-retention or cross-request assembly authority",
